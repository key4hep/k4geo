...
//
//==================================================================
  G4double SDHcalEndcap::BirkAttenuation(const G4Step* aStep)
  {
...
//
//==================================================================
  void SDHcalEndcap::DumpHit(G4Step* aStep)
  {
...
#ifdef SDHcalEndcap_DEBUG
    G4cout << " SDHcalEndcap::DumpHit -  working on --- " << name() <<"  currentLayer = " << currentLayer << G4endl; 
    G4cout << " ---------------------------------------------------------  "    << G4endl;
#endif
    ...
    localMiddlePoint.setY(0.0);                                       // local thickness  center of SD
    localMiddlePoint.setZ( double(jID)*_cellSize.at(currentLayer-1)); // local cartensian center of SD
...
//
//==================================================================
  void SDHcalEndcap::Clear()
  {
#ifdef SDHcalEndcap_DEBUG
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    G4cout << name()
	   << "====> SDHcalEndcap::Clear " << currentLayer
	   << G4endl;
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#endif
  ...
  void SDHcalEndcap::StartNewHit(G4int aLayerNumber,
  ...
  void SDHcalEndcap::UpdateHit(G4Step *aStep)
  {
#ifdef SDHcalEndcap_DEBUG
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    G4cout << name()
	   << "====> SDHcalEndcap::UpdateHit " << currentLayer
	   << G4endl;
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#endif
    DepositedEnergy+=aStep->GetTotalEnergyDeposit();
...
//
//==================================================================
  bool SDHcalEndcap::process(G4Step* aStep,G4TouchableHistory* ) 
  {
    ...
    if (energyDeposition >0 )
      {      
	G4cout << "\n\n #####################################################################  "    << G4endl; 
	G4cout << "             SDHcalEndcap::process -  start...........................  "   << G4endl; 
	G4cout << "             SDHcalEndcap::process -  energyDeposition : "<<energyDeposition   << G4endl;
      }
#endif
    ...
    if( preID == 0 ) {
      ...
      G4cout << " #####################################################################  "    << G4endl; 
      G4cout << "  WARNING:   SDHcalEndcap::process -  invalid prestep volumeID : " << preID  << G4endl; 
      G4cout << " #####################################################################  "    << G4endl;
...
DECLARE_GEANT4SENSITIVE(SDHcalEndcap)
