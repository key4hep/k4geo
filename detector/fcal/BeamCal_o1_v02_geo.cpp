...
static dd4hep::Ref_t create_detector(dd4hep::Detector& theDetector, dd4hep::xml::Handle_t element,
  ...
  if (theDetector.buildType() == dd4hep::BUILD_ENVELOPE) {
    std::cout << "Building envelope.\n";
    return sdet;
  } else {
    std::cout << "Building all.\n";
  }
  ...
  dd4hep::Tube incomingBeamPipe(0.0, incomingBeamPipeRadius,
                                bcalThickness); // we want this to be longer than the BeamCal
  ...
  dd4hep::Position incomingBeamPipeAtEndOfBeamCalPosition(-incomingBeamPipeRadius, incomingBeamPipeRadius,
                                                          bcalInnerZ + bcalThickness);
  // This Rotation needs to be the fullCrossing angle, because the incoming beampipe has that much to the outgoing beam
  // pipe And the BeamCal is centred on the outgoing beampipe
  incomingBeamPipeAtEndOfBeamCalPosition =
      dd4hep::RotationY(-fullCrossingAngle) * incomingBeamPipeAtEndOfBeamCalPosition;
    ...
    for (int i = 0, repeat = xmlLayer.repeat(); i < repeat; ++i) {
      ...
      for (dd4hep::xml::Collection_t collSlice(xmlLayer, _U(slice)); collSlice; ++collSlice) {
        ...
        if (isAbsorberStructure) {
          ...
          const double thisPositionZ =
              bcalCentreZ + referencePosition + 0.5 * layerThickness + inThisLayerPosition + slice_thickness * 0.5;
        ...
        dd4hep::PlacedVolume pv =
            layer_vol.placeVolume(slice_vol, dd4hep::Position(0, 0, inThisLayerPosition + slice_thickness * 0.5));
  ...
  const dd4hep::Rotation3D bcBackwardRot(
      dd4hep::RotationZYX((180 * dd4hep::degree), (180 * dd4hep::degree - fullCrossingAngle * 0.5), (0.0)));

  dd4hep::PlacedVolume pv = envelope.placeVolume(envelopeVol, dd4hep::Transform3D(bcForwardRot, bcForwardPos));
  pv.addPhysVolID("barrel", 1);
  beamCalDE_1.setPlacement(pv);
  dd4hep::PlacedVolume pv2 = envelope.placeVolume(envelopeVol, dd4hep::Transform3D(bcBackwardRot, bcBackwardPos));
