...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  if (lcalExtraS > 0.) {
    ...
    Tube mixFE(lcalOuterR, lcalOuterR + lcalExtraS - 0.2 * dd4hep::cm, 0.5 * lcalThickness, -phiSpanFE / 2.,
               phiSpanFE / 2.);
  ...
  std::cout << "   LumiCal length : " << mtotalDepthZ / dd4hep::mm << " ( rad. length X0: " << mtotalRadLen << "   )"
            << std::endl;
