...
static Ref_t create_detector(Detector& theDetector, xml_h e, SensitiveDetector sens) {
  ...
  double tb = (outer_r / std::cos(hphi) * std::cos(hphi + 2 * dphi) > inner_r)
                  ? (outer_r - outer_r / std::cos(hphi) * std::cos(hphi + 2 * dphi) - tc)
                  : (outer_r - inner_r - tc);
  ...
  for (int m = 1; m < 3; m++) {
    ...
    case 1:
      ...
      y_off = (outer_r / std::cos(hphi) * std::cos(hphi + 2 * dphi) > inner_r)
                  ? (l_i / 2 + tb + tc / 2 + outer_r / std::cos(hphi) * std::cos(hphi + 2 * dphi) - inner_r)
                  : (l_i / 2 + tb + tc / 2);
      break;
    case 2:
      y1 = tol * tb / 2;
      ...
      y_off = (outer_r / std::cos(hphi) * std::cos(hphi + 2 * dphi) > inner_r)
                  ? (l_i / 2 + tb / 2 + outer_r / std::cos(hphi) * std::cos(hphi + 2 * dphi) - inner_r)
                  : (l_i / 2 + tb / 2);
      break;
    case 3:
    default:
