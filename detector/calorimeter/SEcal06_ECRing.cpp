...
/*
  ...
  SEcal06_ECRing
  ===============
  remove reference to paramters not directly connected to the EC ring (previously some were still here)
  offset of central hole is an input param, not calculated within driver
  ...
  SEcal05_ECRing driver corrects this (applies offset according to the crossing algle, to center it on the outgoing
  beampipe). it also deals with the presence or not of a preshower layer

  this is a hack of the 04 version, not a clean rewrite as was done for barrel and endcap, so it's not very clean &
  tidy....but I think it works
...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  cout << "---------------------------------" << endl;
  cout << " creating Ecal ECRing ( SEcal06_ECRing ) " << endl;
  cout << "---------------------------------" << endl;
  ...
  for (int i = 0; i < Ecal_nlayers1 + Ecal_nlayers2 + Ecal_nlayers3; i++) {
    ...
    if (inStructure) {
      ...
      if (i >= Ecal_nlayers1)
        thickness = Ecal_radiator_thickness2;
      if (i >= Ecal_nlayers1 + Ecal_nlayers2)
        thickness = Ecal_radiator_thickness3;
  ...
  for (xml_coll_t li(x_det, _U(layer)); li; ++li) { // types of layers (i.e. thin/thick absorber) or "stack"
    ...
    for (int j = 0; j < x_layer.repeat(); j++) { // layers within this type (or "stack")
      ...
      module_thickness +=
          thisthick + 2 * Ecal_fiber_thickness_alveolus; // slab thickness, and the alveolar wall around it
      l_num2++;
    }
  }

  if (fabs(Ecal_barrel_thickness - module_thickness) > 0.1) {
    cout << "ERROR EC ecal thickness inconsistency: calculated, nominal = " << module_thickness << " "
         << Ecal_barrel_thickness << endl;
    assert(0);
  }
      ...
      for (int j = 0; j < repeat; j++) {
        ...
        if (isFrontFace) { // the first part of the module depends on whether we have preshwer or not
          if (Ecal_Barrel_PreshowerLayer == 1) { // don't include W+CF wrapping; only one side of alveolus
            ...
            // rad_pos_Z = radiator_dim_Z/2. + this_struct_CFthick_beforeAbs; // distance from top surface of structure
            // to centre of radiator
            absorber_index++;
          }
          ...
          if (absorber_index < Ecal_nlayers1)
            radiator_dim_Z = Ecal_radiator_thickness1;
          else if (absorber_index < Ecal_nlayers1 + Ecal_nlayers2)
            radiator_dim_Z = Ecal_radiator_thickness2;
          else if (absorber_index < Ecal_nlayers1 + Ecal_nlayers2 + Ecal_nlayers3)
            radiator_dim_Z = Ecal_radiator_thickness3;
          else {
        ...
        if (radiator_dim_y > 0) {
          ...
          string bs_name = "bs";
          Box EndcapStructureLayer_box(ECRingSiplateSize / 2. - tolerance, ECRingSiplateSize / 2. - tolerance,
                                       radiator_dim_y / 2.);
          SubtractionSolid EndcapStructureLayerSolid(EndcapStructureLayer_box, CenterECTubForSi, hole_position);
          Volume EndcapStructureLayer_vol(det_name + "_" + l_name + "_" + bs_name, EndcapStructureLayerSolid,
        ...
        double l_thickness =
            layering.layer(l_num - 1)->thickness(); // Layer's thickness. this is the internal thickness of the alveolus
        ...
        Box ECRingSiBox(ECRingSiplateSize / 2. - tolerance, ECRingSiplateSize / 2. - tolerance,
                        l_thickness / 2.0 - tolerance);
          ...
          if (x_slice.materialStr().compare(x_staves.materialStr()) == 0) {
            ...
            if (Ecal_Barrel_PreshowerLayer == 0 || !isFirstSens) { // add this layer to the caloData (unless its the
              ...
              if (module_num == 0) {
                ...
                std::cout << " caloLayer.cellSize0, 1: " << caloLayer.cellSize0 << " " << caloLayer.cellSize1
                          << std::endl;
        ...
        if (absorber_index < Ecal_nlayers1)
          radiator_dim_y = Ecal_radiator_thickness1;
        else if (absorber_index < Ecal_nlayers1 + Ecal_nlayers2)
          radiator_dim_y = Ecal_radiator_thickness2;
        else if (absorber_index < Ecal_nlayers1 + Ecal_nlayers2 + Ecal_nlayers3)
          radiator_dim_y = Ecal_radiator_thickness3;
        else {
          radiator_dim_y = 0;
        }
...
DECLARE_DETELEMENT(SEcal06_ECRing, create_detector)
