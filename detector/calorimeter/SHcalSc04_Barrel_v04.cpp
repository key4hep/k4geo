...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  printout(dd4hep::DEBUG, "SHcalSc04_Barrel_v04", "TPC_outer_radius : %e   - Ecal_outer_radius: %e ", TPC_outer_radius,
           Ecal_outer_radius);
  ...
  std::vector<double> cellSizeVector =
      seg.cellDimensions(encoder.getValue()); // Assume uniform cell sizes, provide dummy cellID
  ...
  for (int layer_id = 1; layer_id <= (2 * Hcal_nlayers); layer_id++) {
    ...
    printout(dd4hep::DEBUG, "SHcalSc04_Barrel_v04", " chamber x:y:z:  %e:%e:%e", x_length * 2., z_width * 2.,
             y_height * 2.);
    ...
    for (xml_coll_t k(x_layer, _U(slice)); k; ++k) {
      ...
      if (x_slice.isSensitive()) {
        ...
        printout(dd4hep::DEBUG, "SHcalSc04_Barrel_v04", "  logical_layer_id:  %d  tower_id:  %d", logical_layer_id,
                 tower_id);
      }
