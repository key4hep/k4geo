...
static dd4hep::Ref_t create_detector_SCEPCal_MainLayer(dd4hep::Detector& theDetector, xml_h xmlElement,
  ...
  auto getSingleCrystalVertices = [&](int i, int j, int xtalDiv, const double* vertices, bool reflected,
      ...
      for (int i = 0; i < XTAL_DIV_F; i++) {
        for (int j = 0; j < XTAL_DIV_F; j++) {
          ...
          CreateEightPointShapeVolume_SetVolAttributes_Place_SetCellId(
              ...
              ENDCAP_SYSTEM_NO, 2 * N_THETA_ENDCAP + N_THETA_BARREL - 1 - iTheta, nGamma, nEpsilon, 0, posGlobal_1, ENDCAP_PHI_START, ENDCAP_PHI_END, phi_endcap_rotations);
          numCrystalsEndcap += 1;
        }
      }
