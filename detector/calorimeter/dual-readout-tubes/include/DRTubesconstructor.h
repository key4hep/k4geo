...
public:
  ...
  // Function to calculate all parameters which depend on theta (both tower theta and the theta that has been covered by
  // placing towers) Since this is different for each tower, this function is called multiple times
  ...
  // Same as trap width, but for the tower volume (so dimension of "air" insdie the support structure which is the
  // _trap_ volume)
