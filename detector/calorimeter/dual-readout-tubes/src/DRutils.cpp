...
// Fast rounding function which do not do some safety checks the std:: functions provide, since we don't expect problems
// with the input
int fast_floor(double x) { return (int)x - (x < (int)x); }

int fast_ceil(double x) { return (int)x + (x > (int)x); }

// Check if a double is an integer within a certain tolerance
bool check_for_integer(double x) { return (std::abs(x - std::round(x)) < 1e-6); }
...
Position get_intersection(const std::vector<double>& plane_coefficients, const Position& line_point,
  ...
  double t = (-1.0 * (A * line_point.x() + B * line_point.y() + C * line_point.z() + D)) /
             (A * line_direction.x() + B * line_direction.y() + C * line_direction.z());
