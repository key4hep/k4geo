...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens) {
  ...
  std::cout << "--> DREndcapTubes: from XML description: innerR " << innerR / m << " m, tower length "
            << tower_height / m << " m, z-rotations " << NbOfZRot;
  ...
  Volume core_SLog("DRETScoreS_Log", core_S, description.material(x_core_S.attr<std::string>(_U(material))));
  core_SLog.setVisAttributes(description, x_core_S.visStr());
  if (x_core_S_sens)
    core_SLog.setSensitiveDetector(sens);
  ...
  Volume clad_CLog("DRETSclad_CLog", clad_C, description.material(x_clad_C.attr<std::string>(_U(material))));
  clad_CLog.setVisAttributes(description, x_clad_C.visStr());
  if (x_clad_C_sens)
    clad_CLog.setSensitiveDetector(sens);
  ...
  Volume core_CLog("DRETScoreC_Log", core_C, description.material(x_core_C.attr<std::string>(_U(material))));
  core_CLog.setVisAttributes(description, x_core_C.visStr());
  if (x_core_C_sens)
    core_CLog.setSensitiveDetector(sens);
  ...
  for (int i = 0; i < NbOfEndcapReduced; i++) {
    ...
    if (i <
        NbOfEndcapReduced) { // Place towers up to 35, this "if" is just a protection in case the loop range is changed
#ifndef COUNTTUBES
    ...
    for (std::size_t k = 0; k < 200; k++) {
      ...
      if (k == 0)
        y_tube = y_backplane + tubeRadius + 1.0 * mm; // add 1 mm otherwise the tube immediately intersect with a plane
      else {
      ...
      for (std::size_t j = 0; j < 1000; j++) {
        ...
        if (std::fabs(capillaryLength - length) < 0.0001 * mm) {
          ...
          Tube capillaryShort(0. * mm, tubeRadius, (capillaryLength - TubeLengthOffset) / 2.,
                              2 * M_PI); // reduced capillary length
                                         // by a fixed offset
          Volume capillaryShortLog("capillaryShortLog", capillaryShort,
                                   description.material(x_capillary_S.attr<std::string>(_U(material))));
          ...
          PlacedVolume cladShort_SPlaced = capillaryShortLog.placeVolume(cladShort_SLog, 1, Position(0., 0., 0.));
          ...
          if (x_core_S_sens)
            coreShort_SLog.setSensitiveDetector(sens);
        ...
        if (capillaryLength > 5.0 * cm && !IsLastTube_C && !IsLastRow_C) {
          ...
          if (std::fabs(capillaryLength_C - length) < 0.0001 * mm) {
            PlacedVolume capillaryPlaced_C =
                towerLog.placeVolume(capillary_CLog, CalcCpNo32bits(j, k), Position(x_tube_C, y_tube_C, 0.));
            ...
            Tube capillaryShort_C(0. * mm, tubeRadius, (capillaryLength_C - TubeLengthOffset) / 2.,
                                  2 * M_PI); // reduced capillary length
                                             // by a fixed offset
            Volume capillaryShortLog_C("capillaryShortLog_C", capillaryShort_C,
                                       description.material(x_capillary_C.attr<std::string>(_U(material))));
            ...
            if (x_clad_C_sens)
              cladShort_CLog.setSensitiveDetector(sens);
            PlacedVolume cladShort_CPlaced = capillaryShortLog_C.placeVolume(cladShort_CLog, 1, Position(0., 0., 0.));
            ...
            if (x_core_C_sens)
              coreShort_CLog.setSensitiveDetector(sens);
            ...
            PlacedVolume capillaryShortPlaced_C = towerLog.placeVolume(
                ...
                Position(x_tube_C, y_tube_C, length / 2. - capillaryLength_C / 2. + TubeLengthOffset / 2.));
        ...
        if (-1. * x_backplane + delta_x - x_tube < tubeDiameter + tubeRadius)
          break;
      } // end x loop
      ...
      if ((-1. * y_backplane) - y_tube < (2. * y_pitch + tubeRadius))
        break;
