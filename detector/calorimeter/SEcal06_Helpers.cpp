...
void SEcal06_Helpers::checkLayerConsistency() {
  ...
  if (_preshower == 1 && n_total_abs_layers % 2 == 0) {
    std::cout << "SEcal06_Helpers ERROR: inconsistent ECAL model !! if you request a preshower layer, the number of "
                 "absorber layers = _nlayers1 + _nlayers2 + _nlayers3 must be odd"
              << std::endl;
    std::cout << " Ecal_PreshowerLayer = " << _preshower << " ; _nlayers1/2/3 = " << _nlayers1 << " " << _nlayers2
              << " " << _nlayers3 << std::endl;
    assert(0);
  } else if (_preshower == 0 && n_total_abs_layers % 2 == 1) {
    std::cout << "SEcal06_Helpers ERROR: inconsistent ECAL model !! if you request no preshower layer, the number of "
                 "absorber layers = _nlayers1 + _nlayers2 + _nlayers3 must be even"
              << std::endl;
    std::cout << " Ecal_PreshowerLayer = " << _preshower << " ; _nlayers1/2/3 = " << _nlayers1 << " " << _nlayers2
              << " " << _nlayers3 << std::endl;
    assert(0);
...
float SEcal06_Helpers::getTotalThickness() {
  ...
  for (unsigned int i = 0; i < _nlayers1 + _nlayers2 + _nlayers3; i++) {
    ...
    if (inStructure) {
      ...
      if (i >= _nlayers1)
        thickness = _radiator_thickness2;
      if (i >= _nlayers1 + _nlayers2)
        thickness = _radiator_thickness3;
...
double
SEcal06_Helpers::getAbsThickness(unsigned int iAbsLay) { //, int n1, int n2, int n3, double t1, double t2, double t3) {
  // get the thickness of a given absorber layer
  if (iAbsLay < _nlayers1)
    return _radiator_thickness1;
  else if (iAbsLay - _nlayers1 < _nlayers2)
    return _radiator_thickness2;
  else if (iAbsLay - _nlayers1 - _nlayers2 < _nlayers3)
    return _radiator_thickness3;
...
std::vector<SEcal06_Helpers::dimposXYStruct> SEcal06_Helpers::getAbsPlateXYDimensions(double ztop) {
  ...
  if (_module_XZtype == 1)
    assert(ztop >= 0 &&
           "getAbsPlateXYDimensions: ztop not specified"); // must specify Z position for case with non-uniform layers
...
std::vector<SEcal06_Helpers::dimposXYStruct> SEcal06_Helpers::getSlabXYDimensions(double ztop) {
  ...
  if (_module_XZtype == 1)
    assert(ztop >= 0 &&
           "getSlabXYDimensions: ztop not specified"); // must specify Z position for case with non-uniform layers

  std::vector<dimposXYStruct> layerslabdims;
  if (_module_XZtype == 0 &&
      _constantSlabXYDimensions.size() > 0) { // every layer is the same, and has already been calculated
    layerslabdims = _constantSlabXYDimensions;
  } else {
    dimposXYStruct ss;
    int totTow(0);
    for (size_t imod = 0; imod < _ntowers.size();
         imod++) { // loop over "modules" [nb, for barrel, we only make a single module; for endcaps, typically have 3
                   // per quadrant]
      for (int itow = 0; itow < _ntowers[imod]; itow++) { // towers within the module
        ...
        ss.posY = _moduleGap * (1 + 2 * imod) +           // edge-of-module gaps from Y=0 to this slab
    ...
    if (_module_XZtype == 0)
      _constantSlabXYDimensions = layerslabdims;
  }
  return layerslabdims;
}

void SEcal06_Helpers::updateCaloLayers(double thickness, dd4hep::Material mat, bool isAbsorber, bool isSensitive,
                                       double cell_size_x, double cell_size_y, bool isFinal) {

  //  cout << "updating calo layers ! absorber, sensitive, final " << isAbsorber << " " << isSensitive << " " << isFinal
  //  << endl;
  ...
  if (isAbsorber)
    _caloLayer.absorberThickness = _layer_thickness;
...
SEcal06_Helpers::dxinfo SEcal06_Helpers::getNormalMagicUnitsInX(double dx_total, double dx_unit, double dx_cell,
                                                                double dx_dead, int magicStrategy) {
  ...
  if (magicStrategy == 0) { // just integer number of standard units (wafers/megatiles)
    ...
    if (magicStrategy == 1) { // last magic megatile has integer number of standard-sized cells
      ...
      const double shortestMagicCell =
          1.0; // this means that the last magic cell is at least as long as the standard cell
      ...
      assert(magic2cellsize / dx_cell >= shortestMagicCell && magic2cellsize / dx_cell <= shortestMagicCell + 1.0 &&
             "problem in deciding magic unit size");
  ...
  if (magicStrategy == 2) {
    ...
    if (fabs(dxInf.normal_nX * dxInf.magic1_unitDX + dxInf.magic1_unitDX + dxInf.magic2_unitDX - dx_total) <
        0.01 * dd4hep::mm) {
      cout << " SEcal06_Helpers ERROR : " << endl;
      cout << dxInf.normal_nX * dxInf.magic1_unitDX << " " << dxInf.magic1_unitDX << " " << dxInf.magic2_unitDX << endl;
      cout << dxInf.normal_nX * dxInf.magic1_unitDX + dxInf.magic1_unitDX + dxInf.magic2_unitDX << " " << dx_total
           << endl;
...
SEcal06_Helpers::getSliceSegmentation(dd4hep::DDSegmentation::MultiSegmentation* multiSeg, int slice_number) {
...
void SEcal06_Helpers::makeModule(dd4hep::Volume& mod_vol,                       // the volume we'll fill
                                 ...
                                 dd4hep::Detector& theDetector, dd4hep::SensitiveDetector& sens) {
  // make the module
  _caloData = &caloData;
  ...
  dd4hep::DDSegmentation::WaferGridXY* waferSeg =
      dynamic_cast<dd4hep::DDSegmentation::WaferGridXY*>(_geomseg->segmentation());

  dd4hep::DDSegmentation::MegatileLayerGridXY* megatileSeg =
      dynamic_cast<dd4hep::DDSegmentation::MegatileLayerGridXY*>(_geomseg->segmentation());

  dd4hep::DDSegmentation::MultiSegmentation* multiSeg =
      dynamic_cast<dd4hep::DDSegmentation::MultiSegmentation*>(_geomseg->segmentation());
  ...
  if (multiSeg) {
    ...
    for (size_t k = 0; k < segs.size(); k++) {
      ...
      const dd4hep::DDSegmentation::MegatileLayerGridXY* ts =
          dynamic_cast<const dd4hep::DDSegmentation::MegatileLayerGridXY*>(entr.segmentation);
      ...
      if (mtl) {
        ...
        cout << "hmm, weird??? multiple segmentation, but not a megatile? this probably won;t work, bailing out!"
             << endl;
        assert(0);
      }
    }
  ...
  double currentLayerBase_pos_Z =
      0; // this gets updated: it's the position of the bottom face of the next detector slice
    ...
    for (int j = 0; j < x_layer.repeat(); j++) { // layers within this type (or "stack")
      ...
      if (isFrontFace) {       // the first part of the module depends on whether we have preshwer or not
        if (_preshower == 1) { // don't include W+CF wrapping; only one side of alveolus
          ...
          this_struct_CFthick_beforeAbs =
              _CF_front +
              _CF_absWrap; // allow for initial CF front plate also in no preshower case - djeans 6 july 2017
          this_struct_CFthick_afterAbs = _CF_absWrap + _CF_alvWall;
          radiator_dim_Z = getAbsThickness(absorber_index++);
          rad_pos_Z = radiator_dim_Z / 2. +
                      this_struct_CFthick_beforeAbs; // distance from top surface of structure to centre of radiator
        }
        isFrontFace = false;
        ...
        rad_pos_Z = this_struct_CFthick_beforeAbs +
                    radiator_dim_Z / 2.; // distance from top surface of structure to centre of radiator
      }

      // create the radiator volume
      if (radiator_dim_Z > 0) { // only create the volume if we have radiator
        std::vector<dimposXYStruct> absorbersheets =
            getAbsPlateXYDimensions(currentLayerBase_pos_Z + this_struct_CFthick_beforeAbs +
                                    radiator_dim_Z); // add CF before abs. added djeans 21 nov 2016
        ...
        for (size_t ipl = 0; ipl < absorbersheets.size(); ipl++) {
          ...
          dd4hep::Volume barrelStructureLayer_vol(_det_name + "_" + l_name + "_" +
                                                      dd4hep::_toString(int(ipl), "bs%02d"),
                                                  barrelStructureLayer_box, _radiator_material);

          barrelStructureLayer_vol.setVisAttributes(_radiator_visatt);

          // Position the layer.
          dd4hep::Position bsl_pos =
              getTranslatedPosition(plSize.posX, plSize.posY, currentLayerBase_pos_Z + rad_pos_Z);

          mod_vol.placeVolume(barrelStructureLayer_vol, bsl_pos);

        } // loop over sheets
        if (myLayerNum >= 0)
          myLayerNum++; // increment sensitive layer number in case we have crossed an absorber which is not before any
                        // sensitive material
      }
      ...
      for (xml_coll_t si(x_layer, _U(slice)); si; ++si) {
        ...
        if (x_slice.materialStr().compare(x_staves.materialStr()) == 0) {
          ...
          // whether to define this as sensitive for the calolayers (yes if single segmentation, but if
          // multisegmentation, not necessarily)
          bool referenceSensitiveLayer = true;

          if (myLayerNumTemp < 0)
            myLayerNumTemp = 0; //  be careful for first sens layer, which may or may not be preceeded by absorber
          ...
          if (multiSeg) {
            ...
            const dd4hep::DDSegmentation::MegatileLayerGridXY* ts =
                dynamic_cast<const dd4hep::DDSegmentation::MegatileLayerGridXY*>(thisSeg);

            assert(ts && "error: multi is not megatile?");
            megatileSeg = const_cast<dd4hep::DDSegmentation::MegatileLayerGridXY*>(ts);

            referenceSensitiveLayer =
                find(multi_refSlices.begin(), multi_refSlices.end(), slice_number) != multi_refSlices.end();
          }
          ...
          if (waferSeg) {
            ...
            if (megatileSeg->getUnifNCellsX() == 0 ||
                megatileSeg->getUnifNCellsY() == 0) { // not set via talk-to, use parameters passed to helper
              ...
              if (laytype == 0) {
                ...
                megatileSeg->setMegaTileCellsXY(myLayerNumTemp, _strips_across_megatile,
                                                _strips_along_megatile); // strips in one orientation
              } else if (laytype == 2) {
                megatileSeg->setMegaTileCellsXY(myLayerNumTemp, _strips_along_megatile,
                                                _strips_across_megatile); // and in the other
              } else {
          ...
          updateCaloLayers(s_thick, slice_material, false, referenceSensitiveLayer, cell_size_x,
                           cell_size_y); // sensitive
      ...
      for (size_t islab = 0; islab < slabDims.size(); islab++) {
        ...
        dd4hep::Box l_box(slab_dim_X / 2., slabDims[islab].sizeY / 2. - _CF_alvWall, slab_dim_Z / 2.);
        ...
        for (xml_coll_t si(x_layer, _U(slice)); si; ++si) { // the sub-layers
          ...
          if (!x_slice.isSensitive()) { // not the sensitive slice: just a layer of stuff
            ...
            double cell_size_x =
                waferSeg ? waferSeg->cellDimensions(0)[0] : megatileSeg->cellDimensions(myLayerNumTemp, 0)[0];
            double cell_size_y =
                waferSeg ? waferSeg->cellDimensions(0)[1] : megatileSeg->cellDimensions(myLayerNumTemp, 0)[1];

            // work out how to make the magic units, if requested
            dxinfo xseg =
                getNormalMagicUnitsInX(slab_dim_X, unit_dim_Y, cell_size_x, _unitDeadEdge, _magicMegatileStrategy);

            int n_wafers_x = xseg.normal_nX;
            int ncellsy =
                int(unit_sensitive_dim_Y / cell_size_y +
                    0.5); // should be exactly integer. take nearest integer to account for possible rounding errors.

            int wafer_num = 0;

            double wafer_pos_X = -slab_dim_X / 2.; // keep track of wafer position in X

            for (int n_wafer_x = 0; n_wafer_x < n_wafers_x + 2;
                 n_wafer_x++) { // loop along slab, including magic megatile/wafer
              ...
              if (megatile_size_x <= 0)
                continue;

              double megatile_sensitive_size_x = megatile_size_x - 2 * _unitDeadEdge;

              for (int n_wafer_Y = 0; n_wafer_Y < _unitsPerTower;
                   n_wafer_Y++) { // loop across slab (usually 2 wafers, or 1 EBU) [ie along beam dir for barrel module]
                ...
                dd4hep::Box* box =
                    isMagic ? new dd4hep::Box(megatile_sensitive_size_x / 2, unit_sensitive_dim_Y / 2, s_thick / 2.)
                            : &WaferSiSolid;
                dd4hep::Volume WaferSiLog(_det_name + "_" + l_name + "_" + s_name + "_" + Wafer_name, *box,
                                          slice_material);
                ...
                if (multiSeg)
                  wafer_phv.addPhysVolID("slice", s_num); // need to keep slice id in case of multireadout

                if (isMagic) {
                  if (megatileSeg) { // define the special megatile
                    megatileSeg->setSpecialMegaTile(myLayerNumTemp, wafer_num, megatile_sensitive_size_x,
                                                    unit_sensitive_dim_Y, -megatile_size_x / 2.,
                                                    -unit_sensitive_dim_Y / 2., // the offset
                                                    ncellsx, ncellsy);          // the segmentation
                  }
                } else {          // not magic
                  if (waferSeg) { // Normal squared wafers, this waferOffsetX is 0.0 // if its an odd number of cells,
