//====================================================================
//  ATLAS-inspired EM LAr Calorimeter for the Future Circular Collider
//
//  Author     : A. Zaborowska
//               Adapted from FCCSW/Detector/DetFCChhECalInclined by V. Volkl
//
//
//  WARNING: This driver is an adaptation of upstream sources in FCCSW
//  (github.com/HEP-FCC/FCCSW) check the original source for the latest version.
//
//====================================================================
...
static dd4hep::detail::Ref_t createECalBarrelInclined(dd4hep::Detector& aLcdd,
  ...
  if (activePassiveOverlap < 0 || activePassiveOverlap > 0.5) {
    std::cout  << "Overlap between active and passive cannot be more than half of passive plane!" << std::endl;
  }
  ...
  std::cout << "Number of layers: " << numLayers << " total thickness " << layersTotalHeight << std::endl;
  ...
  if (cryoThicknessFront > 0) {
    ...
    std::cout  << "ECAL cryostat: front: rmin (cm) = " << cryoDim.rmin1() << " rmax (cm) = " << cryoDim.rmin2() << " dz (cm) = " << cryoDim.dz()  << std::endl;
    std::cout  << "ECAL cryostat: back: rmin (cm) = " << cryoDim.rmax1() << " rmax (cm) = " << cryoDim.rmax2() << " dz (cm) = " << cryoDim.dz() << std::endl;
    std::cout  << "ECAL cryostat: side: rmin (cm) = " << cryoDim.rmin2() << " rmax (cm) = " << cryoDim.rmax1() << " dz (cm) = " << cryoDim.dz() - caloDim.dz()  << std::endl;
    ...
    if (cryoFrontSensitive) {
      ...
      std::cout << "Cryostat front volume set as sensitive" << std::endl;
    }
    if (cryoBackSensitive) {
      ...
      std::cout << "Cryostat back volume set as sensitive" << std::endl;
    }
    ...
    std::cout << "ECAL services: front: rmin (cm) = " << cryoDim.rmin2() << " rmax (cm) = " << bathRmin << " dz (cm) = " << caloDim.dz() << std::endl;
    std::cout << "ECAL services: back: rmin (cm) = " << bathRmax << " rmax (cm) = " <<
    cryoDim.rmax1() << " dz (cm) = " << caloDim.dz() << std::endl;
    ...
    if (cryoFrontSensitive) {
      ...
      std::cout << "Services front volume set as sensitive" << std::endl;
    }
    if (cryoBackSensitive) {
      ...
      std::cout << "Services back volume set as sensitive" << std::endl;
    }
  ...
  std::cout << "ECAL bath: material = " << activeMaterial << " rmin (cm) =  " << bathRmin
       << " rmax (cm) = " << bathRmax << " thickness in front of ECal (cm) = " << caloDim.rmin() - cryoDim.rmin2()
       << " thickness behind ECal (cm) = " << cryoDim.rmax1() - caloDim.rmax() << std::endl;
  ...
  std::cout << "ECAL calorimeter volume rmin (cm) =  " << caloDim.rmin() << " rmax (cm) = " << caloDim.rmax()
       << std::endl;
  ...
  // PASSIVE PLANES
  //////////////////////////////
  std::cout << "passive inner material = " << passiveInnerMaterial
       ...
       << " thickness of total (cm) =  " << passiveThickness << " rotation angle = " << angle <<
       std::endl;
  uint numPlanes =
      M_PI / asin((passiveThickness + activeThickness + readoutThickness) / (2. * caloDim.rmin() * cos(angle)));
  double dPhi = 2. * M_PI / numPlanes;
  std::cout << "number of passive plates = " << numPlanes << " azim. angle difference =  " << dPhi
  << std::endl;
  std::cout << " distance at inner radius (cm) = " << 2. * M_PI * caloDim.rmin() / numPlanes
       << " distance at outer radius (cm) = " << 2. * M_PI * caloDim.rmax() / numPlanes << std::endl;
  ...
  std::cout << "readout material = " << readoutMaterial
  ...
  std::cout << "thickness of calorimeter (cm) = " << dR
       << " length of passive or readout planes (cm) =  " << planeLength << std::endl;
  ...
  for (uint iLay = 0; iLay < numLayers; iLay++) {
    ...
    std::cout << "Thickness of layer " << iLay << " : " << layerHeight[iLay] << std::endl;
  }
  ...
  if (passiveInner.isSensitive()) {
    std::cout << "Passive inner volume set as sensitive" << std::endl;
  ...
  if (passiveOuter.isSensitive()) {
    std::cout << "Passive outer volume set as sensitive" << std::endl;
  ...
  if (passiveGlue.isSensitive()) {
    std::cout << "Passive glue volume set as sensitive" << std::endl;
    double layerOffset = layerFirstOffset;
    for (uint iLayer = 0; iLayer < numLayers; iLayer++) {
  ...
  if (readout.isSensitive()) {
    std::cout << "Readout volume set as sensitive" << std::endl;
    double layerOffset = layerFirstOffset;
    for (uint iLayer = 0; iLayer < numLayers; iLayer++) {
  ...
  std::cout << "active material = " << activeMaterial
  ...
  std::cout
       ...
       << " = " << activePassiveOverlap * 100 << " %" << std::endl;
