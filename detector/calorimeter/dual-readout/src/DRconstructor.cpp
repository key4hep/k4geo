...
ddDRcalo::DRconstructor::DRconstructor(xml_det_t& x_det)
    : fX_det(x_det),
      ...
      fX_worldTube(fX_struct.child(_Unicode(worldTube))), fX_barrelTube(fX_struct.child(_Unicode(barrelTube))) {
...
void ddDRcalo::DRconstructor::construct() {
  ...
  // Tube to cover all +eta DRC geometry, Rmin = 252 - 1 mm (Rmin in endcap region), Rmax = 4500 + 1 mm, length = 4500 +
  // 1mm
  ...
  dd4hep::SubtractionSolid AssemblyTube(WorldTube, BarrelTube,
                                        dd4hep::Position(0, 0, -(fX_worldTube.height() - fX_barrelTube.height()) / 2.));
  ...
  dd4hep::PlacedVolume PlacedAssemblyTubeVol =
      fExperimentalHall->placeVolume(AssemblyTubeVol, dd4hep::Position(0, 0, fX_worldTube.height() / 2.));
...
void ddDRcalo::DRconstructor::placeAssembly(dd4hep::DDSegmentation::DRparamBase_k4geo* param,
                                            dd4hep::Volume& AssemblyBoxVol, dd4hep::Volume& towerVol,
  ...
  dd4hep::PlacedVolume sipmWaferPhys =
      AssemblyBoxVol.placeVolume(sipmWaferVol, towerId32, dd4hep::Transform3D(param->GetRotationZYX(nPhi), sipmPos));
...
void ddDRcalo::DRconstructor::implementFibers(xml_comp_t& x_theta, dd4hep::Volume& towerVol, dd4hep::Trap& trap,
                                              dd4hep::DDSegmentation::DRparamBase_k4geo* param) {
  ...
  int numxBl2 = static_cast<int>(std::floor((param->GetBl2() * 2. - sipmSize / 2.) / gridSize)) +
