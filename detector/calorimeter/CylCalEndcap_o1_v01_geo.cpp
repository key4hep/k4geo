...
//
// Original Author: M.Frank
// Customised for SiD by: D.Protopopescu - Nov 2017
//==========================================================================
//
...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens)  {
  ...
  double     z         = zmin;
  double     env_safe  = 0.1;
  Tube       envelope   (rmin, rmax, totWidth/2+4*env_safe);
  ...
  phv = assembly.placeVolume(envelopeVol,Position(0,0,zmin+totWidth/2+4*env_safe));
  phv.addPhysVolID("side",1);
  sdetA.setPlacement(phv);

  if ( reflect )   {
    phv=assembly.placeVolume(envelopeVol,Transform3D(RotationZ(M_PI),Position(0,0,-zmin-totWidth/2-4*env_safe)));
    phv.addPhysVolID("side",-1);
...
DECLARE_DETELEMENT(CylCalEndcap_o1_v01,create_detector)
