...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  PolyhedraRegular YokeBarrelSolid(symmetry, M_PI / 2.0 - M_PI / symmetry, rInnerBarrel, rOuterBarrel,
  ...
  for (xml_coll_t li(x_det, _U(layer)); li; ++li) {
    ...
    for (int i = 0; i < repeat; i++) {
      ...
      if (i >= 10)
        radius_low = rInnerBarrel + 0.05 + i * gap_thickness + (i + (i - 10) * 4.6) * iron_thickness;
      //{ radius_low =
