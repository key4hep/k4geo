#ifndef __SECAL06_HELPERS_H__
#define __SECAL06_HELPERS_H__ 1
...
class SEcal06_Helpers {

public:
  SEcal06_Helpers();

  ~SEcal06_Helpers() { delete _layering; }
  ...
  void setLayerConfig(std::vector<int> layerConfig) { _layerConfig = layerConfig; }

  void setSegmentation(dd4hep::Segmentation& seg) { _geomseg = &seg; }
  ...
  void setPreshower(bool p) {
    _preshower =
        p ? 1 : 0; // do we have preshower layer? (if 1, first layer is sensitive; if 0, first layer is absorber)
  }
  ...
  void setModuleDimensions(int XYtype,              // module shape in XY
    ...
    if (XYtype > 0)
      assert(XZtype == 0); // endcap module has vertical edges
    if (XZtype > 0)
      assert(XYtype == 0); // barrel module should have rectangular shadow
...
private:
  ...
  dd4hep::Position getTranslatedPosition(double x, double y, double z) { return dd4hep::Position(x, y, z) + _trans; }

  dxinfo getNormalMagicUnitsInX(double dx_total, double dx_unit, double dx_cell, double dx_dead, int magicStrategy);

  void updateCaloLayers(double thickness, dd4hep::Material mat, bool isAbsorber, bool isSensitive,
                        double cell_size_x = 0, double cell_size_y = 0, bool isFinal = false);

  const dd4hep::DDSegmentation::Segmentation* getSliceSegmentation(dd4hep::DDSegmentation::MultiSegmentation* multiSeg,
                                                                   int slice_number);
