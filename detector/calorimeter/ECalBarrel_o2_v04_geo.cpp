    ...
    for (int m = 0; m < n_stacks; m++) {
      ...
      for (xml_coll_t li(x_det, _U(layer)); li; ++li) {
        ...
        for (int j = 0; j < repeat; j++) {
...
#if VERBOSE_LEVEL > 1
          std::cout << l_name << " thickness: " << l_thickness << " l_dim_x=" << l_dim_xj << " shift=" << x_shift
                    << std::endl;
#endif
          std::cout << "Barrel ECal layer_" << l_num << "-" << repeat << "_" << j << " l_dim_x=" << l_dim_xj
                    << " shift=" << x_shift << std::endl;
          ...
          std::cout << " caloLayer total thickness: " << caloLayer.inner_thickness + caloLayer.outer_thickness
                    << std::endl;
#endif
