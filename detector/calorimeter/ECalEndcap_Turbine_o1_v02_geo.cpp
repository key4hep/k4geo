...
namespace ECalEndcap_Turbine_o1_v02 {
  ...
  void buildWheel(dd4hep::Detector& aLcdd, dd4hep::SensitiveDetector& aSensDet, dd4hep::Volume& aEnvelope,
    ...
    if (TMath::Abs(TMath::Tan(BladeAngle)) < delZ / (2. * ri)) {
      dd4hep::printout(dd4hep::ERROR, "ECalEndcap_Turbine_o1_v02",
    ...
    if (AbsThickMin < 0.) {
      dd4hep::printout(
          dd4hep::ERROR, "ECalEndcap_Turbine_o1_v02",
    ...
    dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02", "LArGap at inner radius is %f", LArgapi);
    ...
    for (unsigned iLayer = 0; iLayer < numNonActiveLayers; iLayer++) {
      ...
      dd4hep::printout(dd4hep::INFO, "ECalEndcap_Turbine_o1_v02", "Making layer with inner, outer radii %f, %f",
                       riLayer, roLayer);

      AbsThicko = AbsThicki + BladeThicknessScaleFactor * ((roLayer / riLayer) - 1.) * AbsThicki;

      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02", "Inner and outer absorber thicknesses %f, %f ",
                       AbsThicki, AbsThicko);
    ...
    for (unsigned iLayer = 0; iLayer < ECalEndcapNumCalibLayers; iLayer++) {
      ...
      dd4hep::printout(dd4hep::INFO, "ECalEndcap_Turbine_o1_v02",
                       "Inner and outer thicknesses of noble liquid volume %f, %f", ElectrodeThick + LArgapi * 2,
                       ElectrodeThick + LArgapo * 2);

      dd4hep::Solid electrodeBladeAndGapLayer = buildOneBlade(
          ElectrodeThick + LArgapi * 2, ElectrodeThick + LArgapo * 2, xRange, roLayer, riLayer, BladeAngle, delZ);
    ...
    for (auto absBladeLayerVol : absBladeLayerVols) {
      ...
      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02_geo", "Blade layer, rho is %d, %f, %f", iLayer,
                       absBladeVol_pv.position().Rho(), roLayer / 2.);
      absBladeVol_pv.addPhysVolID("layer", iWheel * numNonActiveLayers + iLayer);

      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02_geo", "AbsBalde volume %s",
    ...
    for (auto claddingLayerVol : claddingLayerVols) {
      ...
      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02_geo", "Cladding volume %s",
    ...
    for (auto electrodeBladeLayerVol : electrodeBladeLayerVols) {
      ...
      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02_geo", "Electrode volume %s",
    ...
    for (auto LArTotalLayerVol : LArTotalLayerVols) {
      ...
      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02", "LAr layer: %d layer in readout: %d", iLayer,
                       iWheel * ECalEndcapNumCalibLayers + iLayer);
    ...
    for (int iUnitCell = 0; iUnitCell < nUnitCellsToDraw; iUnitCell++) {
      ...
      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02", "Placing blade, ro, ri = %f %f", ro, ri);
      ...
      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02_geo", "Passive volume %s",
      ...
      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02_geo", "Active volume %s",
      ...
      dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02", "LArTotalLayerVols.size = %d",
  ...
  void buildOneSide_Turbine(dd4hep::Detector& aLcdd, dd4hep::DetElement& caloDetElem,
    ...
    if (cryoThicknessFront > 0) {
      ...
      dd4hep::printout(dd4hep::INFO, "ECalEndcap_Turbine_o1_v02", "Cryostat is made out of %s",
    ...
    for (unsigned iWheel = 0; iWheel < nWheels; iWheel++) {
      ...
      buildWheel(aLcdd, aSensDet, bathVol, aXmlElement, bathDetElem, ri + supportTubeThickness, ro,
                 bathDelZ * 2 - bathThicknessFront - bathThicknessBack, iWheel);
      ri = ro;
      ro *= radiusRatio;
      if (ro > rmax)
        ro = rmax;
  ...
  static dd4hep::Ref_t createECalEndcapTurbine(dd4hep::Detector& aLcdd, dd4hep::xml::Handle_t aXmlElement,
    ...
    dd4hep::printout(dd4hep::DEBUG, "ECalEndcap_Turbine_o1_v02",
