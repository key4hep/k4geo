...
namespace det {
  ...
  namespace ECalEndcap_Turbine_o1_v02 {
	...
	return;
      ...
      if (AbsThickMin < 0.) {
	dd4hep::printout(dd4hep::ERROR, "ECalEndcap_Turbine_o1_v02",  "Error: requested absorber thickness is negative after accounting for glue and cladding thickness");
      }
...
>>>>>>> c85fb7c2 (Fix bugs in LAr bath placement and in placement of passive elements when split into layers)
    ...
    void buildOneSide_Turbine(dd4hep::Detector& aLcdd, dd4hep::DetElement& caloDetElem, dd4hep::SensitiveDetector& aSensDet,
      ...
      // double cryoThicknessOuter = aLcdd.constant<float>("CryoEMECThicknessOuter");
	...
	dd4hep::printout( dd4hep::INFO, "ECalEndcap_Turbine_o1_v02",  "Cryostat is made out of %s", cryostat.materialStr().c_str() );
