...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  cout << "------------------------" << endl;
  cout << "creating SEcal06_Endcaps" << endl;
  cout << "------------------------" << endl;
  ...
  double EcalBarrel_inner_radius =
  ...
  cout << "absorber layers " << Ecal_nlayers1 << "*" << Ecal_radiator_thickness1 << "mm + " << Ecal_nlayers2 << "*"
       << Ecal_radiator_thickness2 << "mm + " << Ecal_nlayers3 << "*" << Ecal_radiator_thickness3 << "mm " << endl;

  // check this setup is self-consistent
  helper.checkLayerConsistency();

  // set structural CF thicknesses
  helper.setCFthickness(Ecal_fiber_thickness_structure, Ecal_fiber_thickness_alveolus, Ecal_front_face_thickness,
  ...
  double barrel_alv_width = (Ecal_Barrel_halfZ * 2. / Ecal_barrel_z_modules - 2. * Ecal_lateral_face_thickness) /
  ...
  if (fabs(calc_endcap_rout - EcalEndcap_outer_radius) > 0.1 * dd4hep::mm) {
    ...
    cout << "consider changing Ecal_endcap_extra_size from " << Ecal_endcap_extra_size << " to "
         << calc_endcap_rout - EcalBarrel_inner_radius - Ecal_barrel_thickness << endl;
    assert(0);
  }

  helper.setTowersUnits(ntowers, barrel_alv_width, Ecal_n_wafers_per_tower, Ecal_lateral_face_thickness,
  ...
  IntersectionSolid EndCapSolid(ECPolyHedra, quadrant,
                                Position(quadr - EcalEndcap_inner_radius, quadr + EcalEndcap_inner_radius, 0));
  ...
  helper.makeModule(EnvLogEndCap, mod_det, *caloData, theDetector, sens);
  ...
  for (int iend = 0; iend < 2; iend++) { // the 2 endcaps
    ...
    double rotZ_offset =
        iend == 0 ? M_PI / 8. - M_PI / 2. : M_PI / 8. + 3. * M_PI / 4.; // magic rotation to get modules in right place
    for (int iquad = 0; iquad < 4; iquad++) {                           // the 4 quadrants per endcap
      ...
      DetElement sd = (iend == 0 && iquad == 0)
                          ? mod_det
                          : mod_det.clone(_toString(module_id, "module%d") + _toString(stave_id, "stave%d"));
      sd.setPlacement(pv);
    }
  }
  ...
  //  cout << "finished SEcal06_Endcaps" << endl;

  return sdet;
}

DECLARE_DETELEMENT(SEcal06_Endcaps, create_detector)
