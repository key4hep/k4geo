...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  std::cout << "Hcal_barrel_thickness:" << Hcal_barrel_thickness << " Hcal_requested_thickness: " << Hcal_total_dim_y
            << endl;
  std::cout << "Hcal_regular_chamber_dim_z, Hcal_normal_dim_z:  " << Hcal_regular_chamber_dim_z << " "
            << Hcal_normal_dim_z << endl;
  ...
  std::cout << "Hcal_barrel_trapezoid_height: " << Hcal_trap_height_dim_y
            << " # of layers fitting into it: " << Nlayers_trap_height << endl;
  std::cout << "Hcal_barrel remainning space: " << Hcal_remaining_space
            << " # of layers fitting into it: " << Nlayers_remaining << endl;
  ...
  cout << " \n radiator_thickness, airgap_thickness  " << Hcal_radiator_thickness << " " << Hcal_airgap_thickness
       << endl;
  cout << " \n AngleRatio, d_InnerOctoSize, trap top size  " << AngleRatio << " " << d_InnerOctoSize << " "
       << Hcal_trap_top_dim_x << endl;
  ...
  for (int layer_id = 1; layer_id <= 2 * Hcal_nlayers; layer_id++) {
    ...
    if (logical_layer_id * layer_thickness < Hcal_trap_height_dim_y) {
      ...
      cout << "Bottom : layer_id, ncells, x_length, z_width, y_height, x_offset :" << layer_id << " "
           << floor(x_length / cell_sizeX) << " " << x_length << " " << z_width << " " << y_height << " " << xOffset
           << endl;
      ...
      ***/
      cout << "Top barrel: layer_id, ncells, x_length, z_width, y_height, xOffset :" << layer_id << " "
           << floor(x_length / cell_sizeX) << " " << x_length << " " << z_width << " " << y_height << " " << xOffset
           << endl;
    }
    ...
    for (xml_coll_t k(x_layer, _U(slice)); k; ++k) {
      ...
      if (logical_layer_id == 1)
        cout << "  Layer_slice: " << slice_name << " slice_thickness: " << slice_thickness
             << " slice_pos_z :" << slice_pos_z << endl;
      // Slice volume & box
    ...
    chamber_y_offset = -(-Hcal_total_dim_y / 2. + (logical_layer_id - 1) * layer_thickness + layer_thickness / 2.);

    if (logical_layer_id == 1)
      cout << "logical_layer_id, chamber_ x_offset, z_offset, y_offset :" << logical_layer_id << " " << chamber_x_offset
           << " " << chamber_z_offset << " " << chamber_y_offset << endl;
    ...
    for (int module_id = 1; module_id <= Hcal_barrel_number_modules; module_id++) {
      ...
      if (stave_id == 1) {
        cout << "\n module_id:    " << module_id << "   module_z_offset:    " << module_z_offset
             << " Hcal_normal_dim_z: " << Hcal_normal_dim_z
             << " Hcal_module_wall_thickness: " << Hcal_module_wall_thickness << endl;
      }
      ...
      lateral_plate_z_offset =
          -Hcal_half_length + Hcal_module_wall_thickness / 2. + (module_id - 1) * Hcal_normal_dim_z;
      ...
      if (stave_id == 1) { // place only once for the whole lateral_plate
        ...
        //            cout <<" lateral_plate_z_offset LP2,  module_id, stave_id:   "<< lateral_plate_z_offset <<" , "<<
        //            module_id<<" , "<<stave_id <<endl;
      }
    }
