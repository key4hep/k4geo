...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  for (xml_coll_t c(x_det.child(_U(dimensions)), _U(dimensions)); c; ++c) {
    ...
    pVol = FEELayer.placeVolume(FEELayerSteel,
                                Position(0, 0, (-FEELayer_thickness / 2.0 + Hcal_steel_cassette_thickness / 2.0)));
    ...
    pVol = FEELayer.placeVolume(FEELayerFR4, Position(0, 0,
    ...
    for (xml_coll_t m(x_det, _U(layer)); m; ++m) {
      ...
      for (xml_coll_t k(x_layer, _U(slice)); k; ++k) {
        ...
        // Slice volume & box
        Volume slice_vol(slice_name, Box(active_layer_dim_x, active_layer_dim_y, slice_thickness / 2.0),
                         slice_material);
      ...
      for (int j = 0; j < repeat; j++) {
        ...
        pVol = FEEModule.placeVolume(FEELayer, Position(0, 0, layer_pos_z));
        //-----------------------------------------------------------------------------------------
        if (caloData->layers.size() < (unsigned int)repeat) {

          caloLayer.distance =
              HcalEndcap_min_z + box_half_z + layer_pos_z -
