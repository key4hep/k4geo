...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  printout(dd4hep::DEBUG, "SHcalSc04_Barrel_v03", "TPC_outer_radius : %e   - Ecal_outer_radius: %e ", TPC_outer_radius,
           Ecal_outer_radius);
  ...
  double Hcal_total_dim_y =
      Hcal_nlayers * (Hcal_radiator_thickness + Hcal_chamber_thickness) + Hcal_back_plate_thickness;
  ...
  Volume EnvLogHcalModule1Barrel(det_name + "_module1", barrelModule1Solid, stavesMaterial);

  IntersectionSolid barrelModule2Solid(m2stave_BMT, solidCaloTube, mtran3D);
  Volume EnvLogHcalModule2Barrel(det_name + "_module2", barrelModule2Solid, stavesMaterial);
  ...
  Volume EnvLogHcalModuleBarrelTop_LP(det_name + "_Module_lateral_plate_top", Module_lateral_plate_top, stavesMaterial);

  IntersectionSolid Module_lateral_plate_middle(lateral_plate_middle, solidCaloTube_LP, Tube_LP_rot);
  Volume EnvLogHcalModuleBarrelMiddle_LP(det_name + "_Module_lateral_plate_middle", Module_lateral_plate_middle,
                                         stavesMaterial);

  IntersectionSolid Module_lateral_plate_bottom(lateral_plate_bottom, solidCaloTube_LP, Tube_LP_rot);
  Volume EnvLogHcalModuleBarrelBottom_LP(det_name + "_Module_lateral_plate_bottom", Module_lateral_plate_bottom,
                                         stavesMaterial);
  ...
  for (int layer_id = 1; layer_id <= (2 * Hcal_nlayers); layer_id++) {
    ...
    if (logical_layer_id * (Hcal_radiator_thickness + Hcal_chamber_thickness) <
      ...
      xOffset =
          (logical_layer_id * Hcal_radiator_thickness + (logical_layer_id - 1) * Hcal_chamber_thickness) * TanPiDiv8;
      ...
      xOffset = (logical_layer_id * Hcal_radiator_thickness + (logical_layer_id - 1) * Hcal_chamber_thickness -
                 Hcal_y_dim1_for_x) /
                    TanPiDiv8 +
    ...
    printout(dd4hep::DEBUG, "SHcalSc04_Barrel_v03", " m1 chamber x:y:z:  %e:%e:%e", x_length * 2., m1_z_width * 2.,
             y_height * 2.);
    printout(dd4hep::DEBUG, "SHcalSc04_Barrel_v03", " m2 chamber x:y:z:  %e:%e:%e", x_length * 2., m2_z_width * 2.,
             y_height * 2.);
    ...
    if (layer_id <= Hcal_nlayers) {
      ...
      if (fracPart == 0) { // divisible
        if (noOfIntCells % 2) {
          if (tileSeg != 0)
            tileSeg->setLayerOffsetX(0);
        } else {
          if (tileSeg != 0)
            tileSeg->setLayerOffsetX(1);
        }
      ...
      if ((int)((m1_z_width * 2.) / cell_sizeX) % 2) {
        if (tileSeg != 0)
          tileSeg->setLayerOffsetY(0);
      } else {
        if (tileSeg != 0)
          tileSeg->setLayerOffsetY(1);
      }
    }

    // x + air gaps at two side, do not need to build air gaps individualy.
    Box m1ChamberSolid((x_length + Hcal_layer_air_gap), m1_z_width, y_height);
    ...
    Box m2ChamberSolid((x_length + Hcal_layer_air_gap), m2_z_width, y_height);
    ...
    for (xml_coll_t k(x_layer, _U(slice)); k; ++k) {
      ...
      if (x_slice.isSensitive()) {
        ...
        printout(dd4hep::DEBUG, "SHcalSc04_Barrel_v03", "  logical_layer_id:  %d  slice_id:  %d", logical_layer_id,
                 slice_id);
      }
    ...
    chamber_y_offset =
        -(-Hcal_total_dim_y / 2. + (logical_layer_id - 1) * (Hcal_chamber_thickness + Hcal_radiator_thickness) +
          Hcal_radiator_thickness + Hcal_chamber_thickness / 2.);

    pv = EnvLogHcalModule1Barrel.placeVolume(m1ChamberLogical,
                                             Position(chamber_x_offset, chamber_z_offset + chambers_z_off_correction,
  ...
  for (int m_id = 1; m_id <= 2; m_id++) {
    ...
    pv = envelope.placeVolume(EnvLogHcalModuleBarrelTop_LP, Position(0, 0, lateral_plate_z_offset));

    lateral_plate_z_offset = lateral_plate_z_offset + Hcal_stair_dim_z;
    pv = envelope.placeVolume(EnvLogHcalModuleBarrelMiddle_LP, Position(0, 0, lateral_plate_z_offset));

    lateral_plate_z_offset = lateral_plate_z_offset + Hcal_stair_dim_z;
    pv = envelope.placeVolume(EnvLogHcalModuleBarrelBottom_LP, Position(0, 0, lateral_plate_z_offset));
