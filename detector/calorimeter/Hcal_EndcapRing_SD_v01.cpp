...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  Position IntersectXYZtrans((pRMax + (Hcal_stave_gaps / 2.)), (pRMax + (Hcal_stave_gaps / 2.)), (zlen / 2.));
  ...
  for (int stave_id = 0; stave_id < 4; stave_id++) {

    for (int layer_id = 1; layer_id <= number_of_chambers; layer_id++) {
      // Insert layers from zmin -> zmax  BUT! be sure to end the detector with  "last_layer + back_plate"  ... and no
      // free space
      ...
      for (xml_coll_t k(x_layer, _U(slice)); k; ++k) {
        ...
        if (stave_id == 0 && layer_id == 1)
          cout << "Layer_slice: " << slice_name << " slice_thickness: " << slice_thickness
               << " slice_position_z: " << slice_pos_z << endl;
        ...
        // set up the translation and rotation for the intersection process
        // this happens in the mother volume coordinate system, so a coordinate transformation is needed
        Position sIntersectXYZtrans((pRMax + (Hcal_stave_gaps / 2.)), (pRMax + (Hcal_stave_gaps / 2.)), (zlen / 2.));
        //					(Hcal_total_dim_z/2.));
