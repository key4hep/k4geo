...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  Trapezoid trd(top_dim_x / 2, bottom_dim_x / 2, Ecal_Barrel_module_dim_z / 2, Ecal_Barrel_module_dim_z / 2,
                module_thickness / 2);
  ...
  nRadiationLengths = Ecal_radiator_thickness1 / (stave_material.radLength()) + y_floor / air.radLength();
  nInteractionLengths = Ecal_radiator_thickness1 / (stave_material.intLength()) + y_floor / air.intLength();
  ...
  for (xml_coll_t li(x_det, _U(layer)); li; ++li) {
    ...
    for (int j = 0; j < repeat; j++) {
      ...
      double l_pos_y =
          Ecal_Barrel_module_dim_z / 2. - (Ecal_lateral_face_thickness + Ecal_fiber_thickness * N_FIBERS_ALVOULUS +
                                           Ecal_Slab_shielding + Ecal_Slab_H_fiber_thickness + alveolus_dim_z / 2.);
      for (int i = 0; i < Ecal_barrel_number_of_towers; i++) { // need four clone
        ...
        l_pos_y -= (alveolus_dim_z + 2. * Ecal_fiber_thickness * N_FIBERS_ALVOULUS + 2. * Ecal_Slab_H_fiber_thickness +
                    2. * Ecal_Slab_shielding);
      }
      ...
      for (xml_coll_t si(x_layer, _U(slice)); si; ++si) {
        ...
        if (x_slice.isSensitive()) {
          ...
          double real_wafer_size_x = wafer_dim_x + 2 * Ecal_guard_ring_size;

          int n_wafers_x = int(floor(slab_dim_x * 2 / real_wafer_size_x));

          double wafer_pos_x = -slab_dim_x + Ecal_guard_ring_size + wafer_dim_x / 2;
          int n_wafer_x;
          int wafer_num = 0;
          ...
          // for that.
          double resting_dim_x = slab_dim_x * 2 - (wafer_dim_x + 2 * Ecal_guard_ring_size) * n_wafers_x;

          if (resting_dim_x > (cell_dim_x + 2 * Ecal_guard_ring_size)) {
            int N_cells_x_remaining = int(floor((resting_dim_x - 2 * Ecal_guard_ring_size) / cell_dim_x));
            ...
            wafer_pos_x = -slab_dim_x + n_wafers_x * real_wafer_size_x + (wafer_dim_x + 2 * Ecal_guard_ring_size) / 2;
            ...
            for (int n_wafer_z = 1; n_wafer_z < 3; n_wafer_z++) {
              ...
              Volume MagicWaferSiLog(det_name + "_" + l_name + "_" + s_name + "_" + MagicWafer_name, MagicWaferSiSolid,
                                     slice_material);
              MagicWaferSiLog.setSensitiveDetector(sens);
              // MagicWaferSiLog.setVisAttributes(theDetector.visAttributes(x_slice.visStr()));
              PlacedVolume wafer_phv = s_vol.placeVolume(MagicWaferSiLog, Position(wafer_pos_x, wafer_pos_z, 0));
              wafer_phv.addPhysVolID("wafer", wafer_num);
...
#endif
          //-----------------------------------------------------------------------------------------
          caloLayer.distance = Ecal_inner_radius + module_thickness / 2.0 - l_pos_z + l_thickness / 2. +
                               (s_pos_z + s_thick / 2.) - caloLayer.inner_thickness;
      ...
      double radiator_dim_z =
          Ecal_Barrel_module_dim_z - 2 * Ecal_lateral_face_thickness - 2 * N_FIBERS_W_STRUCTURE * Ecal_fiber_thickness;

      string bs_name = "bs";
      Box barrelStructureLayer_box(radiator_dim_x / 2., radiator_dim_z / 2., radiator_dim_y / 2.);
      Volume barrelStructureLayer_vol(det_name + "_" + l_name + "_" + bs_name, barrelStructureLayer_box,
                                      stave_material);
      ...
      double bsl_pos_z =
          l_pos_z - (radiator_dim_y / 2. + Ecal_fiber_thickness * (N_FIBERS_ALVOULUS + N_FIBERS_W_STRUCTURE));
  ...
  for (int stave_id = 1; stave_id <= nsides; stave_id++)
    for (int module_id = 1; module_id < 6; module_id++) {
      ...
      Transform3D tr(
          RotationZYX(0, phirot, M_PI * 0.5),
          Translation3D(X * cos(phirot) - Y * sin(phirot), X * sin(phirot) + Y * cos(phirot), module_z_offset));
      PlacedVolume pv = envelope.placeVolume(mod_vol, tr);
      pv.addPhysVolID("module", module_id);
      pv.addPhysVolID("stave", stave_id);
      DetElement sd = (module_id == 0 && stave_id == 0)
                          ? stave_det
                          : stave_det.clone(_toString(module_id, "module%d") + _toString(stave_id, "stave%d"));
      sd.setPlacement(pv);
      sdet.add(sd);
    }
