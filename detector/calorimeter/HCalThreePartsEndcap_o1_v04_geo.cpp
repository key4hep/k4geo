...
static dd4hep::Ref_t createHCalEC(dd4hep::Detector& lcdd, xml_h xmlElement, dd4hep::SensitiveDetector sensDet) {
  ...
  // Sensitive barrel (stacks of layers) min radii
  double sensitiveBarrel1Rmin = dimensions.rmin1() + dRhoFacePlate;
  double sensitiveBarrel2Rmin = dimensions.rmin2() + dRhoFacePlate;
  double sensitiveBarrel3Rmin = dimensions.rmin() + dRhoFacePlate;
  ...
  if (!sequences[0] || !sequences[1]) {
    dd4hep::printout(dd4hep::ERROR, "HCalThreePartsEndcap_o1_v04",
  ...
  dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04", "sequence thickness %.2f", dzSequence);
  ...
  dd4hep::printout(
      dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04",
  ...
  dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04",
  ...
  dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04", "number of sequences in the whole Endcap ",
  ...
  dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04",
                   "correction of dz (negative = size reduced) first part EC: %.2f",
                   dzDetector1 * 2 - dimensions.width() * 2);
  dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04", "dz second part EC: %.2f", dzDetector2 * 2);
  dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04", "width second part EC: %.2f", dimensions.dz() * 2);
  dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04",
                   "correction of dz (negative = size reduced) second part EB: %.2f",
                   dzDetector2 * 2 - dimensions.dz() * 2);

  dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04", "dz third part EC: %.2f", dzDetector3 * 2);

  // Sensitive barrel (stacks of layers) max radii
  ...
  for (int iSign = -1; iSign < 2; iSign += 2) {
    ...
    if (iSign < 0) {
      sign = -1;
      dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "Placing detector on the negative side: (cm) %.2f",
                       -(dimensions.offset() + dimensions.dz()));
    } else {
      sign = +1;
      dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "Placing detector on the positive side: (cm) %.2f",
                       (dimensions.offset() + dimensions.dz()));
    }

    // Faceplate for 1st part of extended Barrel
    DetElement facePlate1(caloDetElem, "FacePlate_" + std::to_string(1 * sign), 0);
    dd4hep::Tube facePlateShape1(dimensions.rmin1(), dimensions.rmin1() + dRhoFacePlate, dzDetector1 - dZEndPlate);
    ...
    dd4hep::Tube facePlateShape2(dimensions.rmin2(), dimensions.rmin2() + dRhoFacePlate, dzDetector2 - dZEndPlate);
    ...
    dd4hep::Tube facePlateShape3(dimensions.rmin(), dimensions.rmin() + dRhoFacePlate, dzDetector3 - dZEndPlate);
    ...
    dd4hep::Tube supportShape1(rminSupport1, rmaxSupport1, (dzDetector1 - dZEndPlate));
    ...
    dd4hep::Tube supportShape2(rminSupport2, rmaxSupport2, (dzDetector2 - dZEndPlate));
    ...
    dd4hep::Tube supportShape3(rminSupport3, rmaxSupport3, (dzDetector3 - dZEndPlate));
    ...
    for (unsigned int idxLayer = 0; idxLayer < layerDepths1.size(); ++idxLayer) {
      ...
      if (iSign < 0) {
        dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "first part: layer %d (cm): %.2f - %.2f",
                         idxLayer, rminLayer, rmaxLayer);
      }

      dd4hep::Tube layerShape(rminLayer, rmaxLayer, dzDetectorSens1);
      ...
      double zOffset = -dzDetectorSens1 + 0.5 * dzSequence;
    ...
    for (unsigned int idxLayer = 0; idxLayer < layerDepths2.size(); ++idxLayer) {
      ...
      if (iSign < 0) {
        dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "second part: layer %d (cm): %.2f - %.2f",
                         idxLayer, rminLayer, rmaxLayer);
      }

      dd4hep::Tube layerShape(rminLayer, rmaxLayer, dzDetectorSens2);
    ...
    for (unsigned int idxLayer = 0; idxLayer < layerDepths3.size(); ++idxLayer) {
      ...
      if (iSign < 0) {
        dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "third part: layer %d (cm): %.2f - %.2f",
                         idxLayer, rminLayer, rmaxLayer);
      }

      dd4hep::Tube layerShape(rminLayer, rmaxLayer, dzDetectorSens3);
  ...
  if (seg_gridphitheta || seg_phitheta) { // FCCSWGridPhiTheta_k4geo or FCCSWHCalPhiTheta_k4geo
    if (seg_gridphitheta)
      dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04", "Segmentation is of type FCCSWGridPhiTheta_k4geo");
    else
      dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04", "Segmentation is of type FCCSWHCalPhiTheta_k4geo");
  ...
  // same granularity.
  //       If the grid_size_row parameter in the XML file is different for different layers in a given section,
  ...
  //       so the pseudo-layers will not be used anyway.
    ...
    dd4hep::printout(dd4hep::DEBUG, "HCalThreePartsEndcap_o1_v04", "Segmentation is of type FCCSWHCalPhiRow_k4geo");
    // ------------------------------------
    ...
    if (!groupedRows.empty() && (groupedRows.size() % numSequences.size() != 0)) {
      dd4hep::printout(
          dd4hep::ERROR, "HCalThreePartsEndcap_o1_v04",
    ...
    for (unsigned int i_section = 0; i_section < numSequences.size(); i_section++) {
      ...
      for (unsigned int i_row = 0; i_row < numSequences[i_section]; i_row++) {
        ...
        if (rowNumber.empty() || rowNumber.back() != idx) {
          ...
          double zpos = minSectionZ + (idx - 1) * dzCell + 0.5 * dzCell;
          ...
          dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "  PseudoLayer %d", pseudoLayer);
          dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "    z-position is: %.2f cm", zpos);
          dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "    thickness along the z-axis is: %.2f cm",
                           dzCell);
          dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "    sensitive thickness is: %.2f cm",
                           thickness_sen);
          dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "    absorber thickness is: %.2f cm",
                           absorberThickness);
          dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "    number of radiation length is: %.2f",
                           nRadiationLengths);
          ...
          dd4hep::printout(dd4hep::INFO, "HCalThreePartsEndcap_o1_v04", "    cell size along R and phi: %.3f , %.3f cm",
    ...
    dd4hep::printout(dd4hep::ERROR, "HCalThreePartsEndcap_o1_v04", "Unknown segmentation");
    throw std::runtime_error("Incorrect readout in calorimeter xml description!");
  }
...
DECLARE_DETELEMENT(CaloThreePartsEndcap_o1_v04, det::createHCalEC)
