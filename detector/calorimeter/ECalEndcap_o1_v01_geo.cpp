...
static Ref_t create_detector(Detector& theDetector, xml_h e, SensitiveDetector sens) {
  ...
  if ((z_max - z_min) < mod_z) { // throw exception
    ...
    std::cout << "Got z_max - z_min = " << z_max - z_min << " cm" << std::endl;
  ...
  if (l_i < l_o) { // throw exception
    throw std::runtime_error("ERROR: This driver is for the l_i > l_o case!");
  } else {
  ...
  double ta = (l_i - l_o) / 2;
  double tb = l_o * std::sin(2 * dphi) - ta;
  double tc = l_o * std::sin(dphi);
  double td = l_o * (1 + std::cos(dphi));
  double te = td + l_o * std::cos(2 * dphi) - ta / std::tan(2 * dphi);
  double tf = tc + tb;
  double tg = tf - ta / std::tan(2 * dphi);

  std::cout << "Set a=" << ta << " b=" << tb << " c=" << tc << " d=" << td << " e=" << te << " f=" << tf << " g=" << tg
            << " (cm)" << std::endl;
  ...
  for (int m = 1; m < 3; m++) {
    ...
    for (xml_coll_t li(x_det, _U(layer)); li; ++li) {
      ...
      for (int k = 0; k < repeat; k++) {
        ...
        Trap l_box(l_thickness / 2., theta, phi, y1 - tolerance, x1 - tolerance, x2 - tolerance, alpha1, y1 - tolerance,
                   x1 - tolerance, x2 - tolerance, alpha1);
        ...
        for (xml_coll_t si(x_layer, _U(slice)); si; ++si) {
          ...
          Trap s_box(s_thick / 2., theta, phi, y1 - xtol, x1 - xtol, x2 - xtol, alpha1, y1 - xtol, x1 - xtol, x2 - xtol,
                     alpha1);
