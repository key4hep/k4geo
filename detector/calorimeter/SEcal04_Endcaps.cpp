...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  WaferGridXY* waferSeg = dynamic_cast<WaferGridXY*>(seg.segmentation());
  ...
  double Ecal_total_SiSlab_thickness = Ecal_Slab_shielding + Ecal_Slab_copper_thickness + Ecal_Slab_PCB_thickness +
                                       Ecal_Slab_glue_gap + Ecal_Si_thickness + Ecal_Slab_ground_thickness +
                                       Ecal_Alveolus_Air_Gap / 2;
#ifdef VERBOSE
  std::cout << " Ecal_total_SiSlab_thickness = " << Ecal_total_SiSlab_thickness << std::endl;
#endif

  double Ecal_total_ScSlab_thickness = Ecal_Slab_shielding + Ecal_Slab_copper_thickness + Ecal_Slab_Sc_PCB_thickness +
                                       Ecal_Sc_thickness + Ecal_Sc_reflector_thickness * 2 + Ecal_Alveolus_Air_Gap / 2;
#ifdef VERBOSE
  ...
  double alveolus_dim_z = (Ecal_Barrel_module_dim_z - 2. * Ecal_lateral_face_thickness) / Ecal_barrel_number_of_towers -
                          2 * N_FIBERS_ALVOULUS * Ecal_fiber_thickness - 2 * Ecal_Slab_H_fiber_thickness -
  ...
  double module_thickness = Ecal_nlayers1 * Ecal_radiator_thickness1 + Ecal_nlayers2 * Ecal_radiator_thickness2 +
                            ...
                            Number_of_Si_Layers_in_Barrel * // Silicon slabs plus fiber around and inside
                                (Ecal_total_SiSlab_thickness + (N_FIBERS_ALVOULUS + 1) * Ecal_fiber_thickness) +

                            Number_of_Sc_Layers_in_Barrel * // Scintillator slabs plus fiber around and inside
                                (Ecal_total_ScSlab_thickness + (N_FIBERS_ALVOULUS + 1) * Ecal_fiber_thickness) +
  ...
  double util_SI_wafer_dim_z = total_Si_dim_z / 2 - 2 * Ecal_guard_ring_size;

  double cell_dim_z = util_SI_wafer_dim_z / floor(util_SI_wafer_dim_z / cell_dim_x);
  ...
  double Ecal_endcap_rmax = Ecal_inner_radius + module_thickness + Ecal_endcap_extra_size;
  ...
  //
  double z_floor = -module_thickness / 2 + Ecal_front_face_thickness + N_FIBERS_ALVOULUS * Ecal_fiber_thickness;

  double EC_y_bottom = +Ecal_endcap_center_box_size / 2 + Ecal_lateral_face_thickness;

  double y_middle =
      (Ecal_inner_radius + module_thickness + Ecal_endcap_extra_size) * tan(M_PI / 8) - Ecal_lateral_face_thickness;

  double EC_y_top = Ecal_inner_radius + module_thickness + Ecal_endcap_extra_size - Ecal_lateral_face_thickness;

  double x_left = -(Ecal_inner_radius + module_thickness + Ecal_endcap_extra_size) + Ecal_lateral_face_thickness;

  double x_right = +Ecal_endcap_center_box_size / 2 - Ecal_lateral_face_thickness;

  double fiber_inter_alveolus =
      2 * (Ecal_Slab_H_fiber_thickness + Ecal_Slab_shielding + N_FIBERS_ALVOULUS * Ecal_fiber_thickness);
  ...
  thickness_sum = Ecal_radiator_thickness1 + (Ecal_front_face_thickness + N_FIBERS_ALVOULUS * Ecal_fiber_thickness);
  ...
  for (xml_coll_t li(x_det, _U(layer)); li; ++li) {
    ...
    for (int j = 0; j < repeat; j++) {
      ...
      while ((y_floor + EC_alveolus_dim_y) < EC_y_top) {
        ...
        for (xml_coll_t si(x_layer, _U(slice)); si; ++si) {
          ...
          if (x_slice.materialStr().compare(x_staves.materialStr()) == 0) {
            ...
            if (fill_DDRec) {
              ...
              if (!isFirstSens) {
                ...
                std::cout << " caloLayer.outer_nInteractionLengths: " << caloLayer.outer_nInteractionLengths
                          << std::endl;
          ...
          if (x_slice.isSensitive()) {
            ...
            double real_wafer_size_x = wafer_dim_z + 2 * Ecal_guard_ring_size;

            int n_wafers_x = int(floor(slab_dim_z * 2 / real_wafer_size_x));

            double wafer_pos_x = -slab_dim_z + Ecal_guard_ring_size + wafer_dim_z / 2;
            int n_wafer_x;
            int wafer_num = 0;
            for (n_wafer_x = 1; n_wafer_x < n_wafers_x + 1; n_wafer_x++) {
              double wafer_pos_z = -slab_dim_x + Ecal_guard_ring_size + wafer_dim_x / 2;
              for (int n_wafer_z = 1; n_wafer_z < 3; n_wafer_z++) {
                wafer_num++;
                string Wafer_name = _toString(wafer_num, "wafer%d");
                Volume WaferSiLog(det_name + "_" + l_name + "_" + s_name + "_" + Wafer_name, WaferSiSolid,
                                  slice_material);
                WaferSiLog.setSensitiveDetector(sens);
                WaferSiLog.setVisAttributes(theDetector.visAttributes(x_slice.visStr()));
                PlacedVolume wafer_phv = s_vol.placeVolume(WaferSiLog, Position(wafer_pos_z, wafer_pos_x, 0));
            ...
            // for that.
            double resting_dim_x = slab_dim_z * 2 - (wafer_dim_z + 2 * Ecal_guard_ring_size) * n_wafers_x;

            if (resting_dim_x > (cell_dim_z + 2 * Ecal_guard_ring_size)) {
              int N_cells_x_remaining = int(floor((resting_dim_x - 2 * Ecal_guard_ring_size) / cell_dim_z));
              ...
              if (N_cells_x_remaining % 2)
                thisWaferOffsetX = cell_dim_x / 2.0;

              wafer_pos_x = -slab_dim_z + n_wafers_x * real_wafer_size_x + (wafer_dim_x + 2 * Ecal_guard_ring_size) / 2;
              ...
              for (int n_wafer_z = 1; n_wafer_z < 3; n_wafer_z++) {
                ...
                Volume MagicWaferSiLog(det_name + "_" + l_name + "_" + s_name + "_" + MagicWafer_name,
                                       MagicWaferSiSolid, slice_material);
                MagicWaferSiLog.setSensitiveDetector(sens);
                MagicWaferSiLog.setVisAttributes(theDetector.visAttributes(x_slice.visStr()));
                PlacedVolume wafer_phv = s_vol.placeVolume(MagicWaferSiLog, Position(wafer_pos_z, wafer_pos_x, 0));
                wafer_phv.addPhysVolID("wafer", wafer_num);
...
#endif
              //-----------------------------------------------------------------------------------------
              caloLayer.distance = EC_module_z_offset + l_pos_z + (s_pos_z + s_thick / 2) - caloLayer.inner_thickness;
        ...
        if (radiator_dim_y <= 0) {
          stringstream err;
          err << " \n ERROR: The subdetector " << x_det.nameStr()
              << " geometry parameter -- radiator_dim_y = " << radiator_dim_y;
        ...
        Volume EndcapStructureLayer_vol(det_name + "_" + l_name + "_" + bs_name, EndcapStructureLayer_box,
                                        theDetector.material(x_staves.materialStr()));
        ...
        limit_staves = 4;
        // **********************
        for (int i_stave = 1; i_stave <= limit_staves; i_stave++) {
          ...
          Position l_new =
              rotz * l_pos; // The position of EC_alveolus in each stave, place same EC_alveolus into 4 stave.

          RotationZYX rot(angle_module, 0, 0);
          Transform3D tran3D(rot,
                             l_new); // The rotation of EC_alveolus in each stave, place same EC_alveolus into 4 stave.
...
#ifdef VERBOSE
          std::cout << "Ecal_endcap_towers: " << EC_Number_of_towers << " has been placed into EnvLogEndCap stave "
                    << i_stave << "." << std::endl;
#endif
          ...
          double bsl_pos_z = l_pos_z + l_thickness / 2. +
                             (radiator_dim_y / 2. + Ecal_fiber_thickness * (N_FIBERS_ALVOULUS + N_FIBERS_W_STRUCTURE));

          Position bsl_pos(-(y_floor + EC_alveolus_dim_y / 2), -(-EC_alveolus_dim_x / 2 + x_right),
                           bsl_pos_z); // Position of the layer.
      ...
      l_pos_z += (l_thickness / 2. +
                  (radiator_dim_y / 2. + Ecal_fiber_thickness * (N_FIBERS_ALVOULUS + N_FIBERS_W_STRUCTURE)) * 2.);
      ...
      caloLayer.outer_nInteractionLengths =
          nInteractionLengths + (Ecal_fiber_thickness * (N_FIBERS_ALVOULUS + N_FIBERS_W_STRUCTURE)) / air.intLength();
      caloLayer.outer_thickness = thickness_sum + (Ecal_fiber_thickness * (N_FIBERS_ALVOULUS + N_FIBERS_W_STRUCTURE));
      ...
      thickness_sum = radiator_dim_y + (Ecal_fiber_thickness * (N_FIBERS_ALVOULUS + N_FIBERS_W_STRUCTURE));
    }
  }
