...
using dd4hep::DDSegmentation::TiledLayerGridXY;
...
static bool validateEnvelope(double rInner, double rOuter, double radiatorThickness, double layerThickness,
  ...
  if (spaceNeeded > spaceAllowed) {
    printout(ERROR, "SHcalSc04_Barrel_v01", " Layer number is more than it can be built! ");
    Error = true;
  } else if (spaceToleranted < spaceAllowed) {
    printout(WARNING, "SHcalSc04_Barrel_v01", " Layer number is less than it is able to build!");
    Warning = true;
  } else {
    printout(DEBUG, "SHcalSc04_Barrel_v01", " has been validated and start to build it.");
    Error = false;
    Warning = false;
  }
...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  printout(DEBUG, "SHcalSc04_Barrel_v01", "TPC_outer_radius : %e   - Ecal_outer_radius: %e ", TPC_outer_radius,
           Ecal_outer_radius);
  ...
  TiledLayerGridXY* tileSeg = dynamic_cast<TiledLayerGridXY*>(seg.segmentation());
  ...
  // double Hcal_cell_dim_z            = Hcal_regular_chamber_dim_z / floor
  // (Hcal_regular_chamber_dim_z/Hcal_cell_dim_x);
  ...
  for (int layer_id = 1; layer_id <= (2 * Hcal_nlayers); layer_id++) {
    ...
    //-------------------------------------------------------------------------------
    printout(DEBUG, "SHcalSc04_Barrel_v01", " \n Start to build layer chamber - layer_id: %d", layer_id);
    printout(DEBUG, "SHcalSc04_Barrel_v01", " chamber x:y:z:  %e:%e:%e", x_length * 2., z_width * 2., y_height * 2.);
    ...
    Box ChamberSolid(
        (x_length + Hcal_layer_air_gap), // x + air gaps at two side, do not need to build air gaps individualy.
        z_width,                         // z attention!
    ...
    for (xml_coll_t k(x_layer, _U(slice)); k; ++k) {
      ...
      if (x_slice.isSensitive()) {
        ...
        printout(DEBUG, "SHcalSc04_Barrel_v01", "  logical_layer_id:  %d  slice_id:  %d", logical_layer_id, slice_id);
      }
    ...
    pv = EnvLogHcalModuleBarrel.placeVolume(
        ChamberLogical, Position(chamber_x_offset, chamber_z_offset, chamber_y_offset + chambers_y_off_correction));

    //-----------------------------------------------------------------------------------------
    if (layer_id <= Hcal_nlayers) { // add the first set of layers to the reconstruction data

      caloLayer.distance =
          Hcal_inner_radius + Hcal_total_dim_y / 2.0 - (chamber_y_offset + chambers_y_off_correction) -
          caloLayer.inner_thickness; // Will be added later at "DDMarlinPandora/DDGeometryCreator.cc:226" to get center
  ...
  if (tileSeg != 0) {
    ...
    for (std::vector<double>::const_iterator ix = LOX.begin(); ix != LOX.end(); ++ix)
      sts << *ix << ' ';
    printout(DEBUG, "SHcalSc04_Barrel_v01", "%s", sts.str().c_str());
    sts.clear();
    sts.str("");
    sts << " layerOffsetY(): ";
