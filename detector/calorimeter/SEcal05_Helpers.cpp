...
void SEcal05_Helpers::checkLayerConsistency() {
  ...
  if (_preshower == 1 && n_total_abs_layers % 2 == 0) {
    std::cout << "SEcal05_Helpers ERROR: inconsistent ECAL model !! if you request a preshower layer, the number of "
    ...
    std::cout << "SEcal05_Helpers ERROR: inconsistent ECAL model !! if you request no preshower layer, the number of "
...
double
SEcal05_Helpers::getAbsThickness(unsigned int iAbsLay) { //, int n1, int n2, int n3, double t1, double t2, double t3) {
...
void SEcal05_Helpers::updateCaloLayers(double thickness, dd4hep::Material mat, bool isAbsorber, bool isSensitive,
...
SEcal05_Helpers::dxinfo SEcal05_Helpers::getNormalMagicUnitsInX(double dx_total, double dx_unit, double dx_cell,
        ...
        for (xml_coll_t si(x_layer, _U(slice)); si; ++si) { // the sub-layers
          ...
          if (!x_slice.isSensitive()) { // not the sensitive slice: just a layer of stuff
            ...
            for (int n_wafer_x = 0; n_wafer_x < n_wafers_x + 2;
              ...
              // cout << "ismagic " << isMagic << " wafer " << n_wafer_x << " / " << n_wafers_x << " : sizex " <<
              // megatile_size_x << " nx " << ncellsx << endl;
