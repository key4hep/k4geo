...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  PolyhedraRegular stave_shaper_LP(Hcal_inner_symmetry, Hcal_inner_radius, Hcal_outer_radius, DHZ_LP);
  ...
  for (int layer_id = 1; layer_id <= Hcal_nlayers; layer_id++) {
    ...
    double yn = sqrt(Hcal_outer_radius * Hcal_outer_radius - (Hcal_inner_radius + layer_id * layer_thickness) *
                                                                 (Hcal_inner_radius + layer_id * layer_thickness));
    ...
    if ((Yn <= 0 && yn <= xShift) || (CellsInTransvPlane < Hcal_MinNumCellsInTransvPlane))
      continue;
    ...
    chamber_y_offset = -(-Hcal_total_dim_y / 2. + (layer_id - 1) * layer_thickness + layer_thickness / 2.);
  ...
  for (int stave_id = 1; stave_id <= Hcal_inner_symmetry; stave_id++) {
    ...
    for (int module_id = 1; module_id <= Hcal_barrel_number_modules; module_id++) {
      ...
      if (stave_id == 1) { // place only once for the whole lateral_plate
        ...
        cout << " lateral_plate_z_offset LP2,  module_id, stave_id:   " << lateral_plate_z_offset << " , " << module_id
             << " , " << stave_id << endl;
      }
    }
