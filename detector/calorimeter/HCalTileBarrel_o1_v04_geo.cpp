...
static dd4hep::Ref_t createHCal(dd4hep::Detector& lcdd, xml_det_t xmlDet, dd4hep::SensitiveDetector sensDet) {
  ...
  dd4hep::printout(dd4hep::DEBUG, "HCalTileBarrel_o1_v04", "steel support thickness (cm): %.2f", dSteelSupport);
  ...
  if (!sequences[0] || !sequences[1]) {
    dd4hep::printout(dd4hep::ERROR, "HCalTileBarrel_o1_v04",
  ...
  dd4hep::printout(dd4hep::DEBUG, "HCalTileBarrel_o1_v04", "sequence thickness %.2f", dzSequence);
  ...
  double masterPlateThk = 0.;
  for (xml_coll_t xCompColl(sequences[0], _Unicode(module_component)); xCompColl; ++xCompColl) {
    xml_comp_t xComp = xCompColl;
    if ((masterPlateThk <= 0) && (xComp.nameStr() == "master")) {
      masterPlateThk = xComp.thickness();
      break;
    }
  }
  double dzDetectorSens =
      (numSequencesZ * dzSequence - masterPlateThk) / 2; // net (i.e. only layers) detector half-width along z
  double dzDetector =
      dzDetectorSens + dZEndPlate + space; // gross (i.e. layers + side mechanics) detector half-width along z

  dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "dzDetector (cm): %.2f", dzDetector);
  dd4hep::printout(dd4hep::DEBUG, "HCalTileBarrel_o1_v04", "correction of dz in cm (negative = size reduced): %.2f",
                   dzDetector - xDimensions.dz());

  dd4hep::printout(dd4hep::DEBUG, "HCalTileBarrel_o1_v04",
                   "retrieved number of radial layers: %d , which end up to a full module depth in rho of %.2f cm",
                   numLayersR, moduleDepth);
  dd4hep::printout(dd4hep::DEBUG, "HCalTileBarrel_o1_v04", "retrieved number of radial layers: %d", layerDepths.size());
  dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04",
  ...
  for (unsigned int idxLayer = 0; idxLayer < layerDepths.size(); ++idxLayer) {
    ...
    dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "layer %d (cm): %.2f - %.2f", idxLayer, rminLayer,
                     rmaxLayer);
    ...
    for (xml_coll_t xCompColl(sequences[sequenceIdx], _Unicode(module_component)); xCompColl;
      ...
      if ((xComp.nameStr() == "master") && (tileZOffset <= -0.5 * dzSequence_L))
        continue; // i.e. cut sequence filling at the final master plate

      dd4hep::Position tileOffset(0, 0, tileZOffset + 0.5 * xComp.thickness());
      tileSequenceVolume_L.placeVolume(tileVol, tileOffset);
  ...
  if (seg_phitheta) {
    dd4hep::printout(dd4hep::DEBUG, "HCalTileBarrel_o1_v04", "Segmentation is of type FCCSWHCalPhiTheta_k4geo");
    layerFieldName = seg_phitheta->fieldNameLayer();
  } else if (seg_phirow) {
    dd4hep::printout(dd4hep::DEBUG, "HCalTileBarrel_o1_v04", "Segmentation is of type FCCSWHCalPhiRow_k4geo");
    layerFieldName = seg_phirow->fieldNameLayer();
  } else {
    dd4hep::printout(dd4hep::ERROR, "HCalTileBarrel_o1_v04", "Unknown segmentation");
  ...
  dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "Layer structure information:");
  for (unsigned int idxLayer = 0; idxLayer < layerDepths.size(); ++idxLayer) {
    dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "  Layer %d", idxLayer);
    ...
    dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "    sensitive thickness is: %lf", thickness_sen);
    dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "    absorber thickness is: %lf", absorberThickness);
    dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "    number of radiation length is: %lf",
                     nRadiationLengths);
    dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "    number of interaction length is: %lf",
                     nInteractionLengths);
    ...
    if (seg_phitheta) {
      ...
      dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "    cell sizes in theta, phi: %lf rad , %lf rad",
      ...
      dd4hep::printout(dd4hep::INFO, "HCalTileBarrel_o1_v04", "    cell sizes in z, phi: %.4f cm , %.4f cm", cellSizeZ,
                       cellSizePhi);
...
DECLARE_DETELEMENT(HCalTileBarrel_o1_v04, det::createHCal)
