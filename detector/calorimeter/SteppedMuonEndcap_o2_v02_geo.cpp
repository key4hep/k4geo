  ...
  for (xml_coll_t c(x_det, _U(layer)); c; ++c) {
    ...
    for (int j = 0; j < layer_repeat; ++j) {
      ...
      for (xml_coll_t s(x_layer, _U(slice)); s; ++s) {
        ...
        Volume slice_vol(slice_name, PolyhedraRegular(nsides_outer, rmin + layer_rcutout, layer_rmax, slice_thickness),
                         slice_material);
      ...
      pv = envelopeVol.placeVolume(layer_vol, Transform3D(RotationZYX(0, M_PI, 0), Position(0, 0, -z_pos)));
      pv.addPhysVolID("side", -1);
      pv.addPhysVolID("layer", layer_num);
