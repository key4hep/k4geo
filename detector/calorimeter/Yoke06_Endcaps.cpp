...
// -- modified to give a square central hole as in ILD design: DJeans Mar12, 2018
//
...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  //... Barrel parameters:
  //... tolerance 1 mm
  double yokeBarrelThickness = gap_thickness + number_of_layers * (iron_thickness + gap_thickness) +
                               3 * (5.6 * iron_thickness + gap_thickness) + 0.1; // the tolerance 1 mm

  double yokeEndcapThickness =
      number_of_layers * (iron_thickness + gap_thickness) + 2 * (5.6 * iron_thickness + gap_thickness) + gap_thickness;
  ...
  Box innerBox(rInnerEndcap, rInnerEndcap, zStartEndcap + Yoke_Endcap_module_dim_z); // this is the square hole

  //  PolyhedraRegular YokeEndcapSolid( symmetry, M_PI/symmetry, rInnerEndcap, rOuterEndcap,  Yoke_Endcap_module_dim_z);

  SubtractionSolid YokeEndcapSolid(
      PolyhedraRegular(symmetry, M_PI / symmetry, rInnerEndcap, rOuterEndcap, Yoke_Endcap_module_dim_z), innerBox,
      Position(0, 0, 0));
  ...
  for (xml_coll_t li(x_det, _U(layer)); li; ++li) {
    ...
    for (int i = 0; i < repeat; i++) {
      ...
      if (i >= 10)
        radiator_thickness = gap_thickness + 5.6 * iron_thickness - l_thickness;
      ...
      for (xml_coll_t si(x_layer, _U(slice)); si; ++si) {
        ...
        // PolyhedraRegular sliceSolid( symmetry, M_PI/symmetry, rInnerEndcap + tolerance, rOuterEndcap - tolerance,
        // s_thickness);

        SubtractionSolid sliceSolid(PolyhedraRegular(symmetry, M_PI / symmetry, rInnerEndcap + tolerance,
                                                     rOuterEndcap - tolerance, s_thickness),
                                    innerBox, Position(0, 0, 0));
      ...
      double shift_middle = -yokeEndcapThickness / 2 + 0.05 // 0.5*mm
                            + iron_thickness * (i + 1) + (i + 0.5) * gap_thickness;
      ...
      caloLayer.distance = zStartEndcap + yokeEndcapThickness / 2.0 + shift_middle - caloLayer.inner_thickness;
  ...
  double zEndcap =
      zStartEndcap + yokeEndcapThickness / 2.0 + 0.1; // Need 0.1 (1.0*mm) according to the Mokka Yoke05 driver.
  double zPlug =
      zStartEndcap - plug_thickness / 2.0 - 0.05; //  Need 0.05 (0.5*mm) according to the Mokka Yoke05 driver.
  ...
  for (int module_num = 0; module_num < 2; module_num++) {
    ...
    if (build_plug == true) {
      ...
      SubtractionSolid YokePlugSolid(
          PolyhedraRegular(symmetry, M_PI / symmetry, rInnerPlug, rOuterPlug, Yoke_Plug_module_dim_z), innerBox,
          Position(0, 0, 0));
...
DECLARE_DETELEMENT(Yoke06_Endcaps, create_detector)
