...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  double SpaceForLayers = HcalEndcapRing_max_z - HcalEndcapRing_min_z - Hcal_back_plate_thickness;
  ...
  stop_z = start_z + MaxNumberOfLayers * (Hcal_chamber_thickness + Hcal_radiator_thickness) + Hcal_back_plate_thickness;
  ...
  Position IntersectXYZtrans((pRMax + (Hcal_stave_gaps / 2.)), (pRMax + (Hcal_stave_gaps / 2.)),
                             (Hcal_total_dim_y / 2.));
  ...
  for (int stave_id = 1; stave_id <= 4; stave_id++) {
    ...
    for (int layer_id = 1; layer_id <= number_of_chambers; layer_id++) {
      double Zoff = zlen / 2. - (layer_id - 1) * (Hcal_chamber_thickness + Hcal_radiator_thickness) -
      ...
      for (xml_coll_t k(x_layer, _U(slice)); k; ++k) {
        ...
        // set up the translation and rotation for the intersection process
        // this happens in the mother volume coordinate system, so a coordinate transformation is needed
        Position sIntersectXYZtrans((pRMax + (Hcal_stave_gaps / 2.)), (pRMax + (Hcal_stave_gaps / 2.)),
                                    (Hcal_total_dim_y / 2.));
