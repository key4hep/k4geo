...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens)  {
  ...
  cout << "SEcal06_Barrel: absorber layers " << 
  ...
  if ( fabs( Ecal_barrel_thickness - module_thickness ) > 0.01 ) {
    cout << "ERROR SEcal06_Barrel : ECAL barrel thickness in comapct decription not consistent with what I calculate!" << endl;
  ...
  if ( min_dim_x<0 || max_dim_x<0 ) {
    std::cout << "SEcal06_Barrel ERROR : requesting too many sides! barrel modules have max/min extent " << max_dim_x << " " << min_dim_x << std::endl;
