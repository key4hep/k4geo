 ...
 *
 * D. Jeans: 07/2017: adaption for "hybrid ecal" with multi-technology readout
 *
 *
 */
...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  cout << "-----------------------" << endl;
  cout << "creating SEcal06_Barrel" << endl;
  cout << "-----------------------" << endl;
  ...
  double Ecal_barrel_thickness =
      theDetector.constant<double>("Ecal_barrel_thickness"); // what's assumed in the compact description
  ...
  // const int N_FIBERS_W_STRUCTURE = 2; // number of CF layers around absorber layers in the structure
  // const int N_FIBERS_ALVEOLUS    = 3; // number of CF layers used to make alveolus
  double Ecal_fiber_thickness_structure =
      theDetector.constant<double>("Ecal_fiber_thickness_structure"); // absorber wrapping thickness
  double Ecal_fiber_thickness_alveolus =
      theDetector.constant<double>("Ecal_fiber_thickness_alveolus"); // alveolar wall thickness
  ...
  try {
    ...
    cout << "SEcal06_Barrel: Ecal_plugLength not found, using " << Ecal_plugLength << endl;
  }
  ...
  SEcal06_Helpers helper;
  helper.setDet(&x_det);

  //   absorber layer structure
  helper.setPreshower(Ecal_Barrel_PreshowerLayer);

  cout << "SEcal06_Barrel: Preshower ? " << Ecal_Barrel_PreshowerLayer << endl;

  helper.setAbsLayers(Ecal_nlayers1, Ecal_radiator_thickness1, Ecal_nlayers2, Ecal_radiator_thickness2, Ecal_nlayers3,
                      Ecal_radiator_thickness3);

  cout << "SEcal06_Barrel: absorber layers " << Ecal_nlayers1 << "*" << Ecal_radiator_thickness1 / dd4hep::mm
       << " mm + " << Ecal_nlayers2 << "*" << Ecal_radiator_thickness2 / dd4hep::mm << " mm + " << Ecal_nlayers3 << "*"
       << Ecal_radiator_thickness3 / dd4hep::mm << " mm " << endl;
  ...
  helper.setCFthickness(Ecal_fiber_thickness_structure, // was N_FIBERS_W_STRUCTURE*Ecal_fiber_thickness, : updated DJ
                        ...
                        Ecal_front_face_thickness, Ecal_support_thickness);
  ...
  if (fabs(Ecal_barrel_thickness - module_thickness) > 0.01) {
    cout << "ERROR SEcal06_Barrel : ECAL barrel thickness in comapct decription not consistent with what I calculate!"
         << endl;
    cout << "    calculated = " << module_thickness << "    compact description: " << Ecal_barrel_thickness << endl;
    assert(0); // exit
  }

  cout << "SEcal06_Barrel : module thickness = " << module_thickness << endl;
  ...
  cout << "SEcal06_Barrel : layer config: ";
  ...
  cout << "SEcal06_Barrel : width of module, alveolus = " << Ecal_Barrel_module_dim_z << " " << alv_width << endl;

  std::vector<int> ntowers;
  ntowers.push_back(Ecal_barrel_number_of_towers);

  helper.setTowersUnits(ntowers, alv_width, Ecal_n_wafers_per_tower, Ecal_lateral_face_thickness,
  ...
  //  double max_dim_x    = 2. * tan(M_PI/8.) * Ecal_inner_radius + module_thickness_noSupport/sin(M_PI/4.); // longest
  //  side assumes oct double min_dim_x = max_dim_x - 2*module_thickness; // shorter one (assumes octagon)

  //  double max_dim_x    = 2. * tan(M_PI/nsides) * Ecal_inner_radius + module_thickness_noSupport / tan( 2*M_PI/nsides
  //  );  // longest side
  double max_dim_x = 2. * tan(M_PI / nsides) * Ecal_inner_radius +
                     module_thickness_noSupport / sin(2 * M_PI / nsides);       // longest side : fix DJeans 03/2017
  double min_dim_x = max_dim_x - 2 * module_thickness / tan(2 * M_PI / nsides); // shorter one

  if (min_dim_x < 0 || max_dim_x < 0) {
    std::cout << "SEcal06_Barrel ERROR : requesting too many sides! barrel modules have max/min extent " << max_dim_x
              << " " << min_dim_x << std::endl;
    std::cout << "exiting" << std::endl;
    assert(0); // exit gracefully
  }

  Trapezoid trd(max_dim_x / 2, min_dim_x / 2, Ecal_Barrel_module_dim_z / 2, Ecal_Barrel_module_dim_z / 2,
                module_thickness / 2);
  ...
  helper.makeModule(mod_vol, stave_det, *caloData, theDetector, sens);

  for (size_t i = 0; i < caloData->layers.size(); i++) {
    caloData->layers[i].distance += Ecal_inner_radius; // add IP->front face distance
  }

  //  cout << "SEcal06_Barrel : cell sizes: " << endl;
  //  for (size_t i=0; i<caloData->layers.size(); i++) {
  //    cout << "sensitive layer " << i << " : x,y = " << caloData->layers[i].cellSize0 << " " <<
  //    caloData->layers[i].cellSize1 << endl;
  //  }
  ...
  for (int istave = 0; istave < nsides; istave++) {
    ...
    // dstave is the change in stave index from the top module to the one with smallest positive phi (which has
    // istave=0)
    ...
    for (int imodule = 0; imodule < Ecal_barrel_z_modules; imodule++) {
      ...
      Transform3D tr(RotationZYX(0, phirot, M_PI / 2.), // magic rotation!
                     Translation3D((X * cos(phirot2) - Y * sin(phirot2)), (X * sin(phirot2) + Y * cos(phirot2)),
      ...
      DetElement sd = (imodule == 0 && istave == 0)
                          ? stave_det
  ...
  //  cout << "finished SEcal06_Barrel" << endl;

  return sdet;
}

DECLARE_DETELEMENT(SEcal06_Barrel, create_detector)
