...
static dd4hep::detail::Ref_t createECalBarrelInclined(dd4hep::Detector& aLcdd, dd4hep::xml::Handle_t aXmlElement,
  ...
  if (activePassiveOverlap < 0 || activePassiveOverlap > 0.5) {
    ...
    lLog << ERROR << "Overlap between active and passive cannot be more than half of passive plane!" << endmsg;
  ...
  lLog << DEBUG << "Number of layers: " << numLayers << " total thickness " << layersTotalHeight << endmsg;
  ...
  if (nLayers > 0 && nLayers != int(numLayers)) {
    lLog << ERROR << "Incorrect number of layers (ECalBarrelNumLayers) in xml file!" << endmsg;
    // todo: incidentSvc->fireIncident(Incident("ECalConstruction", "GeometryFailure"));
  ...
  if (cryoThicknessFront > 0) {
    ...
    lLog << INFO << "ECAL cryostat: front: rmin (cm) = " << cryoDim.rmin1() << " rmax (cm) = " << cryoDim.rmin2()
         << " dz (cm) = " << cryoDim.dz() << endmsg;
    lLog << INFO << "ECAL cryostat: back: rmin (cm) = " << cryoDim.rmax1() << " rmax (cm) = " << cryoDim.rmax2()
         << " dz (cm) = " << cryoDim.dz() << endmsg;
    lLog << INFO << "ECAL cryostat: side: rmin (cm) = " << cryoDim.rmin2() << " rmax (cm) = " << cryoDim.rmax1()
    ...
    if (cryoFrontSensitive) {
      ...
      lLog << INFO << "Cryostat front volume set as sensitive" << endmsg;
    }
    if (cryoBackSensitive) {
      ...
      lLog << INFO << "Cryostat back volume set as sensitive" << endmsg;
    }
    ...
    lLog << INFO << "ECAL services: front: rmin (cm) = " << cryoDim.rmin2() << " rmax (cm) = " << bathRmin
         << " dz (cm) = " << caloDim.dz() << endmsg;
    lLog << INFO << "ECAL services: back: rmin (cm) = " << bathRmax << " rmax (cm) = " << cryoDim.rmax1()
         << " dz (cm) = " << caloDim.dz() << endmsg;
    ...
    if (cryoFrontSensitive) {
      ...
      lLog << INFO << "Services front volume set as sensitive" << endmsg;
    }
    if (cryoBackSensitive) {
      ...
      lLog << INFO << "Services back volume set as sensitive" << endmsg;
    }
  ...
  lLog << INFO << "ECAL bath: material = " << activeMaterial << " rmin (cm) =  " << bathRmin
  ...
  lLog << INFO << "ECAL calorimeter volume rmin (cm) =  " << caloDim.rmin() << " rmax (cm) = " << caloDim.rmax()
       << endmsg;
  ...
  lLog << INFO << "passive inner material = " << passiveInnerMaterial << "\n"
  ...
  lLog << INFO << "number of passive plates = " << numPlanes << " azim. angle difference =  " << dPhi << endmsg;
  lLog << INFO << " distance at inner radius (cm) = " << 2. * M_PI * caloDim.rmin() / numPlanes << "\n"
  ...
  if (nModules > 0 && nModules != int(numPlanes)) {
    lLog << ERROR << "Incorrect number of planes (ECalBarrelNumPlanes) in xml file!" << endmsg;
    // todo: incidentSvc->fireIncident(Incident("ECalConstruction", "GeometryFailure"));
    // make the code crash (incidentSvc does not work)
    // Andre, Alvaro, assert replaced by exception
  ...
  lLog << INFO << "readout material = " << readoutMaterial << "\n"
  ...
  lLog << INFO << "thickness of calorimeter (cm) = " << dR << "\n"
  ...
  for (uint iLay = 0; iLay < numLayers; iLay++) {
    ...
    lLog << DEBUG << "Thickness of layer " << iLay << " : " << layerHeight[iLay] << endmsg;
  }
  ...
  if (passiveInner.isSensitive()) {
    lLog << DEBUG << "Passive inner volume set as sensitive" << endmsg;
  ...
  if (passiveOuter.isSensitive()) {
    lLog << DEBUG << "Passive outer volume set as sensitive" << endmsg;
  ...
  if (passiveGlue.isSensitive()) {
    lLog << DEBUG << "Passive glue volume set as sensitive" << endmsg;
    double layerOffset = layerFirstOffset / cosPassiveAngle;
    for (uint iLayer = 0; iLayer < numLayers; iLayer++) {
  ...
  lLog << INFO << "active material = " << activeMaterial
