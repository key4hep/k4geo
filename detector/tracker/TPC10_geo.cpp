...
static Ref_t create_element(Detector& theDetector, xml_h e, SensitiveDetector sens) {
  ...
  cout << " Cathode Grip Ring Material: " << material_Cathode_Grip->GetName()
       << " : Rad length = " << material_Cathode_Grip->GetMaterial()->GetRadLen() / mm << " mm." << std::endl;
  ...
  // note the will be two gas volumes one in each z-half. The cathode and readout are considered to be placed inside the
  // Gas volume
  ...
  const double dz_Sensitive =
      dz_GasVolume - (dz_Cathode / 2.0 + dz_Readout); // the d_Cathode spans both halfs of the TPC
  ...
  //  //     Control::Abort("TPC endplate material fractions do not add up to
  //  100%",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
  //  //   }
  //  // Material *endplate_MaterialMix = new Material("TPC_endplate_mix", densityTotal, material_fractions.size());
  //  // for(  std::map<Material* const, double>::iterator it=material_fractions.begin();
  //  it!=material_fractions.end();++it )
  //  //   {
  ...
  cout << "Endplate material mix Density   = " << endplate_MaterialMix->GetMaterial()->GetDensity() / g * cm3
       << " g/cm3" << endl;
  cout << "Endplate material mix Radlength = " << endplate_MaterialMix->GetMaterial()->GetRadLen() / mm << " mm"
       << endl;
  ...
  cout << "TPC10: Total Gas material corresponds to "
       << (((rOuter - dr_OuterWall) - (rInner + dr_InnerWall)) / (material_TPC_Gas->GetMaterial()->GetRadLen() / mm) *
           100.0)
  ...
  for (xml_coll_t c(x_iWall, _Unicode(row)); c; ++c) {
    ...
    cout << "TPC10: Add Material to Inner Wall: dr =  " << std::setw(4) << dr / mm
         << " mm. Material = " << layerMaterial->GetName() << " X0 = " << layerMaterial->GetMaterial()->GetRadLen()
         << "  " << dr / layerMaterial->GetMaterial()->GetRadLen() << "% X0" << endl;
    ...
    //     cout << "TPC10: gear_inner_wall_material_total_density = " << std::setw(4) <<
    //     gear_inner_wall_material_total_density << endl; if( gear_inner_wall_material_thicknesses.find( material_name
    //     ) ==  gear_inner_wall_material_thicknesses.end() ) {
  ...
  cout << "TPC10: Inner wall material corresponds to " << int(fracRadLengthInnerWall * 1000) / 10.
       << "% of a radiation length." << endl;
  ...
  for (xml_coll_t c(x_oWall, _Unicode(row)); c; ++c) {
    ...
    cout << "TPC10: Add Material to Outer Wall: dr =  " << std::setw(4) << dr / mm
         << " mm. Material = " << layerMaterial->GetName() << " X0 = " << layerMaterial->GetMaterial()->GetRadLen()
         << "  " << dr / layerMaterial->GetMaterial()->GetRadLen() << "% X0" << endl;
    ...
    //     std::string material_name = db->fetchString("material");
    //     cout << "TPC10: gear_outer_wall_material_total_density = " << std::setw(4) <<
    //     gear_outer_wall_material_total_density << endl; if( gear_outer_wall_material_thicknesses.find( material_name
    //     ) ==  gear_outer_wall_material_thicknesses.end() )
    //       {
  ...
  cout << "TPC10: Outer wall material corresponds to " << int(fracRadLengthOuterWall * 1000) / 10.0
       << "% of a radiation length." << endl;
  ...
  // insulator
  Tube cathodeInsulatorSolid(rMin_Sensitive, rMax_Sensitive, (dz_Cathode_Insulator / 2.0) - 0.00000001 * mm, phi1,
                             phi2);
  ...
  // place plus and minus z
  pv = cathodeLog.placeVolume(cathodeConductorLog,
                              Position(0.0, 0.0, +(dz_Cathode_Insulator + (dz_Cathode_Conductor / 2.0))));
  pv = cathodeLog.placeVolume(cathodeConductorLog,
                              Position(0.0, 0.0, -(dz_Cathode_Insulator + (dz_Cathode_Conductor / 2.0))));
  //-----------------------------------------------------------------------------------------------//
  ...
  // new PVPlacement(Transform3D(RotationMatrix().rotateY(   0 * deg), ThreeVector(0, 0, +( dz_Cathode/2.0 +
  // dz_Sensitive/2.0 ) )), sensitiveGasLog, "TPCSensitiveLog_+z", motherLog, false, 0); new
  // PVPlacement(Transform3D(RotationMatrix().rotateY( 180 * deg), ThreeVector(0, 0, -( dz_Cathode/2.0 +
  // dz_Sensitive/2.0 ) )), sensitiveGasLog, "TPCSensitiveLog_-z", motherLog, false, 1);

  DetElement sensGasDEfwd(tpc, "tpc_sensGas_fwd", x_det.id());
  DetElement sensGasDEbwd(tpc, "tpc_sensGas_bwd", x_det.id());

  pv = tpc_motherLog.placeVolume(sensitiveGasLog,
                                 Transform3D(RotationY(0.), Position(0, 0, +(dz_Cathode / 2.0 + dz_Sensitive / 2.0))));
  pv.addPhysVolID("side", +1);
  sensGasDEfwd.setPlacement(pv);

  pv = tpc_motherLog.placeVolume(sensitiveGasLog,
                                 Transform3D(RotationY(pi), Position(0, 0, -(dz_Cathode / 2.0 + dz_Sensitive / 2.0))));
  pv.addPhysVolID("side", -1);
  ...
  for (int layer = 0; layer < numberPadRows; layer++) {
...
#if 1
    // create twice the number of rings as there are pads, producing an lower and upper part of the pad with the boundry
    // between them the pad-ring centre
    ...
    VolCylinder surf(upperlayerLog, SurfaceType(SurfaceType::Sensitive, SurfaceType::Invisible, SurfaceType::Unbounded),
                     (padHeight / 2.0), (padHeight / 2.0), o);
    ...
    VolCylinder surf(layerLog, SurfaceType(SurfaceType::Sensitive, SurfaceType::Invisible), (padHeight / 2.0),
                     (padHeight / 2.0), o);
  ...
  pv = tpc_motherLog.placeVolume(readoutLog,
                                 Transform3D(RotationY(0.), Position(0, 0, +((dz_GasVolume - (dz_Readout / 2.0))))));
  pv = tpc_motherLog.placeVolume(readoutLog,
                                 Transform3D(RotationY(pi), Position(0, 0, -((dz_GasVolume - (dz_Readout / 2.0))))));

  // new PVPlacement(Transform3D(RotationMatrix().rotateY(  0 * deg), ThreeVector( 0, 0, +(dz_GasVolume - (dz_Readout
  // / 2.0) ))), readoutLog, "TPCReadout_+z", motherLog, false, 0); new
  // PVPlacement(Transform3D(RotationMatrix().rotateY(180 * deg), ThreeVector( 0, 0, -(dz_GasVolume - (dz_Readout / 2.0)
  // ))), readoutLog, "TPCReadout_-z", motherLog, false, 1);
  ...
  // Some verbose output
  cout << "TPC10: Readout material corresponds to " << int(fracRadLengthReadout * 1000) / 10.0
       << "% of a radiation length." << endl;
  ...
  VolPlane surf(endplateLog, SurfaceType(SurfaceType::Helper), dz_Endplate / 2.0 + dz_Readout / 2, dz_Endplate / 2.0, u,
                v, n, o);
  ...
  pv = tpc_motherLog.placeVolume(endplateLog,
                                 Transform3D(RotationY(0.), Position(0, 0, +(dz_GasVolume + (dz_Endplate / 2.0)))));
  endcapDEfwd.setPlacement(pv);
  pv = tpc_motherLog.placeVolume(endplateLog,
                                 Transform3D(RotationY(pi), Position(0, 0, -(dz_GasVolume + (dz_Endplate / 2.0)))));
  endcapDEbwd.setPlacement(pv);

  // new PVPlacement(Transform3D(RotationMatrix().rotateY(  0 * deg), ThreeVector( 0, 0, +(dz_GasVolume + (dz_Endplate
  // / 2.0) ))), endplateLog, "TPCEndplate_+z", motherLog, false, 0); new
  // PVPlacement(Transform3D(RotationMatrix().rotateY(180 * deg), ThreeVector( 0, 0, -(dz_GasVolume + (dz_Endplate
  // / 2.0) ))), endplateLog, "TPCEndplate_-z", motherLog, false, 1);

  cout << "TPC10: Total Endplate material corresponds to "
       << (fracRadLengthReadout * 100.0) +
              (dz_Endplate / (endplate_MaterialMix->GetMaterial()->GetRadLen() / mm) * 100.0)
       << "% of a radiation length." << endl;
  ...
  //   _gear_max_drift_length = dz_Sensitive + dz_Cathode/2.0; // SJA: cathode has to be added as the sensitive region
  //   does not start at 0.00 _gear_z_anode = dzTotal - dz_Endplate; // the edge of the readout terminating the drift
  //   volume
  // #endif
  ...
  tpcData->driftLength =
      dz_Sensitive + dz_Cathode / 2.0; // SJA: cathode has to be added as the sensitive region does not start at 0.00
