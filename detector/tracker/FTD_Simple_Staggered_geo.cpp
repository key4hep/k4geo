...
// void DoAndPlaceDisk( Detector& theDetector,DetElement det,SensitiveDetector sens, std::map<std::string,double>
// valuesDict, Volume  mother ) ;
void petalSupport(Detector& theDetector, DetElement det, std::map<std::string, double> valuesDict,
                  Volume FTDPetalAirLogical);
VolVec petalSensor(Detector& theDetector, DetElement ftd, SensitiveDetector sens,
                   std::map<std::string, double> valuesDict, Volume FTDPetalAirLogical);
Trap SemiPetalSolid(const double& petal_cp_support_dy, const std::string& whereItgoes, const bool isSilicon);

// debug print function
void printVolume(Volume v) { std::cout << " ++++ create Volume " << v.name() << std::endl; }
...
void SetEnvironPar(const EnvDetector& env) {
  ...
  _glEnv.beamTubeTangent =
      (_glEnv.rEnd_IPOuterBulge - _glEnv.rEnd_IPOuterTube) / (_glEnv.zEnd_IPOuterBulge - _glEnv.zEnd_IPOuterTube);
}
...
void SetdbParCommon(xml_comp_t x_det) {
  ...
  // db->exec("select * from common_parameters;");
  {
    ...
    if (_dbParCommon.beamTubeClearance <
        (_dbParCommon.cables_thickness + (2.0 * _dbParCommon.cable_shield_thickness) + 0.5 * mm)) {
      cout << "FTD_Simple_Staggered:Stop: Not enough space for inner support structure and cables: increase "
              "beamTubeClearance"
           << endl;
      exit(1);
    }
...
static Ref_t create_element(Detector& theDetector, xml_h e, SensitiveDetector sens) {
  ...
  cout << "FTD_Simple_Staggered:"
       ...
       << "\t beamTubeClearance = " << _dbParCommon.beamTubeClearance << endl;
  ...
  for (xml_coll_t c(x_det, _U(disk)); c; ++c) {
    ...
    case 1:
      ...
      _zEnd = _z_position + _dbParDisk.petal_support_zoffset + 0.5 * _dbParDisk.petal_cp_support_thickness +
              (_dbParDisk.double_sided * _dbParDisk.disks_Si_thickness);

      // check which part of the beam tube this disk lies above
      _beamTubeRadius = (_zEnd < _glEnv.zEnd_IPOuterTube)
                            ? _glEnv.rEnd_IPOuterTube
                            : _glEnv.rEnd_IPOuterTube + ((_zEnd - _glEnv.zEnd_IPOuterTube) * _glEnv.beamTubeTangent);
      ...
      if (_z_position <= _glEnv.SIT1_Half_Length_Z && _outer_radius >= _glEnv.SIT1_Radius) {
        ...
        cout << "FTD_Simple_Staggered:FTD1 Radius = " << _outer_radius << "SIT1 Radius = " << _glEnv.SIT1_Radius
             << endl;
        exit(1);
      }
      if (db->fetchDouble("z_position_ReltoTPCLength") != 0.0) {
        cout << "FTD_Simple_Staggered:Stop: The z position of FTD1 is not relative. The relative value will not be "
                "used. It should be set to 0.0 in the DB."
             << endl;
        cout << "FTD_Simple_Staggered:Stop: The z position of FTD1 is set by the distance between the centre of the "
                "sensitive layer and the max z of VTX layer 3."
             << endl;
        exit(1);
      }
      break;
    ...
    case 2:
      ...
      InnerCylinderOuterRadius1 =
          _inner_radius - ((_zEnd - _glEnv.zEnd_IPOuterTube) * _glEnv.beamTubeTangent) - 0.5 * mm;

      // check that there is no overlap with SIT1
      if (_z_position <= _glEnv.SIT1_Half_Length_Z && _outer_radius >= _glEnv.SIT1_Radius) {
        cout << "FTD_Simple_Staggered:Stop:Overlap between FTD2 and SIT1" << endl;
        cout << "FTD_Simple_Staggered:FTD2 Radius = " << _outer_radius << "SIT1 Radius = " << _glEnv.SIT1_Radius
             << endl;
        exit(1);
      }
      break;
    ...
    case 3:
      ...
      if (_z_position <= _glEnv.SIT2_Half_Length_Z && _outer_radius >= _glEnv.SIT2_Radius) {
        ...
        cout << "FTD_Simple_Staggered:FTD3 Radius = " << _outer_radius << "SIT2 Radius = " << _glEnv.SIT2_Radius
             << endl;
        exit(1);
      }
      break;
    ...
    case 7:
      ...
      if (db->fetchDouble("z_position_ReltoTPCLength") != 0.0) {
        cout << "FTD_Simple_Staggered:Stop: The z position of FTD7 is not relative. The relative value will not be "
                "used. It should be set to 0.0 in the DB."
             << endl;
        cout << "FTD_Simple_Staggered:Stop: The z position of FTD7 is set by the distance between the centre of the "
                "sensitive layer and the min z of the ECal Endcap."
             << endl;
        exit(1);
      }
      break;
    ...
    cout << "FTD_Simple_Staggered: Disk:" << disk_number << "\t z = " << _z_position
         << "\t inner rad = " << _inner_radius << "\t outer rad = " << _outer_radius
         << "\t beamtube rad = " << _beamTubeRadius << "\t free space = "
         << (_inner_radius - 0.5 * mm - _dbParCommon.inner_cylinder_total_thickness -
             (2 * _dbParCommon.cable_shield_thickness) - _dbParCommon.cables_thickness) -
                _beamTubeRadius
         << endl;
    ...
    double petalairthickness_half = 0.5 * (_dbParDisk.petal_cp_support_thickness + 2.0 * _dbParDisk.disks_Si_thickness);
...
//     // Place negative copy
    ...
    for (int i = 0; i < petal_max_number; i++) {
      ...
      sspz << "ftd_petal_posZ_" << disk_number << "_" << i;
      std::stringstream ssnz;
      ssnz << "ftd_petal_negZ_" << disk_number << "_" << i;
    ...
    thisLayer.zOffsetSensitive =
        zSignPetal0 * (fabs(_dbParDisk.petal_support_zoffset) +
                       0.5 * (_dbParDisk.disks_Si_thickness + _dbParDisk.petal_cp_support_thickness));
    ...
    // surf0 is used for the first sensor and includes the complete support material - surf1 is used for the second
    ...
    for (int i = 0; i < petal_max_number; i++) {
      ...
      std::stringstream sspz;
      sspz << "ftd_sensor_posZ_" << disk_number << "_" << i << "_0";
      std::stringstream ssnz;
      ssnz << "ftd_sensor_negZ_" << disk_number << "_" << i << "_0";
      ...
      if (_dbParDisk.double_sided == 1) { // first two disks are single sided pixel
        ...
        std::stringstream sspz1;
        sspz1 << "ftd_sensor_posZ_" << disk_number << "_" << i << "_1";
        std::stringstream ssnz1;
        ssnz1 << "ftd_sensor_negZ_" << disk_number << "_" << i << "_1";
  ...
  Tube FTDOuterCylinderSolid(OuterCylinderInnerRadius,
                             ...
                             OuterCylinder_half_z, phi1, phi2);
  ...
  double InnerCylinderRmin1 = InnerCylinderOuterRadius1 - _dbParCommon.inner_cylinder_total_thickness -
                              (2.0 * _dbParCommon.cable_shield_thickness) - _dbParCommon.cables_thickness;
  double InnerCylinderRmax1 = InnerCylinderOuterRadius1;
  double InnerCylinderRmin2 = InnerCylinderOuterRadius2 - _dbParCommon.inner_cylinder_total_thickness -
                              (2.0 * _dbParCommon.cable_shield_thickness) - _dbParCommon.cables_thickness;
  double InnerCylinderRmax2 = InnerCylinderOuterRadius2;

  double cableShieldRmin1 = InnerCylinderRmin1;
  double cableShieldRmax1 =
      cableShieldRmin1 + (2.0 * _dbParCommon.cable_shield_thickness) + _dbParCommon.cables_thickness;
  double cableShieldRmin2 = InnerCylinderRmin2;
  double cableShieldRmax2 =
      cableShieldRmin2 + (2.0 * _dbParCommon.cable_shield_thickness) + _dbParCommon.cables_thickness;
  ...
  ConeSegment FTDInnerCylinderSolid(InnerCylinder_half_z, InnerCylinderRmin1, InnerCylinderRmax1, InnerCylinderRmin2,
                                    InnerCylinderRmax2, phi1, phi2);
  ...
  ConeSegment FTDCableShieldSolid(InnerCylinder_half_z, cableShieldRmin1, cableShieldRmax1, cableShieldRmin2,
                                  cableShieldRmax2, phi1, phi2);
  ...
  ConeSegment FTDCablesSolid(InnerCylinder_half_z, cablesRmin1, cablesRmax1, cablesRmin2, cablesRmax2, phi1, phi2);
...
void petalSupport(Detector& theDetector, DetElement ftd, std::map<std::string, double> valuesDict,
                  Volume FTDPetalAirLogical) {
  ...
  if (_dbParDisk.sensor_is_pixel == 1) {
    ...
    Volume FTDPetalSupportLogical(_toString(_dbParDisk.disk_number, "FTDPetalSupportLogical_%d"), FTDPetalSupportSolid,
                                  _CarbonFiberMat);
    ...
    Trap FTDPetalSupportCPSolid(_dbParDisk.petal_cp_support_thickness / 2.0, // thickness
                                ...
                                petal_cp_supp_half_dxMin, _dbParDisk.petal_cp_support_dxMax / 2.0, 0.0,
                                petal_cp_support_dy / 2.0, // height
                                petal_cp_supp_half_dxMin, _dbParDisk.petal_cp_support_dxMax / 2.0, 0.0);
    ...
    Trap FTDPetalSupportHoleDownSolid(2. * _dbParDisk.petal_cp_support_thickness / 2.0, // thickness
                                      ...
                                      petal_hole_down_dxMin / 2., petal_hole_down_dxMax / 2., 0.0,
                                      petal_hole_dy / 2.0, // height
                                      petal_hole_down_dxMin / 2., petal_hole_down_dxMax / 2., 0.0);
    ...
    Trap FTDPetalSupportHoleUpSolid(2. * _dbParDisk.petal_cp_support_thickness / 2.0, // thickness
                                    0.0, 0.0,
                                    petal_hole_dy / 2.0, // height
                                    petal_hole_up_dxMin / 2., petal_hole_up_dxMax / 2., 0.0,
                                    petal_hole_dy / 2.0, // height
                                    petal_hole_up_dxMin / 2., petal_hole_up_dxMax / 2., 0.0);
    ...
    std::cout << " *** support holes parameters : "
              << " petal_hole_dy " << petal_hole_dy << " petal_hole_down_dxMin " << petal_hole_down_dxMin
              << " petal_hole_down_dxMax " << petal_hole_down_dxMax << " petal_hole_up_dxMin " << petal_hole_up_dxMin
              << " petal_hole_up_dxMax " << petal_hole_up_dxMax << " y shift: " << (petal_hole_dy / 4. + spfw / 2.);
#endif
...
VolVec petalSensor(Detector& theDetector, DetElement ftd, SensitiveDetector sens,
  ...
  Volume FTDPetalSensitiveLogical(_toString(_dbParDisk.disk_number, "FTDPetalSensitiveLogical_%d"),
                                  FTDPetalSensitiveSolid, _SiMat);
  ...
  //            {
  //			rowTrans.setX( x_offset + pixel_si_length/2.0 + pixel_si_interspace + pixelId*pixel_si_length);
  ////pixel_si_offset 			for(int j = 0; j < numberRows; ++j)
  //                {
  //				rowTrans.setY( pixel_si_width*(1.0/2.0 + j) + pixel_si_interspace );
  ...
  //        //	double dz = _dbParDisk.petal_cp_support_thickness/2.0 + _dbParDisk.kapton_petal_thickness +
  //        _dbParDisk.disks_Si_thickness/2.0;
...
std::vector<double> GetPetalDimensions(const double& petal_cp_support_dy, const std::string& whereItgoes,
...
#ifdef DEBUG_VALUES
  cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ \n"
       << "    IsSilicon?: " << isSilicon << " " << whereItgoes << " xMin=" << 2. * xMin_half
       << " xMax=" << 2.0 * xMax_half << " dy= " << dy << std::endl;
  cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ \n" << endl;
#endif
...
void GearSetup() {
  ...
  for (unsigned int layer = 0; layer < _ftdparameters[gearpar::NPETALS].size(); layer++) {
    ...
    ftdParam->addLayer(nPetals, nSensors, isDoubleSided, sensorType, phalfangle, phi0, alpha, zposition, zoffset,
                       signoffset, suprtRin, suprtThic, suprtLMin, suprtLMax, suprtW, suprtRL, sensitRin, sensitThic,
                       sensitLMin, sensitLMax, sensitW, sensitRL);
  }
