  ...
  for (xml_coll_t mi(x_det, _U(stave)); mi; ++mi) {
    ...
    for (c_components.reset(); c_components; ++c_components) {
      ...
      for (c_component.reset(); c_component; ++c_component) {
        ...
        Volume ele_vol = Volume(components.name + _toString(iComponent, "_%d"), ele_box,
    ...
    for (c_component.reset(); c_component; ++c_component) {
      ...
      Box ele_box = Box(m.sensor_thickness / 2., abs(component.xmax() - component.xmin()) / 2.,
  ...
  for (xml_coll_t c(e, _U(layer)); c; ++c) {
    ...
    for (int iStave = 0; iStave < nLadders; ++iStave) {
      ...
      for (int iModule = 0; iModule < nmodules; iModule++) {
        ...
        double x_pos = r_component * cos(phi) - r_offset_component * sin(phi);
        double y_pos = r_component * sin(phi) + r_offset_component * cos(phi);
        double z_pos = -(nmodules - 1) / 2. * (m.sensor_length) - (nmodules - 1) / 2. * step +
        ...
        for (int i = 0; i < int(m.sensor_volumes.size()); i++) {
          ...
          if (m.sensor_sensitives[i]) {
            ...
            // VolPlane surf( sensitive_vol , SurfaceType(SurfaceType::Sensitive) , inner_thickness, outer_thickness,
            // u,v,n);

            DetElement sensorDE(sdet, sensor_name, x_det.id());
            PlacedVolume pv_sensor =
                module_assembly.placeVolume(m.sensor_volumes[i], Transform3D(rot, pos) * Translation3D(pos2));
