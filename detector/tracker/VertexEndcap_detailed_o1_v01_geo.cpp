...
static Ref_t create_detector(Detector& theDetector, xml_h e, SensitiveDetector sens) {
  ...
  for (xml_coll_t mi(x_det, _U(module)); mi; ++mi) {
    ...
    cout << "Module: " << m.name << ", sensor width: " << to_string(m.sensor_width)
         << ", sensor length: " << to_string(m.sensor_length) << endl;
        ...
        for (xml_coll_t ri(x_layer, _U(stave)); ri; ++ri) {
          ...
          for (auto& component : m.components_vec) {
            ...
            for (int i = 0; i < int(component.thicknesses.size()); i++) {
              ...
              double z_pos = z + z_alternate_petal + z_offset + component.z_offset + component.z_offsets[i] +
                             component.thicknesses[i] / 2.;
              if (side == -1) {
                z_pos = -z_pos;
              }
          ...
          for (auto& endOfStave : m.endOfStaves) {
            ...
            for (int i = 0; i < int(endOfStave.thicknesses.size()); i++) {
              ...
              double r_offset_component = endOfStave.xs[i] > 0
                                              ? stave_length / 2. + endOfStave.lengths[i] / 2. + endOfStave.dxs[i]
                                              : -(stave_length / 2. + endOfStave.lengths[i] / 2. + endOfStave.dxs[i]);
              double x_pos = r_component * cos(phi) - r_offset_component * sin(phi);
              double y_pos = r_component * sin(phi) + r_offset_component * cos(phi);
              double z_pos = z + z_alternate_petal + z_offset + endOfStave.z_offset + endOfStave.z_offsets[i] +
                             endOfStave.thicknesses[i] / 2.;
              if (side == -1) {
                z_pos = -z_pos;
              }
              Position pos(x_pos, y_pos, z_pos);
          ...
          for (int iModule = 0; iModule < nmodules; iModule++) {
            ...
            double r_offset_component = -(nmodules - 1) / 2. * (m.sensor_length) - (nmodules - 1) / 2. * step +
                                        iModule * m.sensor_length + iModule * step;
            double x_pos = r_component * cos(phi) - r_offset_component * sin(phi);
            double y_pos = r_component * sin(phi) + r_offset_component * cos(phi);
            double z_pos =
                z + z_alternate_petal + z_offset + m.sensor_z_offset + z_alternate_module + m.sensor_thickness / 2.;
            if (side == -1) {
              z_pos = -z_pos;
            }
            Position pos(x_pos, y_pos, z_pos);
            ...
            for (int i = 0; i < int(m.sensor_volumes.size()); i++) {
              ...
              if (m.sensor_sensitives[i]) {
                string sensor_name =
                    sensors_name + _toString(iModule, "_module%d") + _toString(iSensitive, "_sensor%d");
