...
//
//  v07 properly places det elements into layers for easier consumption as an
...
static Ref_t create_detector(Detector& theDetector, xml_h e, SensitiveDetector sens) {
  ...
  for (xml_coll_t mi(x_det, _U(module)); mi; ++mi) {
    ...
    for (xml_coll_t incl(x_mod, _U(include)); incl; ++incl) {
      ...
      for (ci.reset(), c_id = 0, posZ = -dz; ci; ++ci, ++c_id) {
        xml_comp_t c = ci;
...
DECLARE_DETELEMENT(TrackerEndcap_o2_v07, create_detector)
