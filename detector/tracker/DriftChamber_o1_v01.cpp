...
void CDCHBuild::PlaceGuardWires(struct wire& w, double outwrap, double halflength, int SL = 999, int ilayer = 999) {
  ...
  for (int n = 0; n < w.num; n++) {
    ...
    if (n % 1 == 0)
      w.mother_volume.placeVolume(w.volume, dd4hep::Transform3D(iRot * T));
  }
}
...
void CDCHBuild::build_layer(DetElement parent, Volume parentVol, dd4hep::SensitiveDetector sens_det) {
  ...
  //------------------------------------------------------------------------
  // The wireThicknessDilution parameter is used to see the wires in the rendering. NB: not propagated to the envelope
  // volumes --> you will have volume extrusions if != 1
  //------------------------------------------------------------------------
  ...
  // bool setWireSensitive = true; // FIXME: add the possibility to have wires sensitive (parameter in the xml) which
  // could be useful for detailed chamber behavior studies
  double halflength = zHalfExtentWithServices - (GasEndcapWallThick + CopperEndcapWallThick + KaptonEndcapWallThick +
                                                 CarbonEndcapWallThick); // this will be the sensitive volume z extent
  ...
  int nSWire = 0;
  int nHorizontalFtoSWireRatio =
      nFtoSWireRatio -
      nCenterFWirePerCell; // nHorizontalFtoSWireRatio is, per cell, how many wires do we have in top and bottom layer
                           // (summed), knowing that the rightmost wires are not in the current cell
  int sign_epsilon = -1;
  ...
  for (int SL = 0; SL < nSuperLayer; ++SL) {
    ...
    if (ceilf(nFWireTopOrBottom) != nFWireTopOrBottom)
      throw std::runtime_error(
          "Error: Failed to build CDCH. Please make sure that '(nFtoSWireRatio - nCenterFWirePerCell) * "
          "(nSWireFirstLayer + SuperLayerIndex * nSDeltaWire)' is always an even number");
    delta_phi_top_bottom = 2.0 * phi_offset;
    scaleFactor =
        (1.0 + TMath::Pi() / nSWire) /
        (1.0 - TMath::Pi() / nSWire); // used to scale the radial extent of each layer which grows towards outer radius
    dropFactor = (1.0 / cos(halfalpha) - 1.0); // used to determine the radius of the hyperboloid in z = +- halflength
                                               // with r_out = r_min + r_min * dropFactor
    ...
    if (SL == 0) { // SL = 0 is special due to the guard wires and the first field wires that lie outside of the
                   // sensitive volume
      double stereoOut0 = atan(sqrt(diff_of_squares((inGuardRad - inGWradii) + (inGuardRad - inGWradii) * dropFactor,
                                                    inGuardRad - inGWradii)) /
                               halflength);

      dd4hep::Hyperboloid HypeLayer0(guard_layer_inner_radius_at_z_0, 0.0, layer_outer_radius_at_z_0 - secure,
                                     stereoOut0, halflength);
      lvLayerVol.push_back(
          dd4hep::Volume("hyperboloid_inner_guard_layer", HypeLayer0, description.material("GasHe_90Isob_10")));
      ...
      guard_wires.thickness =
          0.5 * InGWireShellThickIn * wireThicknessDilution; // half the inner thickness as radius of tube
      ...
      dd4hep::Hyperboloid HypeLayer1(layer_inner_radius_at_z_0, epsilonIn,
                                     layer_outer_radius_at_z_0 + field_wires_top.thickness + 0.5 * FWireShellThickOut,
                                     epsilonOut, halflength);
      lvLayerVol.push_back(
          dd4hep::Volume("hyperboloid_first_field_wire_ring", HypeLayer1, description.material("GasHe_90Isob_10")));
      lvLayerVol.back().setVisAttributes(description, gascol);
    ...
    for (int ilayer = 0; ilayer < nLayer; ilayer++) {
      ...
      field_wires_bottom.name = dd4hep::_toString(SL, "Wire_SL%d") + dd4hep::_toString(ilayer, "_layer%d") +
                                string("_type") + field_wires_bottom.type +
                                dd4hep::_toString(field_wires_bottom.stereo, "_stereo%f_bottom");
      ...
      sense_wires.name = dd4hep::_toString(SL, "Wire_SL%d") + dd4hep::_toString(ilayer, "_layer%d") + string("_type") +
                         sense_wires.type + dd4hep::_toString(sense_wires.stereo, "_stereo%f");
      ...
      field_wires_center.name = dd4hep::_toString(SL, "Wire_SL%d") + dd4hep::_toString(ilayer, "_layer%d") +
                                string("_type") + field_wires_center.type +
                                dd4hep::_toString(field_wires_center.stereo, "_stereo%f_center");
      apply_wire_coating(field_wires_center, centerFWireShellThickOut, halflength);

      // derive a phi offset to englobe the 'left' (clockwise) wires inside the sensitive volume (center field wires are
      // the thickest)
      ...
      // Create hyperboloid volume of the whole layer for the cell sensitive volume definition, not needed per se but
      // helps having a well balanced volume tree (having too many volumes inside one mother volume harms performance)
      ...
      dd4hep::Hyperboloid whole_layer_hyperboloid = dd4hep::Hyperboloid(
          layer_inner_radius_at_z_end_whole_cell - radial_inner_offset_to_englobe_wires, epsilonIn_whole_cell,
          layer_outer_radius_at_z_0 + radial_outer_offset_to_englobe_wires, epsilonOut, zlength);
      dd4hep::Volume whole_layer_hyperboloid_volume =
          dd4hep::Volume(wholeHyperboloidVolumeName, whole_layer_hyperboloid, description.material("GasHe_90Isob_10"));
      ...
      dd4hep::DetElement whole_layer_hyperboloid_detElement(
          parent, "superLayer_" + dd4hep::_toString(SL) + "_layer_" + dd4hep::_toString(ilayer) + "_hyperboloid",
          SL * nLayer + ilayer);
      ...
      field_wires_top.name = dd4hep::_toString(SL, "Wire_SL%d") + dd4hep::_toString(ilayer, "_layer%d") +
                             string("_type") + field_wires_top.type +
                             dd4hep::_toString(field_wires_top.stereo, "_stereo%f_top");
      ...
      // Using the intersection with the hyperboloid mother volume solves it but severely impacts the perfomance (memory
      // and CPU) of the geometry building Other paths to investigate are twisted tubes (caveat: no TGeo shape
      // equivalent), extruded volumes or tessalated solids
      dd4hep::Tube cellID_tube_segment(layer_inner_radius_at_z_end_whole_cell - radial_inner_offset_to_englobe_wires,
                                       layer_outer_radius_at_z_0 + radial_outer_offset_to_englobe_wires, halflength,
                                       (-sense_wires.delta_phi / 2.0) - phi_offset_to_englobe_wires,
                                       (sense_wires.delta_phi / 2.0) - phi_offset_to_englobe_wires);
      ...
      if (ceilf(middle_to_middle_num_wire_ratio) != middle_to_middle_num_wire_ratio ||
          ceilf(middle_to_bottom_num_wire_ratio) != middle_to_bottom_num_wire_ratio ||
          ceilf(middle_to_top_num_wire_ratio) != middle_to_top_num_wire_ratio)
        throw std::runtime_error("Error: Failed to build CDCH. Please make sure that the number of wires in top/center "
                                 "cell rings is always a multiple of the number of wires in the middle of the cell");
      ...
      for (int phi_index = 0; phi_index < sense_wires.num; phi_index++) {
        ...
        string cellID_volume_name = dd4hep::_toString(SL, "cellIDvolume_SL_%d") +
                                    dd4hep::_toString(ilayer, "_layer_%d") + dd4hep::_toString(phi_index, "_phi_%d");
        dd4hep::Volume cellID_volume =
            dd4hep::Volume(cellID_volume_name, cellID_tube_segment, description.material("GasHe_90Isob_10"));
        ...
        dd4hep::PlacedVolume cellID_placedvolume = whole_layer_hyperboloid_volume.placeVolume(
            cellID_volume, dd4hep::Transform3D(rot_phi_sense_wire * rot_stereo_sense_wire));
        cellID_placedvolume.addPhysVolID("phi", phi_index)
            .addPhysVolID("hitorigin", 0)
            .addPhysVolID("stereo", sense_wires.stereo > 0 ? 0 : 1)
            .addPhysVolID("layerInCell", 0);
        dd4hep::DetElement cellID_detElement(whole_layer_hyperboloid_detElement,
                                             "superLayer_" + dd4hep::_toString(SL) + "_layer_" +
                                                 dd4hep::_toString(ilayer) + "_phi_" + dd4hep::_toString(phi_index) +
                                                 "_cellID",
                                             phi_index);
        ...
        dd4hep::PlacedVolume sense_wire_placedvolume = cellID_volume.placeVolume(
            sense_wires.volume,
            dd4hep::Transform3D(radial_translation_sense_wire)); // only the radial translation is needed as the cellID
                                                                 // volume already had the stereo and phi angle rotation
        // add the sense wire as detElement to be able to retrive its matrix with DD4hep 1.23 (with later verion we can
        // use Volumes daugthers)
        dd4hep::DetElement senseWire_detElement(cellID_detElement,
                                                "superLayer_" + dd4hep::_toString(SL) + "_layer_" +
                                                    dd4hep::_toString(ilayer) + "_phi_" + dd4hep::_toString(phi_index) +
                                                    "_wire",
                                                phi_index);
        ...
        // field wires: the cellID volume has the stereo angle rotation of the sense wires which is different than the
        // top/bototm wires --> Rotations are defined on top of the already applied cellID volume rotation bottom field
        // wires
        for (int sub_phi_index = phi_index * middle_to_bottom_num_wire_ratio;
             sub_phi_index < (phi_index * middle_to_bottom_num_wire_ratio) + middle_to_bottom_num_wire_ratio;
             sub_phi_index++) {
          // phi rotation
          dd4hep::RotationZ rot_phi_bottom_wire(
              (field_wires_bottom.phioffset + field_wires_bottom.delta_phi * sub_phi_index) -
              phi_angle_sense_wire_rotation);
          cellID_volume.placeVolume(
              field_wires_bottom.volume,
              dd4hep::Transform3D(rot_phi_bottom_wire * dd4hep::Translation3D(field_wires_bottom.radius, 0., 0.) *
                                  dd4hep::RotationX(field_wires_bottom.stereo - sense_wires.stereo)));
          // dd4hep::PlacedVolume field_wire_bottom_placedvolume = cellID_volume.placeVolume(field_wires_bottom.volume,
          // dd4hep::Transform3D(rot_phi_bottom_wire * dd4hep::Translation3D(field_wires_bottom.radius, 0., 0.) *
          // dd4hep::RotationX(field_wires_bottom.stereo - sense_wires.stereo))); if(setWireSensitive)
          //   field_wire_bottom_placedvolume.addPhysVolID("phi", sub_phi_index).addPhysVolID("hitorigin",
          //   2).addPhysVolID("stereo", field_wires_center.stereo > 0 ? 0 : 1).addPhysVolID("layerInCell", 1);
        }

        // central field wires
        for (int sub_phi_index = phi_index * middle_to_middle_num_wire_ratio;
             sub_phi_index < (phi_index * middle_to_middle_num_wire_ratio) + middle_to_middle_num_wire_ratio;
             sub_phi_index++) {
          // phi rotation
          dd4hep::RotationZ rot_phi_center_wire(
              (field_wires_center.phioffset + field_wires_center.delta_phi * sub_phi_index) -
              phi_angle_sense_wire_rotation);
          cellID_volume.placeVolume(
              field_wires_center.volume,
              dd4hep::Transform3D(rot_phi_center_wire * dd4hep::Translation3D(field_wires_center.radius, 0., 0.) *
                                  dd4hep::RotationX(field_wires_center.stereo - sense_wires.stereo)));
          // dd4hep::PlacedVolume field_wire_center_placedvolume = cellID_volume.placeVolume(field_wires_center.volume,
          // dd4hep::Transform3D(rot_phi_center_wire * dd4hep::Translation3D(field_wires_center.radius, 0., 0.) *
          // dd4hep::RotationX(field_wires_center.stereo - sense_wires.stereo))); if(setWireSensitive)
          //   field_wire_center_placedvolume.addPhysVolID("phi", sub_phi_index).addPhysVolID("hitorigin",
          //   2).addPhysVolID("stereo", field_wires_center.stereo > 0 ? 0 : 1).addPhysVolID("layerInCell", 2);
        }

        // top field wires
        for (int sub_phi_index = phi_index * middle_to_top_num_wire_ratio;
             sub_phi_index < (phi_index * middle_to_top_num_wire_ratio) + middle_to_top_num_wire_ratio;
             sub_phi_index++) {
          // phi rotation
          dd4hep::RotationZ rot_phi_top_wire((field_wires_top.phioffset + field_wires_top.delta_phi * sub_phi_index) -
                                             phi_angle_sense_wire_rotation);
          cellID_volume.placeVolume(
              field_wires_top.volume,
              dd4hep::Transform3D(rot_phi_top_wire * dd4hep::Translation3D(field_wires_top.radius, 0., 0.) *
                                  dd4hep::RotationX(field_wires_top.stereo - sense_wires.stereo)));
          // dd4hep::PlacedVolume field_wire_top_placedvolume = cellID_volume.placeVolume(field_wires_top.volume,
          // dd4hep::Transform3D(rot_phi_top_wire * dd4hep::Translation3D(field_wires_top.radius, 0., 0.) *
          // dd4hep::RotationX(field_wires_top.stereo - sense_wires.stereo))); if(setWireSensitive)
          //   field_wire_top_placedvolume.addPhysVolID("phi", sub_phi_index).addPhysVolID("hitorigin",
          //   2).addPhysVolID("stereo", field_wires_center.stereo > 0 ? 0 : 1).addPhysVolID("layerInCell", 3);
        }
      }
      ...
      epsilon = atan(iradius * (1.0 + dropFactor) *
                     epsilonFactor); // FIXME the stereo angle is here assumed to be constant for a given cell while it
                                     // should be different for the bottom, middle and top rings of the cell
    }

    if (SL == (nSuperLayer - 1)) { // the last super layer is special since we need to add the field wires outside of
                                   // the sensitive volume and the guard wires
      ...
      dd4hep::Hyperboloid HypeLayerOut(layer_inner_radius_at_z_0, epsilonIn, layer_outer_radius_at_z_0, epsilonOut,
                                       halflength);
      lvLayerVol.push_back(
          dd4hep::Volume("hyperboloid_last_field_wire_ring", HypeLayerOut, description.material("GasHe_90Isob_10")));
      lvLayerVol.back().setVisAttributes(description, gascol);
      ...
      dd4hep::Hyperboloid HypeLayerOutG(layer_inner_radius_at_z_0, epsilonOut,
                                        outer_radius - envelop_Outer_thickness - 0.0001, 0.0, halflength);
      lvLayerVol.push_back(
          dd4hep::Volume("hyperboloid_outer_guard_layer", HypeLayerOutG, description.material("GasHe_90Isob_10")));
      lvLayerVol.back().setVisAttributes(description, gascol);
...
static dd4hep::Ref_t create_element(dd4hep::Detector& description, xml_h e, dd4hep::SensitiveDetector sens_det) {
  ...
  dd4hep::Tube CDCH_envelope(dd4hep::_toDouble("CDCH:inner_radius"), dd4hep::_toDouble("CDCH:outer_radius"),
                             dd4hep::_toDouble("CDCH:zHalfExtentWithServices"));
