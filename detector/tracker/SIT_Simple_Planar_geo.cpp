...
static dd4hep::Ref_t create_element(dd4hep::Detector& theDetector, xml_h e, dd4hep::SensitiveDetector sens) {
  ...
  if (theDetector.buildType() == dd4hep::BUILD_ENVELOPE)
    return sit;
  ...
  // *********************
  //  Read and Store the Extended Reconstruction Parameters which are passed directly through to gear. Note others may
  //  be added below
  ...
  for (xml_coll_t c(x_det, _U(layer)); c; ++c) {
    ...
    std::cout << "SIT_Simple_Planar: Layer:" << layer_id << "\t half length = " << layer_geom.half_z
              ...
              << "\t min r support = " << layer_geom.support_inner_radius << "\t n ladders = " << layer_geom.n_ladders
              << "\t ladder width = " << layer_geom.ladder_width << "\t ladder clearance = " << ladder_clearance
              << "\t ladder dphi = " << ladder_dphi << "\t sensitive mat = " << sensitiveMat->GetName()
              << "\t support mat = " << supportMat->GetName() << "\t faces_IP = " << faces_IP
              << "\t is_SIT1 = " << is_SIT1 << "\t is_SIT2 = " << is_SIT2 << std::endl;
    ...
    dd4hep::Box sitLadderSolid((sensitive_thickness + support_thickness) / 2.0, layer_geom.ladder_width / 2.0,
                               layer_geom.half_z);
    ...
    dd4hep::Box sitSenEnvelopeSolid((sensitive_thickness) / 2.0, layer_geom.ladder_width / 2.0, layer_geom.half_z);

    // fixme: material ???    Volume sitSenEnvelopeLogical( _toString( layer_id,"SIT_SenEnvelopeLogical_%02d"),
    // sitSenEnvelopeSolid, sensitiveMat )  ;
    ...
    dd4hep::Box sitSenSolid((sensitive_thickness) / 2.0, layer_geom.ladder_width / 2.0,
                            (layer_geom.sensor_length / 2.0) -
                                1.e-06 * dd4hep::mm); // added tolerance to avoid false overlap detection
    ...
    dd4hep::rec::VolPlane surf(
        sitSenLogical,
        dd4hep::rec::SurfaceType(dd4hep::rec::SurfaceType::Sensitive, dd4hep::rec::SurfaceType::Measurement1D),
    ...
    pv = sitLadderLogical.placeVolume(
        sitSenEnvelopeLogical, Transform3D(RotationY(0.), Position((-(sensitive_thickness + support_thickness) / 2.0 +
                                                                    (sensitive_thickness / 2.0)),
                                                                   0., 0.)));
    // pv = sitSenEnvelopeLogical.placeVolume( sitLadderLogical, Transform3D( RotationY( 0.),
    // 									   Position( (-(sensitive_thickness
    // +support_thickness ) / 2.0 + ( sensitive_thickness / 2.0) ), 0.,0.) ) );

    // fixme: needed ??    pv.addPhysVolID("layer", layer_id ) ;

    // create support volume which will be placed in the enclosing ladder volume together with the senstive envelope
    // volume

    Box sitSupSolid((support_thickness) / 2.0, layer_geom.ladder_width / 2.0, layer_geom.half_z);
    ...
    pv = sitLadderLogical.placeVolume(
        sitSupLogical, Transform3D(RotationY(0.), Position((-(sensitive_thickness + support_thickness) / 2.0 +
                                                            sensitive_thickness + (support_thickness / 2.0)),
                                                           0., 0.)));

    for (int i = 0; i < n_ladders; ++i) {

      std::stringstream ladder_enum;
      ladder_enum << "sit_ladder_" << layer_id << "_" << i;

      DetElement ladderDE(layerDE, ladder_enum.str(), x_det.id());

      for (int isensor = 0; isensor < layer_geom.n_sensors_per_ladder; ++isensor) {

        std::stringstream sensor_ss;
        sensor_ss << ladder_enum.str() << "_" << isensor;
      ...
      pv = layer_assembly.placeVolume(
          sitLadderLogical,
          Transform3D(RotationZYX(phi_rot, 0., 0.), Position((sensitive_radius + dr) * cos(i * ladder_dphi),
                                                             (sensitive_radius + dr) * sin(i * ladder_dphi), 0.)));
