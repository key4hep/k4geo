...
static dd4hep::Ref_t create_element(dd4hep::Detector& theDetector, xml_h e, dd4hep::SensitiveDetector sens) {
  ...
  for (xml_coll_t c(x_det, _U(layer)); c; ++c) {
    ...
    std::cout << "SET_Simple_Planar: Layer:" << layer_id << "\t half length = " << layer_geom.half_z
              ...
              << "\t support mat = " << supportMat->GetName() << "\t faces_IP = " << faces_IP << std::endl;
    ...
    dd4hep::Box setLadderSolid((sensitive_thickness + support_thickness) / 2.0, layer_geom.ladder_width / 2.0,
                               layer_geom.half_z);
    ...
    dd4hep::Box setSenEnvelopeSolid((sensitive_thickness) / 2.0, layer_geom.ladder_width / 2.0, layer_geom.half_z);

    // fixme: material ???    Volume setSenEnvelopeLogical( _toString( layer_id,"SET_SenEnvelopeLogical_%02d"),
    // setSenEnvelopeSolid, sensitiveMat )  ;
    ...
    dd4hep::Box setSenSolid((sensitive_thickness) / 2.0, layer_geom.ladder_width / 2.0,
    ...
    VolPlane surf(setSenLogical, SurfaceType(SurfaceType::Sensitive, SurfaceType::Measurement1D), inner_thick,
                  outer_thick, u, v, n); //,o ) ;
    ...
    for (int isensor = 0; isensor < layer_geom.n_sensors_per_ladder; ++isensor) {
      ...
      pv = setSenEnvelopeLogical.placeVolume(
          setSenLogical, dd4hep::Transform3D(dd4hep::RotationY(0.), dd4hep::Position(xpos, ypos, zpos)));
    ...
    pv = setLadderLogical.placeVolume(
        setSenEnvelopeLogical,
        dd4hep::Transform3D(dd4hep::RotationY(0.), dd4hep::Position((-(sensitive_thickness + support_thickness) / 2.0 +
    ...
    dd4hep::Box setSupSolid((support_thickness) / 2.0, layer_geom.ladder_width / 2.0, layer_geom.half_z);
    ...
    for (int i = 0; i < n_ladders; ++i) {
      ...
      ladder_enum << "set_ladder_" << layer_id << "_" << i;
      ...
      pv = layer_assembly.placeVolume(
          setLadderLogical, dd4hep::Transform3D(dd4hep::RotationZYX(phi_rot, 0., 0.),
