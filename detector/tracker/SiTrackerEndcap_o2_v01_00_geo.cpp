...
//               D. Protopopescu, modified to allow more elements
//
//====================================================================
...
//                 pv.addPhysVolID("sensor",n_sensor); //Not what we call sensor; see below
    ...
    for(xml_coll_t li(x_det,_U(layer)); li; ++li)  {
		...
		std::cout << "SiTrackerEndcap barrel: 1, side: 0, layer: " << l_id << ", module(ring): " << mod_num << ", sensor: " << k << std::endl;
                module.setPlacement(pv);
                for(size_t ic=0; ic<sensVols.size(); ++ic)  {
                    PlacedVolume sens_pv = sensVols[ic];
                    DetElement comp_elt(module,sens_pv.volume().name(),k);
		    ...
		    std::cout << "SiTrackerEndcap barrel: 2, side: 1, layer: " << l_id << ", module(ring): " << mod_num << ", sensor: " << k << std::endl;
                    DetElement r_module(sdet,m_base+"_neg",det_id);
                    r_module.setPlacement(pv);
                    for(size_t ic=0; ic<sensVols.size(); ++ic)  {
                        PlacedVolume sens_pv = sensVols[ic];
                        DetElement comp_elt(r_module,sens_pv.volume().name(),k);
                        comp_elt.setPlacement(sens_pv);
                    }
                }
                dz   = -dz;
...
DECLARE_DETELEMENT(SiTrackerEndcap_o2_v01_00, create_detector)
