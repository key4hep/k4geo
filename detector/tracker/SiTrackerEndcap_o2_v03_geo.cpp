    ...
    for (xml_coll_t ri(x_layer, _U(ring)); ri; ++ri) {
      ...
      for (int k = 0; k < nmodules; ++k) {
        ...
        for (int s = 1 - 2 * int(reflect); s < 2; s += 1 + int(reflect)) {
          ...
          string e_name = _toString(s, "side%d") + _toString(l_id, "_layer%d") + _toString(ring_num, "_ring%d") +
          ...
          for (size_t ic = 0; ic < sensVols.size(); ++ic) {
            ...
            std::cout << "  ID: side " << s << ", layer " << ring_num << ", module " << mod_count[ring_num] + k
                      << ", sensor" << ic + 1 << std::endl;
          }
        }
        dz = -dz;
        phi += iphi;
