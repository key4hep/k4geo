    ...
    for(xml_coll_t li(x_det,_U(layer)); li; ++li)  {
        ...
        for(xml_coll_t ri(x_layer,_U(ring)); ri; ++ri)  {
            ...
            for(int k=0; k<nmodules; ++k) {
                ...
                if ( reflect ) {
                    pv = envelope.placeVolume(m_vol,Transform3D(RotationZYX(0,-M_PI/2-phi,-M_PI/2),Position(x,y,-zstart-dz*(nmodules-1-k))));
                    pv.addPhysVolID("side",-1).addPhysVolID("layer",l_id).addPhysVolID("module",mod_num).addPhysVolID("sensor",(nmodules-1-k));
...
DECLARE_DETELEMENT(VertexEndcap_o1_v06,create_detector)
