 ...
 * -increased realism in the description of the ladders, the Be support and the cabling, added cooling tubes Y.
 * Voutsinas, September 2011
 *
...
static Ref_t create_element(Detector& theDetector, xml_h e, SensitiveDetector sens) {
  ...
  for (xml_coll_t c(x_det, _U(layer)); c; ++c) {
    ...
    // 		ladder_length+(end_ladd_electronics_option*(2*end_electronics_half_z)) +
    // beryllium_ladder_block_length*2., 		flex_cable_thickness/2.);
    ...
    // calculate optimal offset, such that there is 0.1mm space between to the edge and the surface of two adjacent
    // ladders. in the case of ladders overlapped per superlayer
    /*
    ...
    */
    ...
    double offset_phi = (1 - cos(phirot)) / sin(phirot) * layer_radius -
                        ((ladder_width + (side_band_electronics_option * side_band_electronics_width / 2.)) +
                         (ladder_clothest_approch + cos(phirot) * 2 *
                                                        (active_silicon_thickness + flex_cable_thickness +
                                                         metal_traces_thickness - foam_spacer_thickness / 2.0)) /
                             sin(phirot));
    ...
    if (LayerId == 0 || LayerId == 2 ||
      ...
      for (double ladder_loop = 0; ladder_loop < nb_ladder; ladder_loop++) {
        ...
        supp_assembly.placeVolume(
            FlexCableLogical,
            Transform3D(rot,
                        Position((layer_radius + metal_traces_thickness + (flex_cable_thickness / 2.)) * sin(phirot2) +
                                     offset_phi * cos(phirot2),
                                 -(layer_radius + metal_traces_thickness + (flex_cable_thickness / 2.)) * cos(phirot2) +
                                     offset_phi * sin(phirot2),
                                 0.)));
        // Phys=
        //   new PVPlacement(rot,
        ...
        supp_assembly.placeVolume(
            ...
            Transform3D(
                rot,
                Position((layer_radius + flex_cable_thickness + metal_traces_thickness + foam_spacer_thickness / 2.) *
                                 sin(phirot2) +
                             offset_phi * cos(phirot2),
                         -(layer_radius + flex_cable_thickness + metal_traces_thickness + foam_spacer_thickness / 2.) *
                                 cos(phirot2) +
                             offset_phi * sin(phirot2),
                         0.)));
        ...
        supp_assembly.placeVolume(
            ...
            Transform3D(
                rot,
                Position((layer_radius + (metal_traces_thickness / 2)) * sin(phirot2) + offset_phi * cos(phirot2),
      ...
      for (double ladder_loop = 0; ladder_loop < nb_ladder; ladder_loop++) {
        ...
        supp_assembly.placeVolume(
            ...
            Transform3D(
                rot,
                Position(
                    (layer_radius - (metal_traces_thickness + flex_cable_thickness / 2.) + layer_gap) * sin(phirot2) +
                        offset_phi * cos(phirot2),
                    -(layer_radius - (metal_traces_thickness + flex_cable_thickness / 2.) + layer_gap) * cos(phirot2) +
                        offset_phi * sin(phirot2),
                    0.)));

        supp_assembly.placeVolume(FoamSpacerLogical,
                                  Transform3D(rot, Position((layer_radius + layer_gap - flex_cable_thickness -
                                                             metal_traces_thickness - foam_spacer_thickness / 2.) *
                                                                    sin(phirot2) +
                                                                offset_phi * cos(phirot2),
                                                            -(layer_radius + layer_gap - flex_cable_thickness -
                                                              metal_traces_thickness - foam_spacer_thickness / 2.) *
                                                                    cos(phirot2) +
                                                                offset_phi * sin(phirot2),
                                                            0.)));
        ...
        supp_assembly.placeVolume(
            MetalTracesLogical,
            Transform3D(rot, Position((layer_radius - (metal_traces_thickness / 2) + layer_gap) * sin(phirot2) +
                                          offset_phi * cos(phirot2),
                                      -(layer_radius - (metal_traces_thickness / 2.) + layer_gap) * cos(phirot2) +
                                          offset_phi * sin(phirot2),
                                      0.)));
      }
    }
    ...
    //     //Definition of the VXDSupport composite material. It is going to be used only during the reconstruction
    //     stage, for tracking purposes. It consists by three layers: metal traces, flex cable and the foam spacer
    //     support with user defined materials and thicknesses. Here we define the element and calculate its effective
    //     radiation length, atomic number and atomic mass. For the simulation, the more realistic 3 different layers
    ...
    //     double VXDSupportMass = foam_spacer_thickness*(elemVol)*FoamDensity +
    //     flex_cable_thickness*(elemVol)*KaptonDensity + metal_traces_thickness*(elemVol)*MetalDensity;
    ...
    //     double VXDSupportRadLen = 1. / ((metalTF/metalTracesMaterial->GetMaterial()->GetRadLen()) +
    //     (flexTF/flexCableMaterial->GetMaterial()->GetRadLen()) +
    //     (foamTF/foamSpacerMaterial->GetMaterial()->GetRadLen()));

    //     //Calculation of the effective atomic number of the VXD support. The Z effectives are obtained from the
    //     formula: Zeff = Sum(Wi*Zi) where Wi are the mass fractions of the elements that consist the material
    ...
    //     //Calculation of the effective atomic mass of the VXD support. The Z effectives are obtained from the
    //     formula: Aeff = Zeff / (Z/A)eff where (Z/A)eff = Sum Wi*Zi/Ai
    ...
    //     double VXDSupportIntLength = 1. / ((metalTF/metalTracesMaterial->GetNuclearInterLength()) +
    //     (flexTF/flexCableMaterial->GetNuclearInterLength()) + (foamTF/foamSpacerMaterial->GetNuclearInterLength()));
    ...
    //     VXDSupportMaterial = new gear::SimpleMaterialImpl("VXDSupportMaterial", VXDSupportAeff, VXDSupportZeff,
    //     VXDSupportDensity, VXDSupportRadLen, VXDSupportIntLength );
    ...
    //     // find out type
    //     if( side_band_electronics_option == 0 &&  end_ladd_electronics_option == 1) gearHelpType =
    //     gear::ZPlanarParametersImpl::CCD  ; if( side_band_electronics_option == 1 &&  end_ladd_electronics_option ==
    //     0 ) gearHelpType = gear::ZPlanarParametersImpl::CMOS ; if( side_band_electronics_option == 1 &&
    //     end_ladd_electronics_option == 1) gearHelpType = gear::ZPlanarParametersImpl::HYBRID ;
    ...
    if (LayerId == 1) {
      ...
      Volume BerylliumAnnulusBlockLogical(_toString(LayerId, "BerylliumAnnulusBlock_%02d"), BerylliumAnnulusBlockSolid,
                                          theDetector.material("G4_Be")); //"beryllium") ) ;
      ...
      VolPlane surfAnnBlock(BerylliumAnnulusBlockLogical, SurfaceType(SurfaceType::Helper),
                            beryllium_ladder_block_thickness / 2., beryllium_ladder_block_thickness / 2., u, v,
                            n); //,o ) ;
      //============================================================

      for (double AnnulusBlock_loop = 0; AnnulusBlock_loop < nb_ladder; AnnulusBlock_loop++) {

        std::string annBlockNameP =
            _toString(LayerId, "BerylliumAnnulusBlock_%02d_posZ") + _toString((int)AnnulusBlock_loop, "_%02d");
        std::string annBlockNameN =
            _toString(LayerId, "BerylliumAnnulusBlock_%02d_negZ") + _toString((int)AnnulusBlock_loop, "_%02d");
        ...
        PlacedVolume pv_ann_pos = supp_assembly.placeVolume(
            BerylliumAnnulusBlockLogical,
            Transform3D(rot, Position((layer_radius + beryllium_ladder_block_thickness + layer_gap) * sin(phirot2) +
                                          offset_phi * cos(phirot2),
                                      -(layer_radius + beryllium_ladder_block_thickness + layer_gap) * cos(phirot2) +
                                          offset_phi * sin(phirot2),
                                      ZAnnulusBlock)));
        ...
        PlacedVolume pv_ann_neg = supp_assembly.placeVolume(
            BerylliumAnnulusBlockLogical,
            Transform3D(rot, Position((layer_radius + beryllium_ladder_block_thickness + layer_gap) * sin(phirot2) +
                                          offset_phi * cos(phirot2),
                                      -(layer_radius + beryllium_ladder_block_thickness + layer_gap) * cos(phirot2) +
      ...
      beryllium_ladder_block_length2 =
          beryllium_ladder_block_length +
          (shell_half_z - (end_electronics_half_z * 3. * end_ladd_electronics_option) - ladder_length);

      for (double AnnulusBlock_loop = 0; AnnulusBlock_loop < nb_ladder; AnnulusBlock_loop++) {

        Box BerylliumAnnulusBlockSolid(ladder_width, beryllium_ladder_block_length2 / 2.,
        ...
        Volume BerylliumAnnulusBlockLogical(volName, BerylliumAnnulusBlockSolid,
                                            theDetector.material("G4_Be")); //"beryllium") ) ;
    ...
    if (end_ladd_electronics_option == 1) {
      ...
      Volume ElectronicsEndLogical(_toString(LayerId, "ElectronicsEnd_%02d"), ElectronicsEndSolid,
                                   activeMaterial); //("silicon_2.33gccm")

      vxd.setVisAttributes(theDetector, "GreenVis", ElectronicsEndLogical);

      double end_ladd_electronic_offset_phi =
          offset_phi + (side_band_electronics_option * side_band_electronics_width / 2.);
      ...
      double end_ladd_elec_thick =
          metal_traces_thickness + flex_cable_thickness + foam_spacer_thickness + electronics_structure_thickness;

      VolPlane surfEndElec(ElectronicsEndLogical, SurfaceType(SurfaceType::Helper), end_ladd_elec_thick / 2.,
                           end_ladd_elec_thick / 2., u, v, n); //,o ) ;
      //============================================================
      ...
      if (LayerId == 1 || LayerId == 3 || LayerId == 5) {
        ...
        for (double elec_loop = 0; elec_loop < nb_ladder; elec_loop++) {
          ...
          std::string elecEndLadNameP =
              _toString(LayerId, "ElectronicsEnd_%02d_posZ") + _toString((int)elec_loop, "_%02d");
          std::string elecEndLadNameN =
              _toString(LayerId, "ElectronicsEnd_%02d_negZ") + _toString((int)elec_loop, "_%02d");
          ...
          PlacedVolume pv_el_end_pos = layer_assembly.placeVolume(
              ElectronicsEndLogical,
              Transform3D(rot,
                          Position((layer_radius + (electronics_structure_thickness / 2.) + layer_gap) * sin(phirot2) +
                                       end_ladd_electronic_offset_phi * cos(phirot2),
                                   -(layer_radius + (electronics_structure_thickness / 2.) + layer_gap) * cos(phirot2) +
                                       end_ladd_electronic_offset_phi * sin(phirot2),
                                   Z)));
          ...
          PlacedVolume pv_el_end_neg = layer_assembly.placeVolume(
              ElectronicsEndLogical,
              Transform3D(rot,
                          Position((layer_radius + (electronics_structure_thickness / 2.) + layer_gap) * sin(phirot2) +
                                       end_ladd_electronic_offset_phi * cos(phirot2),
        ...
        for (double elec_loop = 0; elec_loop < nb_ladder; elec_loop++) {
          ...
          PlacedVolume pv_el_end_pos = layer_assembly.placeVolume(
              ElectronicsEndLogical,
              Transform3D(rot, Position((layer_radius - (electronics_structure_thickness / 2.)) * sin(phirot2) +
                                            end_ladd_electronic_offset_phi * cos(phirot2),
                                        -(layer_radius - (electronics_structure_thickness / 2.)) * cos(phirot2) +
                                            end_ladd_electronic_offset_phi * sin(phirot2),
                                        Z)));
    ...
    if (side_band_electronics_option == 1) {
      ...
      Box ElectronicsBandSolid(side_band_electronics_width / 2., ladder_length / 2.,
      ...
      if (LayerId == 1 || LayerId == 3 || LayerId == 5) {
        ...
        for (double elec_loop = 0; elec_loop < nb_ladder; elec_loop++) {
          ...
          PlacedVolume pv_el_band_pos = layer_assembly.placeVolume(
              ElectronicsBandLogical,
              Transform3D(rot,
                          Position((layer_radius + (side_band_electronics_thickness / 2.) + layer_gap) * sin(phirot2) +
                                       side_band_electronic_offset_phi * cos(phirot2),
                                   -(layer_radius + (side_band_electronics_thickness / 2.) + layer_gap) * cos(phirot2) +
                                       side_band_electronic_offset_phi * sin(phirot2),
                                   Z)));

          //**fg: choose sensor 1 for sensitive electronics side band
          if (active_side_band_electronics_option == 1)
            pv_el_band_pos.addPhysVolID("layer", LayerId)
                .addPhysVolID("module", int(elec_loop))
                .addPhysVolID("sensor", 1)
                .addPhysVolID("side", 1);

          PlacedVolume pv_el_band_neg = layer_assembly.placeVolume(
              ElectronicsBandLogical,
              Transform3D(rot,
                          Position((layer_radius + (side_band_electronics_thickness / 2.) + layer_gap) * sin(phirot2) +
                                       side_band_electronic_offset_phi * cos(phirot2),
          ...
          if (active_side_band_electronics_option == 1)
            pv_el_band_neg.addPhysVolID("layer", LayerId)
                .addPhysVolID("module", int(elec_loop))
                .addPhysVolID("sensor", 1)
                .addPhysVolID("side", -1);
        }
        ...
        for (double elec_loop = 0; elec_loop < nb_ladder; elec_loop++) {
          ...
          PlacedVolume pv_el_band_pos = layer_assembly.placeVolume(
              ElectronicsBandLogical,
              Transform3D(rot, Position((layer_radius - (side_band_electronics_thickness / 2.)) * sin(phirot2) +
                                            side_band_electronic_offset_phi * cos(phirot2),
                                        -(layer_radius - (side_band_electronics_thickness / 2.)) * cos(phirot2) +
                                            side_band_electronic_offset_phi * sin(phirot2),
                                        Z)));
    ...
    if (LayerId == 0 || LayerId == 1) {
      strip_line_start_z = ladder_length + ladder_gap / 2. + (end_electronics_half_z * 2.) + shell_thickess +
                           beryllium_ladder_block_length * 2; // to avoid overlaps
    } else {
      strip_line_start_z =
          shell_half_z + shell_endplate_thickness; // ladder_length + ladder_gap/2. - end_electronics_half_z * 2.+
    ...
    if (LayerId == 0 || LayerId == 2 || LayerId == 4) {
      ...
      ConeSegment KaptonLinesSolid(
          ...
          cryostat_apperture + LayerId * final_kapton_striplines_thickness + final_kapton_striplines_thickness, sPhi,
          sPhi + dPhi);
      ...
      ConeSegment MetalLinesSolid(strip_line_half_z,
                                  ...
                                  layer_radius + initial_kapton_striplines_thickness +
                                      initial_metal_striplines_thickness, // outside radius at -fDz
                                  cryostat_apperture + LayerId * final_kapton_striplines_thickness +
                                      final_kapton_striplines_thickness, // inside radius at  +fDz
                                  cryostat_apperture + LayerId * final_kapton_striplines_thickness +
                                      final_kapton_striplines_thickness + final_metal_striplines_thickness,
                                  sPhi, sPhi + dPhi);
      ...
      if (LayerId == 2 || LayerId == 4) {
        ...
        // const double kapton_theta = atan2( -1.*(cryostat_apperture + LayerId*final_kapton_striplines_thickness +
        // final_kapton_striplines_thickness) + layer_radius , 2.* strip_line_half_z ) ; const double metal_theta =
        // atan2( -1.*(cryostat_apperture + LayerId*final_kapton_striplines_thickness +
        // final_kapton_striplines_thickness + final_metal_striplines_thickness) + (layer_radius +
        // initial_kapton_striplines_thickness), 2.* strip_line_half_z ) ;

        const double kapton_theta = atan2(-1. * (cryostat_apperture + LayerId * final_kapton_striplines_thickness +
                                                 final_kapton_striplines_thickness) +
                                              layer_radius,
                                          2. * strip_line_half_z);
        const double metal_theta = atan2(-1. * (cryostat_apperture + LayerId * final_kapton_striplines_thickness +
                                                final_kapton_striplines_thickness + final_metal_striplines_thickness) +
                                             (layer_radius + initial_kapton_striplines_thickness),
                                         2. * strip_line_half_z);

        double cabling_kapton_thickness =
            (final_kapton_striplines_thickness + initial_kapton_striplines_thickness) / 2.;
        double cabling_metal_thickness = (final_metal_striplines_thickness + initial_metal_striplines_thickness) / 2.;

        std::cout << " layer " << LayerId << " length of the cone " << 2. * strip_line_half_z << " opening angle "
                  << kapton_theta << std::endl;

        Vector3D o_kaptoncon(0.5 * (layer_radius + cryostat_apperture + LayerId * final_kapton_striplines_thickness +
                                    final_kapton_striplines_thickness),
                             0., 0.);
        Vector3D o_metalcon(0.5 * (layer_radius + initial_kapton_striplines_thickness + cryostat_apperture +
                                   LayerId * final_kapton_striplines_thickness + final_kapton_striplines_thickness +
                                   final_metal_striplines_thickness),
                            0., 0.);

        Vector3D metal_angle(1., 0., metal_theta, Vector3D::spherical);
        Vector3D kapton_angle(1., 0., kapton_theta, Vector3D::spherical);

        VolCone conSurf1(KaptonLinesLogical, SurfaceType(SurfaceType::Helper), 0.5 * cabling_kapton_thickness,
                         0.5 * cabling_kapton_thickness, kapton_angle, o_kaptoncon);
        VolCone conSurf2(MetalLinesLogical, SurfaceType(SurfaceType::Helper), 0.5 * cabling_metal_thickness,
                         0.5 * cabling_metal_thickness, metal_angle, o_metalcon);

        PlacedVolume pv_kap_pos =
            supp_assembly.placeVolume(KaptonLinesLogical, Transform3D(RotationZYX(), Position(0., 0., Z)));
        ...
        PlacedVolume pv_met_pos =
            supp_assembly.placeVolume(MetalLinesLogical, Transform3D(RotationZYX(), Position(0., 0., Z)));
        ...
        const double kapton_theta = atan2(
            (cryostat_apperture + LayerId * final_kapton_striplines_thickness + final_kapton_striplines_thickness) -
                layer_radius,
            2. * strip_line_half_z);
        const double metal_theta = atan2((cryostat_apperture + LayerId * final_kapton_striplines_thickness +
                                          final_kapton_striplines_thickness + final_metal_striplines_thickness) -
                                             (layer_radius + initial_kapton_striplines_thickness),
                                         2. * strip_line_half_z);
        ...
        std::cout << " kapton:layer " << LayerId << " v phi " << kapton_angle.phi() << " o phi " << o_kaptoncon.phi()
                  << std::endl;
        std::cout << " metal:layer " << LayerId << " v phi " << metal_angle.phi() << " o phi " << o_metalcon.phi()
                  << std::endl;
    ...
    double ZEndPlateCoolPipesL1 = ladder_length + ((end_electronics_half_z * end_ladd_electronics_option) * 2) +
                                  shell_thickess + (beryllium_ladder_block_length * 2);
    ...
    Volume CoolPipeLogical(_toString(LayerId, "CoolPipe_%02d"), CoolPipeSolid,
                           theDetector.material("G4_Ti")); // titanium") ) ;
    ...
    if (LayerId == 3 || LayerId == 5) {
      ...
      supp_assembly.placeVolume(
          CoolPipeLogical, Transform3D(RotationZYX(), Position(0., 0., ZEndPlateCoolPipes + cool_pipe_outer_radius)));
      supp_assembly.placeVolume(
          CoolPipeLogical,
          Transform3D(RotationZYX(), Position(0., 0., -(ZEndPlateCoolPipes + cool_pipe_outer_radius))));

    } else if (LayerId == 1) {

      supp_assembly.placeVolume(
          CoolPipeLogical,
          Transform3D(RotationZYX(), Position(0., 0., ZEndPlateCoolPipesL1 + cool_pipe_outer_radius + shell_thickess)));
      supp_assembly.placeVolume(
          CoolPipeLogical,
          Transform3D(RotationZYX(),
                      Position(0., 0., -(ZEndPlateCoolPipesL1 + cool_pipe_outer_radius + shell_thickess))));
    }
    ...
    if (LayerId == 1) {
      ...
      double thetaTube =
          atan((support_endplate_inner_radious - (layer_radius + layer_gap + 2 * cool_pipe_outer_radius)) /
               (shell_half_z - ZEndPlateCoolPipesL1));
      ...
      Volume CoolPipeTubeLogical(_toString(LayerId, "CoolPipeTube_%02d"), CoolPipeTubeSolid,
                                 theDetector.material("G4_Ti")); // titanium") ) ;
      ...
      supp_assembly.placeVolume(
          CoolPipeTubeLogical,
          Transform3D(rm, Position(0., (layer_radius + layer_gap + support_endplate_inner_radious) / 2.,
                                   (ZEndPlateCoolPipesL1 + 3 * cool_pipe_outer_radius + CoolPipeLength / 2.))));

      supp_assembly.placeVolume(
          CoolPipeTubeLogical,
          Transform3D(rm2, Position(0., -(layer_radius + layer_gap + support_endplate_inner_radious) / 2.,
                                    (ZEndPlateCoolPipesL1 + 3 * cool_pipe_outer_radius + CoolPipeLength / 2.))));
    ...
    // ...
    VolPlane surf(SiActiveLayerLogical, SurfaceType(SurfaceType::Sensitive), active_silicon_thickness / 2 + supp_thick,
                  active_silicon_thickness / 2 + supp_thick, u, v, n); //,o ) ;
    //============================================================
    ...
    for (double active_loop = 0; active_loop < nb_ladder; active_loop++) {
      ...
      if (LayerId == 1) {
        ...
        std::cout << "  ***** place ladder " << int(active_loop) << " for " << _toString(LayerId, "SiActiveLayer_%02d")
                  << std::endl;
      }

      if (LayerId == 1 || LayerId == 3 || LayerId == 5) {

        PlacedVolume pv_layer_pos = layer_assembly.placeVolume(
            SiActiveLayerLogical,
            Transform3D(rot, Position((layer_radius + (active_silicon_thickness / 2.) + layer_gap) * sin(phirot2) +
                                          active_offset_phi * cos(phirot2),
                                      -(layer_radius + (active_silicon_thickness / 2.) + layer_gap) * cos(phirot2) +
                                          active_offset_phi * sin(phirot2),
                                      Z)));

        pv_layer_pos.addPhysVolID("layer", LayerId)
            .addPhysVolID("module", int(active_loop))
        ...
        PlacedVolume pv_layer_neg = layer_assembly.placeVolume(
            SiActiveLayerLogical,
            Transform3D(rot, Position((layer_radius + (active_silicon_thickness / 2.) + layer_gap) * sin(phirot2) +
                                          active_offset_phi * cos(phirot2),
                                      -(layer_radius + (active_silicon_thickness / 2.) + layer_gap) * cos(phirot2) +
        ...
        pv_layer_neg.addPhysVolID("layer", LayerId)
            .addPhysVolID("module", int(active_loop))
            .addPhysVolID("sensor", 0)
            .addPhysVolID("side", -1);
        ...
        PlacedVolume pv_layer_pos = layer_assembly.placeVolume(
            SiActiveLayerLogical,
            Transform3D(rot, Position((layer_radius - (active_silicon_thickness / 2.)) * sin(phirot2) +
                                          active_offset_phi * cos(phirot2),
                                      -(layer_radius - (active_silicon_thickness / 2.)) * cos(phirot2) +
                                          active_offset_phi * sin(phirot2),
                                      Z)));
  ...
  // kapton part
  Tube ExternalKaptonCablesSolid(cryostat_apperture, cryostat_apperture + 3 * external_kapton_thickness / 2.,
                                 external_cable_length, sPhi, sPhi + dPhi);
  // The reason for the factor three is that the thickness refer to the thickness of each single cable, and we have
  // three cables in total, one per layer

  Volume ExternalKaptonCablesLogical("ExternalKaptonCables", ExternalKaptonCablesSolid,
                                     theDetector.material("G4_KAPTON"));

  vxd.setVisAttributes(theDetector, "WhiteVis", ExternalKaptonCablesLogical);

  // metal part
  Tube ExternalMetalCablesSolid(cryostat_apperture - 3 * external_metal_thickness / 2., cryostat_apperture,
                                external_cable_length, sPhi, sPhi + dPhi);
  ...
  VolCylinder surfKaptonExtCables(ExternalKaptonCablesLogical, SurfaceType(SurfaceType::Helper),
                                  external_kapton_thickness / 2., external_kapton_thickness / 2., oextcable);

  //========================================== Metal part ==================================

  Vector3D oextmetalcable(cryostat_apperture - 3 * external_metal_thickness / 2. + cryostat_apperture, 0., 0.);
  VolCylinder surfMetalExtCables(ExternalMetalCablesLogical, SurfaceType(SurfaceType::Helper),
                                 external_metal_thickness / 2., external_metal_thickness / 2., oextmetalcable);

  //==================================================================================

  PlacedVolume pv_kap_pos = supp_assembly.placeVolume(ExternalKaptonCablesLogical,
                                                      Transform3D(RotationZYX(), Position(0., 0., ExternalCablesZ)));
  suppExtKaptonCablesPos.setPlacement(pv_kap_pos);
  volSurfaceList(suppExtKaptonCablesPos)->push_back(surfKaptonExtCables);

  PlacedVolume pv_kap_neg = supp_assembly.placeVolume(ExternalKaptonCablesLogical,
                                                      Transform3D(RotationZYX(), Position(0., 0., -ExternalCablesZ)));
  suppExtKaptonCablesNeg.setPlacement(pv_kap_neg);
  volSurfaceList(suppExtKaptonCablesNeg)->push_back(surfKaptonExtCables);

  PlacedVolume pv_met_pos = supp_assembly.placeVolume(ExternalMetalCablesLogical,
                                                      Transform3D(RotationZYX(), Position(0., 0., ExternalCablesZ)));
  suppExtMetalCablesPos.setPlacement(pv_met_pos);
  volSurfaceList(suppExtMetalCablesPos)->push_back(surfMetalExtCables);

  PlacedVolume pv_met_neg = supp_assembly.placeVolume(ExternalMetalCablesLogical,
                                                      Transform3D(RotationZYX(), Position(0., 0., -ExternalCablesZ)));
  ...
  VolCylinder surfSupShell(SupportShellLogical, SurfaceType(SurfaceType::Helper), shell_thickess / 2.,
                           shell_thickess / 2., osupshell);
  ...
  Tube EndPlateShellSolid(support_endplate_inner_radious, shell_inner_radious + shell_thickess, support_endplate_half_z,
                          sPhi, sPhi + dPhi);

  Volume EndPlateShellLogical("EndPlateShell_outer", EndPlateShellSolid,
                              theDetector.material("G4_Be")); //"beryllium") ) ;
  ...
  PlacedVolume pv_end_pos =
      supp_assembly.placeVolume(EndPlateShellLogical, Transform3D(RotationZYX(), Position(0., 0., ZEndPlateShell)));
  endplateFwdDE.setPlacement(pv_end_pos);
  PlacedVolume pv_end_neg =
      supp_assembly.placeVolume(EndPlateShellLogical, Transform3D(RotationZYX(), Position(0., 0., -ZEndPlateShell)));
  ...
  // need to set the origin of this helper plane to be inside the material ( otherwise it would pick up the vacuum at
  // the origin)
  Vector3D o_endplate(0., 0.5 * (support_endplate_inner_radious + shell_inner_radious + shell_thickess), 0.);

  VolPlane surfEndplate(EndPlateShellLogical, SurfaceType(SurfaceType::Helper), support_endplate_half_z,
                        support_endplate_half_z, up, vp, np, o_endplate);
  ...
  Tube EndPlateShellSolidL1(support_endplate_inner_radious_L1, support_endplate_outer_radious_L1,
                            support_endplate_half_z_L1, sPhi, sPhi + dPhi);

  Volume EndPlateShellLogicalL1("EndPlateShell_inner", EndPlateShellSolidL1,
                                theDetector.material("G4_Be")); //"beryllium") ) ;

  vxd.setVisAttributes(theDetector, "CyanVis", EndPlateShellLogicalL1);

  double ZEndPlateShell2 = ladder_length + ((end_electronics_half_z * end_ladd_electronics_option) * 2) +
                           shell_thickess / 2. + (beryllium_ladder_block_length * 2);
  ...
  ConeSegment SupportShellCone(support_cone_half_z, support_endplate_outer_radious_L1,
                               support_endplate_outer_radious_L1 + shell_thickess, support_endplate_inner_radious,
                               support_endplate_inner_radious + shell_thickess, sPhi, sPhi + dPhi);
  ...
  double ZCone = ladder_length + ((end_electronics_half_z * end_ladd_electronics_option) * 2) + shell_thickess +
                 (beryllium_ladder_block_length * 2) + support_cone_half_z;

  // ================================== adding tracking surfaces for beryllium support cone
  // ====================================
  const double shell_theta =
      atan2(-1. * support_endplate_outer_radious_L1 + support_endplate_inner_radious, 2. * support_cone_half_z);

  Vector3D o_shellcon(0.5 * (support_endplate_outer_radious_L1 + shell_thickess + support_endplate_inner_radious), 0.,
                      0.);
  Vector3D shellcone_angle(1., 0., shell_theta, Vector3D::spherical);

  VolCone suppShellCone(SupportConeLogical, SurfaceType(SurfaceType::Helper), 0.5 * shell_thickess,
                        0.5 * shell_thickess, shellcone_angle, o_shellcon);
  // ==============================================================================================================================

  PlacedVolume pv_supp_pos =
      supp_assembly.placeVolume(SupportConeLogical, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., ZCone)));
  ...
  PlacedVolume pv_supp_neg =
      supp_assembly.placeVolume(SupportConeLogical, Transform3D(RotationZYX(0, 0, M_PI), Position(0., 0., -ZCone)));
  ...
  Tube SupportForSolid(support_endplate_inner_radious, support_endplate_inner_radious + shell_endplate_thickness,
                       forward_shell_half_z, sPhi, sPhi + dPhi);
  ...
  if (useCryo) {
    ...
    PlacedVolume pv_alu_end_pos =
        supp_assembly.placeVolume(aluEndcapInnerLog, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., aluEndcapZ)));
    suppFwdInDE.setPlacement(pv_alu_end_pos);
    PlacedVolume pv_alu_end_neg =
        supp_assembly.placeVolume(aluEndcapInnerLog, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., -aluEndcapZ)));
    suppBwdInDE.setPlacement(pv_alu_end_neg);

    Tube aluEndcapSolidOuter(cryostat_apperture + cryostat_apperture_radius, rAlu + drAlu, drAlu / 2, sPhi,
                             sPhi + dPhi);
    ...
    PlacedVolume pv_alu_end_out_pos =
        supp_assembly.placeVolume(aluEndcapOuterLog, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., aluEndcapZ)));
    suppFwdOutDE.setPlacement(pv_alu_end_out_pos);
    PlacedVolume pv_alu_end_out_neg =
        supp_assembly.placeVolume(aluEndcapOuterLog, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., -aluEndcapZ)));
    ...
    Tube styEndcapSolidOuter(cryostat_apperture + cryostat_apperture_radius, rSty + drSty, drSty / 2, sPhi,
                             sPhi + dPhi);
