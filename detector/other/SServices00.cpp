...
static Ref_t create_element(Detector& theDetector, xml_h element, Ref_t) {
  ...
  if (build_ecal_services) {
    ...
    double Ecal_inner_radius =
        theDetector.constant<double>("TPC_outer_radius") + theDetector.constant<double>("Ecal_Tpc_gap");
    double module_thickness = Ecal_outer_radius - Ecal_inner_radius;

    // module barrel key parameters
    double bottom_dim_x = 2. * tan(M_PI / 8.) * Ecal_inner_radius + module_thickness / sin(M_PI / 4.);
    ...
    double ZMinus_FirstInterrail_PE_Thickness =
        theDetector.constant<double>("EcalBarrelServices_ZMinus_FirstInterrail_PE_Thickness");
    double ZMinus_FirstInterrail_Cu_Thickness =
        theDetector.constant<double>("EcalBarrelServices_ZMinus_FirstInterrail_Cu_Thickness");

    double ZPlus_FirstInterrail_PE_Thickness =
        theDetector.constant<double>("EcalBarrelServices_ZPlus_FirstInterrail_PE_Thickness");
    double ZPlus_FirstInterrail_Cu_Thickness =
        theDetector.constant<double>("EcalBarrelServices_ZPlus_FirstInterrail_Cu_Thickness");
  ...
  if (build_hcal_services) {
    ...
    HcalBarrel_EndCapServices.setBuildHcalElectronicsInterface(
    ...
    //-Z thicknesses
    HcalBarrel_EndCapServices.setZMinus_StainlessSteel_Thickness(
        theDetector.constant<double>("HcalServicesModule_ZMinus_StainlessSteel_Thickness"));
    HcalBarrel_EndCapServices.setZMinus_PE_Thickness(
        theDetector.constant<double>("HcalServicesModule_ZMinus_PE_Thickness"));
    HcalBarrel_EndCapServices.setZMinus_Cu_Thickness(
        theDetector.constant<double>("HcalServicesModule_ZMinus_Cu_Thickness"));
    //+Z thicknesses
    HcalBarrel_EndCapServices.setZPlus_StainlessSteel_Thickness(
        theDetector.constant<double>("HcalServicesModule_ZPlus_StainlessSteel_Thickness"));
    HcalBarrel_EndCapServices.setZPlus_PE_Thickness(
        theDetector.constant<double>("HcalServicesModule_ZPlus_PE_Thickness"));
    HcalBarrel_EndCapServices.setZPlus_Cu_Thickness(
        theDetector.constant<double>("HcalServicesModule_ZPlus_Cu_Thickness"));

    HcalBarrel_EndCapServices.setInnerServicesWidth(
    ...
    HcalBarrel_EndCapServices.setHcal_steel_cassette_thickness(
        theDetector.constant<double>("Hcal_steel_cassette_thickness"));
    HcalBarrel_EndCapServices.setHcalServices_outer_FR4_thickness(
        theDetector.constant<double>("HcalServices_outer_FR4_thickness"));
    HcalBarrel_EndCapServices.setHcalServices_outer_Cu_thickness(
...
bool BuildEcalBarrelServices::FillEcalBarrelServicesContainer(dd4hep::PlacedVolume& pVol,
  ...
  for (int i = 0; i < NMODULES_IN_Z; i++) { // these are the 5 modules in z
    ...
    dd4hep::Position posPE(0, -RailHeight / 2. + pe_thick / 2., -Ecal_barrel_halfZ + moduleLength * (i + 1 / 2.));
    ...
    dd4hep::Position posCu1(0, -RailHeight / 2. + pe_thick + cu_thick / 2.,
...
bool BuildEcalBarrelServices::DoBuildEcalBarrelServices(dd4hep::PlacedVolume& pVol, dd4hep::Assembly& envelope) {
  ...
  for (int stave_id = 1; stave_id < 9; stave_id++) {
    ...
    dd4hep::Position stavePosition((Ecal_outer_radius + env_safety * 2.0 + RailHeight / 2.) * sin(phirot) -
                                       module_thickness * sin(M_PI / 4.) * cos(-phirot),
                                   (Ecal_outer_radius + env_safety * 2.0 + RailHeight / 2.) * cos(phirot) -
                                       module_thickness * sin(M_PI / 4.) * sin(-phirot),
                                   0);
  ...
  for (int i = 0; i <= 1; i++) {
    ...
    for (int stave_id = 1; stave_id < 9; stave_id++) {
      ...
      if (z_position > 0) {
        ...
        rot = rot_P;
        dd4hep::Position stavePosition_P((Ecal_outer_radius - module_thickness / 2.) * sin(phirot) +
                                             (InnerServicesWidth / 2. + container_x_dim / 2.) * cos(-phirot),
                                         (Ecal_outer_radius - module_thickness / 2.) * cos(phirot) +
                                             (InnerServicesWidth / 2. + container_x_dim / 2.) * sin(-phirot),
                                         z_position);
        ...
        rot = rot_M;
        dd4hep::Position stavePosition_M((Ecal_outer_radius - module_thickness / 2.) * sin(phirot) +
                                             (-InnerServicesWidth / 2. - container_x_dim / 2.) * cos(-phirot),
                                         (Ecal_outer_radius - module_thickness / 2.) * cos(phirot) +
                                             (-InnerServicesWidth / 2. - container_x_dim / 2.) * sin(-phirot),
                                         z_position);
...
bool BuildSitCables::DoBuildSitCables(dd4hep::PlacedVolume& pVol, dd4hep::Assembly& envelope) {
  ...
  double SitTube_inner_radius = TPC_inner_radius - ftd4to7_tpc_radial_gap / 2.;
  ...
  double z_half_len = (TPC_Ecal_Hcal_barrel_halfZ - z_start_3) / 2.;

  dd4hep::Tube SitTubeSolid(SitTube_inner_radius, SitTube_inner_radius + Sit_cables_cylinder_thickness, z_half_len, 0.,
                            2 * M_PI);
  ...
  dd4hep::rec::VolCylinder cylSurf0(SitTubeLog0, dd4hep::rec::SurfaceType(dd4hep::rec::SurfaceType::Helper),
                                    0.5 * Sit_cables_cylinder_thickness, 0.5 * Sit_cables_cylinder_thickness, ocyl);
  ...
  double Sit_cables_disk_thickness =
      ((1 + TUBE_IPOuterBulge_end_radius / TPC_inner_radius) * 0.5 * SServices_FTD7_cables_thickness);

  dd4hep::Tube SitDiskSolid(TUBE_IPOuterBulge_end_envradius + SurfaceTolerance, TPC_inner_radius,
                            Sit_cables_disk_thickness / 2. - SurfaceTolerance, 0., 2 * M_PI);
  ...
  // Print out some parameters
  std::cout << "\n   - SIT12_cable_thickness = " << Sit_cables_cylinder_thickness
            << "\n   - SIT1_Radius = " << SIT1_Radius << "\n   - SIT2_Radius = " << SIT2_Radius
            << "\n   - SServices_FTD2_cone_thickness = " << FTD2_cone_thickness
            << "\n   - SServices_FTD3_cone_thickness = " << FTD3_cone_thickness
            << "\n   - TUBE_IPOuterBulge_end_z = " << TUBE_IPOuterBulge_end_z
            << "\n   - SServices_FTD7_cables_thickness = " << SServices_FTD7_cables_thickness << std::endl;
...
bool BuildHcalBarrel_EndCapServices::FillHcalServicesModuleWithInnerServices(dd4hep::PlacedVolume& pVol,
  ...
  for (int i = 0; i <= 1; i++) {
    dd4hep::Position layerPositionSteel(0, Ecal_cables_gap / 2. - StainlessSteel_Thickness / 2., yShift);

    if (!PlaceHcalInnerServicesLayer(pVol, motherLogical, stainless_steel, StainlessSteel_Thickness,
                                     layerPositionSteel))

      return false;

    dd4hep::Position layerPositionPoly(0, Ecal_cables_gap / 2. - StainlessSteel_Thickness - PE_Thickness / 2., yShift);

    if (!PlaceHcalInnerServicesLayer(pVol, motherLogical, polyethylene, PE_Thickness, layerPositionPoly))

      return false;

    dd4hep::Position layerPositionCopper(
        0, Ecal_cables_gap / 2. - StainlessSteel_Thickness - PE_Thickness - Cu_Thickness / 2., yShift);

    if (!PlaceHcalInnerServicesLayer(pVol, motherLogical, copper, Cu_Thickness, layerPositionCopper))
  ...
  // Print out some parameters
  std::cout << "\n   - HcalServicesModule_InnerServicesWidth = " << InnerServicesWidth
            << "\n   - HcalServicesModule_ZMinus_Cu_Thickness = " << ZMinus_Cu_Thickness
            << "\n   - HcalServicesModule_ZMinus_PE_Thickness = " << ZMinus_PE_Thickness
            << "\n   - HcalServicesModule_ZMinus_StainlessSteel_Thickness = " << ZMinus_StainlessSteel_Thickness
            << "\n   - HcalServicesModule_ZPlus_Cu_Thickness = " << ZPlus_Cu_Thickness
            << "\n   - HcalServicesModule_ZPlus_PE_Thickness = " << ZPlus_PE_Thickness
            << "\n   - HcalServicesModule_ZPlus_StainlessSteel_Thickness = " << ZPlus_StainlessSteel_Thickness
            << "\n   - HcalServices_outer_Cu_thickness = " << HcalServices_outer_Cu_thickness
            << "\n   - HcalServices_outer_FR4_thickness = " << HcalServices_outer_FR4_thickness
            << "\n   - Hcal_R_max = " << Hcal_R_max
            << "\n   - Hcal_back_plate_thickness = " << Hcal_back_plate_thickness
            << "\n   - Hcal_nlayers = " << Hcal_nlayers
            << "\n   - Hcal_radiator_thickness = " << Hcal_radiator_thickness
            << "\n   - Hcal_stave_gaps = " << Hcal_stave_gaps
            << "\n   - Hcal_steel_cassette_thickness = " << Hcal_steel_cassette_thickness << std::endl;
...
bool BuildHcalBarrel_EndCapServices::PlaceHcalInnerServicesLayer(dd4hep::PlacedVolume& pVol,
                                                                 ...
                                                                 dd4hep::Material layerMaterial, double layerThickness,
  ...
  dd4hep::Volume Center_Logical("HcalBarrel_EndCap_Center_Logical", Center_Solid, layerMaterial);
  ...
  dd4hep::Volume Logical_Right("HcalBarrel_EndCap_Logical_Right", intersectionSolidRight, layerMaterial);
  ...
  dd4hep::Volume Logical_Left("HcalBarrel_EndCap_Logical_Left", intersectionSolidLeft, layerMaterial);
...
bool BuildHcalBarrel_EndCapServices::FillHcalServicesModuleWithHcalElectronicsInterface(
    dd4hep::PlacedVolume& pVol, dd4hep::Volume& ModuleLogicalZMinus, dd4hep::Volume& ModuleLogicalZPlus) {

  std::cout << "   - BuildHcalElectronicsInterface = true " << std::endl;

  double Hcal_layer_thickenss = (Hcal_total_dim_y - Hcal_back_plate_thickness) / Hcal_nlayers;

  double Hcal_chamber_thickness = Hcal_layer_thickenss - Hcal_radiator_thickness;

  if (Hcal_chamber_thickness <= 0)
    throw std::runtime_error("Hcal Barrel-EndCap services: Hcal chamber thicknesses  <= 0");

#ifdef VERBOSE
  std::cout << "Hcal Barrel-EndCap services: Hcal_chamber_thickness = " << Hcal_chamber_thickness << std::endl;
#endif
  ...
  for (int layer_id = 1; layer_id <= Hcal_nlayers; layer_id++) {
    ...
    layer_y_offset = (layer_id - 1) * Hcal_layer_thickenss + Hcal_radiator_thickness;

    //---- bottom barrel----
    if (layer_id * Hcal_layer_thickenss < Hcal_y_dim1_for_x)
      layer_x_dim = Hcal_bottom_dim_x + 2 * layer_y_offset * tan(M_PI / 8.);
    else //----- top barrel ---
      layer_x_dim =
          Hcal_midle_dim_x - 2 * (layer_y_offset + Hcal_chamber_thickness - Hcal_y_dim1_for_x) / tan(M_PI / 8.);

    if (!FillHcalElectronicsInterfaceLayer(pVol, ModuleLogicalZMinus, ModuleLogicalZPlus, layer_y_offset, layer_x_dim))
...
bool BuildHcalBarrel_EndCapServices::FillHcalElectronicsInterfaceLayer(dd4hep::PlacedVolume& pVol,
  ...
  double y_position = -Hcal_y_dim1_for_x / 2. + layer_y_offset + Hcal_steel_cassette_thickness / 2.;

  if (!PlaceHcalElectronicsInterfaceComponent(pVol, ModuleLogicalZMinus, ModuleLogicalZPlus, S235,
                                              Hcal_steel_cassette_thickness, y_position, layer_x_dim))
    return false;

  y_position += (Hcal_steel_cassette_thickness / 2. + HcalServices_outer_FR4_thickness / 2.);

  if (!PlaceHcalElectronicsInterfaceComponent(pVol, ModuleLogicalZMinus, ModuleLogicalZPlus, PCB,
  ...
  if (!PlaceHcalElectronicsInterfaceComponent(pVol, ModuleLogicalZMinus, ModuleLogicalZPlus, copper,
...
dd4hep::Solid BuildHcalBarrel_EndCapServices::CutLayer(dd4hep::Solid& layerSolid, double y_position) {
  ...
  dd4hep::Box Solid_Side(InnerServicesWidth / 2. + tolerance, Hcal_total_dim_y + 8 * (SurfaceTolerance),
...
bool BuildHcalBarrel_EndCapServices::DoBuildHcalBarrel_EndCapServices(dd4hep::PlacedVolume& pVol,
  ...
  double Hcal_y_dim1_for_x = Hcal_total_dim_y - Hcal_y_dim2_for_x;
  Hcal_bottom_dim_x = 2. * Hcal_inner_radius * tan(M_PI / 8.) - Hcal_stave_gaps;
  Hcal_midle_dim_x = Hcal_bottom_dim_x + 2 * Hcal_y_dim1_for_x * tan(M_PI / 8.);
  Hcal_top_dim_x = Hcal_midle_dim_x - 2 * Hcal_y_dim2_for_x / tan(M_PI / 8.);

#ifdef VERBOSE
  double Hcal_outer_radius = Hcal_inner_radius + Hcal_total_dim_y;

  G4cout << "BuildHcalBarrel_EndCapServices information: "
         << "\n                       Hcal_outer_radius = " << Hcal_outer_radius
         << "\n                       module thickness = " << Hcal_total_dim_y
         << "\n                       Hcal_R_max = " << Hcal_R_max
         << "\n                       Hcal_bottom_dim_x = " << Hcal_bottom_dim_x << G4endl;
#endif
  ...
  if (!(BuildHcalElectronicsInterface == 0))
    if (!FillHcalServicesModuleWithHcalElectronicsInterface(pVol, ModuleLogicalZMinus, ModuleLogicalZPlus))
      return false;
  ...
  for (int stave_id = 1; stave_id <= 8; stave_id++) {
    double module_z_offset = -TPC_Ecal_Hcal_barrel_halfZ - Ecal_cables_gap / 2. - env_safety * 2.0;

    double phirot = stave_phi_offset;

    dd4hep::RotationZYX rot(0, phirot, -M_PI * 0.5);
    dd4hep::Position stave_pos1(Y * sin(phirot), Y * cos(phirot), module_z_offset);
    ...
    dd4hep::Position stave_pos2(Y * sin(phirot), Y * cos(phirot), module_z_offset);
...
bool BuildVXDCables::DoBuildVXDCables(dd4hep::PlacedVolume& pVol, dd4hep::Assembly& envelope) {
  ...
  VXD_cable_cone_middle_thickness =
      std::sqrt(VXD_cable_cross_section_area / M_PI + (VXD_cable_inner1_radius + VXD_cable_inner2_radius) / 2.0 *
  ...
  dd4hep::Tube VXDTubeSolid(VXDTube_inner_radius, VXDTube_inner_radius + VXD_cables_cylinder_thickness, z_half_len, 0.,
                            2 * M_PI);
  ...
  dd4hep::rec::VolCylinder cylSurf0(VXDTubeLog0, dd4hep::rec::SurfaceType(dd4hep::rec::SurfaceType::Helper),
                                    0.5 * VXD_cables_cylinder_thickness, 0.5 * VXD_cables_cylinder_thickness, ocyl);
  ...
  // Print out some parameters
  std::cout << "\n   - VXD_cable_cross_section_area = " << VXD_cable_cross_section_area / dd4hep::mm / dd4hep::mm
            << " mm^2"
            << "\n   - VXD_cable_inner1_radius = " << VXD_cable_inner1_radius / dd4hep::mm << " mm"
            << "\n   - VXD_cable_inner2_radius = " << VXD_cable_inner2_radius / dd4hep::mm << " mm"
            << "\n   - VXD_cable_cone_middle_thickness = " << VXD_cable_cone_middle_thickness / dd4hep::mm << " mm"
            << "\n   - VXD_cable_outer1_radius = " << VXD_cable_outer1_radius / dd4hep::mm << " mm"
            << "\n   - VXD_cable_outer2_radius = " << VXD_cable_outer2_radius / dd4hep::mm << " mm"
            << "\n   - VXD_cable_z_start = " << VXD_cable_z_start / dd4hep::mm << " mm"
            << "\n   - VXD_cable_z_end = " << VXD_cable_z_end / dd4hep::mm << " mm" << std::endl;
