...
static Ref_t create_element(Detector& theDetector, xml_h e, SensitiveDetector /*sens*/) {
  ...
  for (xml_coll_t c(x_det, Unicode("reference")); c; ++c) {
    ...
    if (offsetValue != 0) {
      cout << "TubeX01: Using " << globalName << " = " << currentValue / dd4hep::mm << " mm instead of "
           << assumedValue / dd4hep::mm << " mm" << endl;
      usingOffsets = true;
    }
  }
  if (usingOffsets)
    std::cout << "TubeX01: Be sure you know what you're doing!" << std::endl;

  // unused: bool saveToGear = true; // as long as this is true the vectors gearValZ, gearValRInner and gearValROuter
  // will be filled in the coming loop
  ...
  for (xml_coll_t c(x_det, Unicode("section")); c; ++c) {
    ...
    case kPunchedCenter: {
      ...
      // std::cout << " *** placing tubes for volume " << volName << ": "  << placementTransformer  <<
      // placementTransmirror
      // 		<< " rotation : " << RotationY(rotateAngle) 	<< std::endl
      // 		<< " angle : " << rotateAngle	<< std::endl
      // 		<< " zPos: " << zPosition	<< std::endl ;
    ...
    case kPunchedDnstream: {
      ...
      const double rCenterPunch =
          (crossType == kPunchedUpstream) ? (rInnerStart) : (rInnerEnd); // just alias names denoting what is meant here
      const double rOffsetPunch = (crossType == kPunchedDnstream)
                                      ? (rInnerStart)
    ...
    case kDnstreamSlicedFront: {
      ...
      // relative transformations for the composition of the SubtractionVolumes
      const double clipAngle = (crossType == kUpstreamClippedFront || crossType == kDnstreamClippedFront)
                                   ? (rotateAngle)
                                   : (2 * rotateAngle);
    ...
    case kDnstreamSlicedRear: {
      ...
      const double clipAngle =
          (crossType == kUpstreamClippedRear || crossType == kDnstreamClippedRear) ? (rotateAngle) : (2 * rotateAngle);
      const double clipShift =
    ...
    case kDnstreamSlicedBoth: {
      ...
      // relative transformations for the composition of the SubtractionVolumes
      const double clipAngle =
          (crossType == kUpstreamClippedBoth || crossType == kDnstreamClippedBoth) ? (rotateAngle) : (2 * rotateAngle);
