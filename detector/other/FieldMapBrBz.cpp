...
int FieldMapBrBz::getGlobalIndex(const int rBin, const int zBin) {
  ...
  int myzBin = zBin;
  if (rhoOrdering == -1)
    myrBin = nRho - myrBin - 1; // recalculate rho-axis index in case of high-to-low ordering
  if (zOrdering == -1)
    myzBin = nZ - myzBin - 1; // recalculate z-axis   index in case of high-to-low ordering

  int globalIndex = -1;
  if (coorsOrder == 1)
    globalIndex = myrBin + myzBin * nRho; // RZ coordinates ordering
  else if (coorsOrder == 2)
    globalIndex = myzBin + myrBin * nZ; // ZR coordinates ordering
...
void FieldMapBrBz::fieldComponents(const double* pos, double* globalField) {
  ...
  field[0] = (1.0 - rd) * (1.0 - zd) * B_r0z0.Br + rd * (1.0 - zd) * B_r1z0.Br + (1.0 - rd) * zd * B_r0z1.Br +
             rd * zd * B_r1z1.Br;
  ...
  std::cout << "(x,y,z)     = (" << pos[0]/dd4hep::cm  << "," << pos[1]/dd4hep::cm  << "," << pos[2]/dd4hep::cm  << ")
  cm" << std::endl; std::cout << "B(x,y,z)    = (" << globalField[0]/dd4hep::tesla << "," <<
  globalField[1]/dd4hep::tesla << "," << globalField[2]/dd4hep::tesla << ") tesla" << std::endl; std::cout << std::endl;
  */

  return;
}

void FieldMapBrBz::fillFieldMapFromTree(const std::string& filename, double coorUnits, double BfieldUnits) {
  ...
  if (strCoorsOrder == std::string("RZ"))
    coorsOrder = 1;
  else if (strCoorsOrder == std::string("ZR"))
    coorsOrder = 2;
  ...
  if (elements != treeEntries) {
    ...
    error << "FieldMapBrBz[ERROR]: Tree does not have the expected number of entries "
          << "nRho*nZ (" << nRho << "*" << nZ << ") == " << elements << "  tree entries == " << treeEntries;
    throw std::runtime_error(error.str());
  }
  ...
  for (int iz = 0; iz < nZ; iz++) {
    for (int ir = 0; ir < nRho; ir++) {
      ...
      fieldMap.push_back(
          FieldMapBrBz::FieldValues_t(double(Br) * bScale * BfieldUnits, double(Bz) * bScale * BfieldUnits));
    }
  }
...
static Ref_t create_FieldMap_rzBrBz(Detector&, dd4hep::xml::Handle_t handle) {
  ...
  if (ptr->rhoOrdering == -1)
    strRhoOrdering = "high-to-low";
  if (ptr->zOrdering == -1)
