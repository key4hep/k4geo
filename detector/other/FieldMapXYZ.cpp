...
int FieldMapXYZ::getGlobalIndex(const int xBin, const int yBin, const int zBin) {
  ...
  int myzBin = zBin;
  if (xOrdering == -1)
    myxBin = nX - myxBin - 1; // recalculate x-axis index in case of high-to-low ordering
  if (yOrdering == -1)
    myyBin = nY - myyBin - 1; // recalculete y-axis index in case of high-to-low ordering
  if (zOrdering == -1)
    myzBin = nZ - myzBin - 1; // recalculate z-axis index in case of high-to-low ordering

  int globalIndex = -1;
  if (coorsOrder == 1)
    globalIndex = myxBin + myyBin * (nX) + myzBin * (nX * nY); // XYZ coordinates ordering
  else if (coorsOrder == 2)
    globalIndex = myxBin + myzBin * (nX) + myyBin * (nX * nZ); // XZY coordinates ordering
  else if (coorsOrder == 3)
    globalIndex = myyBin + myxBin * (nY) + myzBin * (nY * nX); // YXZ coordinates ordering
  else if (coorsOrder == 4)
    globalIndex = myyBin + myzBin * (nY) + myxBin * (nY * nZ); // YZX coordinates ordering
  else if (coorsOrder == 5)
    globalIndex = myzBin + myxBin * (nZ) + myyBin * (nZ * nX); // ZXY coordinates ordering
  else if (coorsOrder == 6)
    globalIndex = myzBin + myyBin * (nZ) + myxBin * (nZ * nY); // ZYX coordinates ordering
...
void FieldMapXYZ::fieldComponents(const double* pos, double* globalField) {
  ...
  if (not(x >= xMin && x <= xMax && y >= yMin && y <= yMax && z >= zMin && z <= zMax)) {
    return;
  }
  ...
  std::cout << "(x,y,z)     = (" << x/dd4hep::cm  << "," << y/dd4hep::cm  << "," << z/dd4hep::cm  << ") cm" <<
  std::endl; std::cout << "(x0,y0,z0)  = (" << x0/dd4hep::cm << "," << y0/dd4hep::cm << "," << z0/dd4hep::cm << ") cm"
  << std::endl; std::cout << "(x1,y1,z1)  = (" << x1/dd4hep::cm << "," << y1/dd4hep::cm << "," << z1/dd4hep::cm << ")
  cm" << std::endl; std::cout << "B(x0,y0,z0) = (" << B_x0y0z0.Bx/dd4hep::tesla << "," << B_x0y0z0.By/dd4hep::tesla <<
  "," << B_x0y0z0.Bz/dd4hep::tesla << ") tesla" << std::endl; std::cout << "B(x1,y0,z0) = (" <<
  B_x1y0z0.Bx/dd4hep::tesla << "," << B_x1y0z0.By/dd4hep::tesla << "," << B_x1y0z0.Bz/dd4hep::tesla << ") tesla" <<
  std::endl; std::cout << "B(x0,y0,z1) = (" << B_x0y0z1.Bx/dd4hep::tesla << "," << B_x0y0z1.By/dd4hep::tesla << "," <<
  B_x0y0z1.Bz/dd4hep::tesla << ") tesla" << std::endl; std::cout << "B(x1,y0,z1) = (" << B_x1y0z1.Bx/dd4hep::tesla <<
  "," << B_x1y0z1.By/dd4hep::tesla << "," << B_x1y0z1.Bz/dd4hep::tesla << ") tesla" << std::endl; std::cout <<
  "B(x0,y1,z0) = (" << B_x0y1z0.Bx/dd4hep::tesla << "," << B_x0y1z0.By/dd4hep::tesla << "," << B_x0y1z0.Bz/dd4hep::tesla
  << ") tesla" << std::endl; std::cout << "B(x1,y1,z0) = (" << B_x1y1z0.Bx/dd4hep::tesla << "," <<
  B_x1y1z0.By/dd4hep::tesla << "," << B_x1y1z0.Bz/dd4hep::tesla << ") tesla" << std::endl; std::cout << "B(x0,y1,z1) =
  (" << B_x0y1z1.Bx/dd4hep::tesla << "," << B_x0y1z1.By/dd4hep::tesla << "," << B_x0y1z1.Bz/dd4hep::tesla << ") tesla"
  << std::endl; std::cout << "B(x1,y1,z1) = (" << B_x1y1z1.Bx/dd4hep::tesla << "," << B_x1y1z1.By/dd4hep::tesla << ","
  << B_x1y1z1.Bz/dd4hep::tesla << ") tesla" << std::endl; std::cout << "B(x,y,z)    = (" << globalField[0]/dd4hep::tesla
  << "," << globalField[1]/dd4hep::tesla << "," << globalField[2]/dd4hep::tesla << ") tesla" << std::endl; std::cout <<
  std::endl;
  */
...
void FieldMapXYZ::fillFieldMapFromTree(const std::string& filename, double coorUnits, double BfieldUnits) {
  ...
  if (strCoorsOrder == TString("XYZ"))
    coorsOrder = 1;
  else if (strCoorsOrder == TString("XZY"))
    coorsOrder = 2;
  else if (strCoorsOrder == TString("YXZ"))
    coorsOrder = 3;
  else if (strCoorsOrder == TString("YZX"))
    coorsOrder = 4;
  else if (strCoorsOrder == TString("ZXY"))
    coorsOrder = 5;
  else if (strCoorsOrder == TString("ZYX"))
    coorsOrder = 6;
  ...
  if (elements != treeEntries) {
    ...
    error << "FieldMapXYZ[ERROR]: Tree does not have the expected number of entries "
          << "nX*nY*nZ (" << nX << "*" << nY << "*" << nZ << ") == " << elements << "  tree entries == " << treeEntries;
    throw std::runtime_error(error.str());
  }
  ...
  for (int iz = 0; iz < nZ; iz++) {
    for (int iy = 0; iy < nY; iy++) {
      for (int ix = 0; ix < nX; ix++) {
        ...
        fieldMap.push_back(FieldMapXYZ::FieldValues_t(
            double(Bx) * bScale * BfieldUnits, double(By) * bScale * BfieldUnits, double(Bz) * bScale * BfieldUnits));
      }
    }
  }
...
static Ref_t create_FieldMap_XYZ(Detector&, dd4hep::xml::Handle_t handle) {
  ...
  if (ptr->xOrdering == -1)
    strYOrdering = "high-to-low";
  if (ptr->yOrdering == -1)
    strYOrdering = "high-to-low";
  if (ptr->zOrdering == -1)
    strZOrdering = "high-to-low";
