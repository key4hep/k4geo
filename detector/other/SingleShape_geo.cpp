...
// Edited by A. Ilg, to have only the CAD import left and nothing else
...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens)  {
  ...
  xml_dim_t  xrot   (x_det.child(_U(rotation)));
  ...
  RotationZYX rot(xrot.phi(),xrot.theta(),xrot.psi());
  ...
  for(xml_coll_t c(xshape, _U(position)); c; ++c)   {
    ...
    pv = assembly.placeVolume(vol, Transform3D(rot, vol_pos));
  }
