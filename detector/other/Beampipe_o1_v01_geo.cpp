...
public:
  ...
  SimpleCylinderImpl(Volume vol, dd4hep::rec::SurfaceType type, double thickness_inner, double thickness_outer,
                     dd4hep::rec::Vector3D origin)
      : VolCylinderImpl(vol, type, thickness_inner, thickness_outer, origin), _half_length(0) {}
...
public:
  SimpleCylinder(Volume vol, SurfaceType type, double thickness_inner, double thickness_outer, Vector3D origin)
...
static Ref_t create_element(Detector& theDetector, xml_h element, SensitiveDetector /*sens*/) {
  ...
  for (xml_coll_t c(xmlBeampipe, Unicode("section")); c; ++c) {
    ...
    std::cout << std::setw(8) << zStart / dd4hep::mm << std::setw(8) << zEnd / dd4hep::mm << std::setw(8)
              << rInnerStart / dd4hep::mm << std::setw(8) << rInnerEnd / dd4hep::mm << std::setw(8)
              << rOuterStart / dd4hep::mm << std::setw(8) << rOuterEnd / dd4hep::mm << std::setw(8)
              << thickness / dd4hep::mm << std::setw(8) << crossType << std::setw(35) << volName << std::setw(15)
              << sectionMat.name() << std::endl;
    ...
    case ODH::kDnstream: {
      ...
      if (rInnerStart != rOuterStart || rInnerEnd != rOuterEnd) {
        ...
        if (crossType == ODH::kCenter) {
          ...
          if (rInnerStart < min_radius)
            min_radius = rInnerStart;
          if (rOuterStart < min_radius)
            min_radius = rOuterStart;
        }
    ...
    case ODH::kDnstreamSlicedFront: {
      ...
      // relative transformations for the composition of the SubtractionVolumes
      const double clipAngle = (crossType == ODH::kUpstreamClippedFront || crossType == ODH::kDnstreamClippedFront)
                                   ? (rotateAngle)
                                   : (2 * rotateAngle);
      const double clipShift =
          (zStart - clipSize) / cos(clipAngle) - (zPosition - clipSize / 2); // question: why is this correct?
      ...
      // absolute transformations for the final placement in the world
      Transform3D placementTransformer(RotationY(rotateAngle),
                                       RotateY(Position(0, 0, zPosition - clipSize / 2), rotateAngle));
      Transform3D placementTransmirror(RotationY(mirrorAngle),
                                       RotateY(Position(0, 0, zPosition - clipSize / 2), mirrorAngle));
      ...
      if (rInnerStart != rOuterStart || rInnerEnd != rOuterEnd) {
        // the wall solid: a tubular cone
        ConeSegment wallWholeSolid(zHalf + clipSize / 2, rInnerStart, rOuterStart, rInnerEnd, rOuterEnd, phi1,
                                   phi2); // a bit longer
    ...
    case ODH::kDnstreamSlicedRear: {
      ...
      // relative transformations for the composition of the SubtractionVolumes
      const double clipAngle = (crossType == ODH::kUpstreamClippedRear || crossType == ODH::kDnstreamClippedRear)
                                   ? (rotateAngle)
                                   : (2 * rotateAngle);
      const double clipShift =
          (zEnd + clipSize) / cos(clipAngle) - (zPosition + clipSize / 2); // question: why is this correct?
      Transform3D clipTransformer(RotationY(-clipAngle), Position(0, 0, clipShift));
      Transform3D clipTransmirror(RotationY(+clipAngle), Position(0, 0, clipShift));
      ...
      if (rInnerStart != rOuterStart || rInnerEnd != rOuterEnd) {
        // the wall solid: a tubular cone
        ConeSegment wallWholeSolid(rInnerStart, rOuterStart, rInnerEnd, rOuterEnd, zHalf + clipSize / 2, phi1,
                                   phi2); // a bit longer
    ...
    case ODH::kDnstreamSlicedBoth: {
      ...
      // relative transformations for the composition of the SubtractionVolumes
      const double clipAngle = (crossType == ODH::kUpstreamClippedBoth || crossType == ODH::kDnstreamClippedBoth)
                                   ? (rotateAngle)
                                   : (2 * rotateAngle);
      ...
      if (rInnerStart != rOuterStart || rInnerEnd != rOuterEnd) {
        // the wall solid: a tubular cone
        ConeSegment wallWholeSolid(rInnerStart, rOuterStart, rInnerEnd, rOuterEnd, zHalf + clipSize, phi1,
                                   phi2); // a bit longer
...
DECLARE_DEPRECATED_DETELEMENT(Beampipe_o1_v01, create_element)
