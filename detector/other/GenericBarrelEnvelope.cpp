...
static Ref_t create_detector(Detector& theDetector, xml_h e, SensitiveDetector /*sens*/) {
  ...
  if (nsides_inner != 0 && nsides_outer != 0) {
    if ((nsides_outer - nsides_inner) == 0 && phi_offset == 0) {
      ...
      PolyhedraRegular polyhedron_inner(
          nsides_inner, 0, inner_r,
          2.1 * half_z); // Here the inner polyhedron hald z is increased for better substraction!
    ...
    if (nsides_inner == 0 && nsides_outer == 0) {
      ...
      Tube tube_inner(0, inner_r,
                      1.1 * half_z); // Here the inner polyhedron hald z is increased for better substraction!
      PolyhedraRegular polyhedron_outer(nsides_outer, phi_offset, 0, outer_r, 2 * half_z);
