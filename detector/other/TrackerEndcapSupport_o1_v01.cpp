...
static Ref_t create_detector(Detector& theDetector, xml_h e, SensitiveDetector) {
  ...
  for (xml_coll_t i(x_det, _U(layer)); i; ++i, ++l_num) {
    ...
    for (xml_coll_t j(x_layer, _U(slice)); j; ++j, ++s_num) {
      ...
      VolSurfaceHandle<VolPlaneImpl> cylSurf1(s_vol, SurfaceType(SurfaceType::Helper), 0.5 * thick, 0.5 * thick, u, v,
                                              n, ocyl);
    ...
    if (reflect) {
      ...
      for (xml_coll_t j(x_layer, _U(slice)); j; ++j, ++s_num) {
        ...
        VolSurfaceHandle<VolPlaneImpl> cylSurf2(s_vol2, SurfaceType(SurfaceType::Helper), 0.5 * thick, 0.5 * thick, u,
                                                v, -1. * n, -1. * ocyl);
