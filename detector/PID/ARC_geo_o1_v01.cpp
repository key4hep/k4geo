 ...
 */
double GetVariableFromXML(Detector& desc, std::string varName, double defaultValue, double lowlimit = 0.,
                          double upperlimit = 0.) {
...
static Ref_t create_ARC_endcaps(Detector& desc, xml::Handle_t handle, SensitiveDetector sens) {
  ...
  Tube gasenvelopeS(vessel_inner_r + vessel_wall_thickness, vessel_outer_r - vessel_wall_thickness,
  ...
  Tube vesselEnvelopeSolid(vessel_inner_r, vessel_outer_r, vessel_length / 2.);
  ...
  if (0 < bulk_skin_ratio) {
    ...
    Tube vesselInnerBulkSolid(vessel_bulk_inner_r_ini, vessel_bulk_inner_r_fin,
    ...
    Tube vesselOuterBulkSolid(vessel_bulk_outer_r_ini, vessel_bulk_outer_r_fin,
                              vessel_length / 2. - (1 - bulk_skin_ratio) * 0.5 * vessel_wall_thickness);
    ...
    Tube vesselBaseBulkSolid(vessel_bulk_inner_r_fin, vessel_bulk_outer_r_ini,
  ...
  for (auto& ncell : mycell_v) {
    ...
    for (int phin = 0; phin < phinmax; phin++, cellCounter++) {
      ...
      // if parameter not present, exception is thrown and not catched
      {
        ...
        if (radius_of_sphere <= mirrorThickness)
          throw std::runtime_error(
              Form("Ilegal parameters cell %d: %g <= %g", ncell.RID, radius_of_sphere, mirrorThickness));
      }
      ...
      Sphere mirrorShapeFull(radius_of_sphere - mirrorThickness, radius_of_sphere, 0., 3.14 / 3.5);
      ...
      SkinSurface mirrorSkin(desc, mirrorDE, Form("mirror_optical_surface%d", cellCounter), mirrorSurf,
                             mirrorVol); // FIXME: 3rd arg needs `imod`?
      mirrorSkin.isValid();
      ...
      // ~ ~ ~ ~ ~ ~ ~ ~ ~ ~  AEROGEL PLATE  ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ //
      // ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ //
      auto aerogelTrCell = RotationZYX(0, 0, 0) * Translation3D(0, 0, sensor_z_origin_Martin + aerogel_z_offset);
      ...
      auto sensorTr =
          RotationZ(alpha - 90 * deg) * RotationX(angle_of_sensor) * Translation3D(0, center_of_sensor_x, sensor_z_pos);

      PlacedVolume sensorPV = cellV.placeVolume(sensorVol, sensorTr);
//       sensorPV.addPhysVolID("cellnumber", 6 * cellCounter+2);
#ifdef DUMP_SENSOR_POSITIONS
      ofile_sensor_pos << 6 * cellCounter + 2 << '\t' << ncell.RID << '\t' << ncell.isReflected << '\t' << ncell.x
                       << '\t' << ncell.y << '\t' << phin << '\n';
#endif
      ...
      PlacedVolume cellPV =
          endcap_cells_gas_envelope.placeVolume(cellV, RotationZ(phistep * phin) * Translation3D(ncell.x, ncell.y, 0));
      ...
      if (ncell.isReflected) {
        ...
        SkinSurface mirror_ref_Skin(desc, mirror_ref_DE, Form("mirror_ref_optical_surface%d", cellCounter), mirrorSurf,
                                    mirrorVol_reflected); // FIXME: 3rd arg needs `imod`?
...
#ifdef DUMP_SENSOR_POSITIONS
        ofile_sensor_pos << 6 * cellCounter + 5 << '\t' << ncell.RID << '\t' << ncell.isReflected << '\t' << ncell.x
                         << '\t' << ncell.y << '\t' << phin << '\n';
#endif
        ...
        PlacedVolume cell_ref_PV = endcap_cells_gas_envelope.placeVolume(
            cellV_reflected, RotationZ(phistep * phin) * Translation3D(-ncell.x, ncell.y, 0));
...
static Ref_t create_ARC_barrel(Detector& desc, xml::Handle_t handle, SensitiveDetector sens) {
  ...
  double mirror_z_origin_Martin =
      vessel_outer_r - vessel_wall_thickness - 37 * cm - mirrorThickness - mirror_z_safe_shrink;
  // // //-------------------------------------------------------------// // //
  ...
  // Build cylinder for gas, and the vessel for the gas
  Tube gasenvelopeS(vessel_inner_r + vessel_wall_thickness, vessel_outer_r - vessel_wall_thickness, vessel_length / 2.);
  ...
  Tube vesselEnvelopeSolid(vessel_inner_r, vessel_outer_r, vessel_length / 2. + vessel_wall_thickness);
  ...
  auto cell_shape = create_ARC_barrel_shape(vessel_inner_r + vessel_wall_thickness,
                                            vessel_outer_r - vessel_wall_thickness, hexagon_side_length);
  ...
  for (auto ncell_original : ncell_vector) {
    ...
    for (int phin = 0; phin < phinmax; ++phin) {
      ...
      Sphere mirrorShapeFull(radius_of_sphere - mirrorThickness, radius_of_sphere, 0., 3.14 / 3.5);
      /// 3D transformation of mirrorVolFull in order to place it inside the gas volume
      Transform3D mirrorTr(RotationZYX(0, 0, 0),
                           Translation3D(center_of_sphere_x, 0, center_of_sphere_z - mirror_z_safe_shrink));
      ...
      std::string mirrorName = create_part_name_ff(
          "mirror"); // detName + "_mirror" + std::to_string(ncell) + "z" + std::to_string(reflect_parameters)
      ...
      Transform3D sensorTr(RotationZYX(0, 90 * deg - angle_of_sensor, 0),
                           Translation3D(-sensor_z_pos, 0, center_of_sensor_x));
      ...
      // ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ //
      {
        ...
        Transform3D coolingTr(RotationZYX(0, 90 * deg - angle_of_sensor, 0),
                              Translation3D(-sensor_z_pos + cooling_z_offset, 0, center_of_sensor_x));
