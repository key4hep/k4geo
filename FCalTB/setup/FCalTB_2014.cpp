...
static Ref_t create_detector(Detector& theDetector, xml_h xmlHandle, SensitiveDetector sens) {
    ...
    for (int i = 0, repeat = xmlLayer.repeat(); i < repeat; ++i) {
      ...
      for (dd4hep::xml::Collection_t collSlice(xmlLayer, _U(slice)); collSlice; ++collSlice) {
        ...
        if (sliceType.compare("Square") == 0) {
          ...
          PlacedVolume pv =
              layer_vol.placeVolume(slice_vol, Position(0, 0.0, inThisLayerPosition + sliceThickness * 0.5));
          ...
          slice_vol.setAttributes(theDetector, compSlice.regionStr(), compSlice.limitsStr(), compSlice.visStr());
          PlacedVolume pv =
              layer_vol.placeVolume(slice_vol, Position(0, -offsetArc, inThisLayerPosition + sliceThickness * 0.5));
          pv.addPhysVolID("slice", sliceID);
  ...
  PlacedVolume pv = assembly.placeVolume(envelopeVol, bcForwardPos);
