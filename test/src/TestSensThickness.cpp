...
template <typename T>
void checkExtensions(dd4hep::Detector& lcdd, std::vector<std::string> detectorNames,
                     std::vector<double> sensThickness) {

  int counter = 0;
  for (auto const& detName : detectorNames) {
    const double thickness = sensThickness[counter];
    ++counter;
    ...
    for (auto const& layer : layers) {
      ...
      msg << detName << " layer " << layerCount << ": "
          << "layer sensitive thickness:  " << layer.thicknessSensitive / dd4hep::micrometer << "  expected "
          << thickness / dd4hep::micrometer;
...
int main(int argc, char** args) {
  ...
  if (argc != 4) {
    throw std::runtime_error(
        "need to provide compact file and thickness for tracker and vertex silicon thicknesses in micrometer");
  }
  ...
  std::vector<std::string> endcaps = {"InnerTrackerEndcap", "OuterTrackerEndcap", "VertexEndcap"};

  std::vector<std::string> barrels = {"InnerTrackerBarrel", "OuterTrackerBarrel", "VertexBarrel"};

  std::vector<double> sensThickness = {trackerThick * dd4hep::micrometer, trackerThick * dd4hep::micrometer,
