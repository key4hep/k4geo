...
namespace rec {
  ...
  public:
    ...
    Vector3D Calculate_hitpos_to_wire_vector(int superlayer, int ilayer, int sector, int nphi,
                                             const Vector3D& hit_position /*in cm*/) const;
  ...
  inline std::ostream& operator<<(std::ostream& io, const WireTracker_info& d) {
    d.ShowDatabase(io);
    return io;
  }
  ...
  ///
  inline WireTracker_info_struct::Vector3D
  WireTracker_info_struct::Calculate_hitpos_to_wire_vector(int isuperlayer, int ilayer, int isector, int nphi,
                                                           const Vector3D& hit_position /*in cm*/) const {
  ...
  struct DCH_info_struct : WireTracker_info_struct {
    ...
    int Get_nsuperlayer_minus_1(int ilayer) const { return int((ilayer - 1) / nlayersPerSuperlayer); }

    /// Calculate radius at z=L/2 given at z=0
    length_t Radius_zLhalf(length_t r_z0) const { return r_z0 / cos(twist_angle / 2 / dd4hep::rad); }

    /// tan(stereoangle) = R(z=0)   / (L/2) * tan( twist_angle/2)
    angle_t stereoangle_z0(length_t r_z0) const { return atan(r_z0 / Lhalf * tan(twist_angle / 2 / dd4hep::rad)); }
    ...
    length_t Pitch_z0(LayerInfo layer, length_t r_z0) const { return dd4hep::twopi * r_z0 / (2 * layer.ncells); };
    ...
    bool IsStereoPositive(LayerInfo layer_info) const { return (1 == layer_info.layer % 2); };

    /// calculate sign based on IsStereoPositive
    int StereoSign(LayerInfo layer_info) const { return (IsStereoPositive(layer_info) * 2 - 1); };

    /// dphi between two sensing wires
    angle_t Get_phi_width(int ilayer) const { return (dd4hep::twopi / (database.at(ilayer).ncells)) * dd4hep::rad; };
    ...
    void BuildLayerDatabase() override final {
      ...
      if (not this->IsDatabaseEmpty())
        return;

      auto ff_check_positive_parameter = [](double p, std::string pname) -> void {
        if (p <= 0)
          throw std::runtime_error(Form("DCH: %s must be positive", pname.c_str()));
        return;
      };
    ...
    void ShowDatabase(std::ostream& oss) const override final {
      ...
      for (const auto& [nlayer, l] : database) {
        oss << "\t" << l.layer << "\t" << l.ncells << "\t" << l.height_z0 / dd4hep::mm << "\t"
            << l.width_z0 / dd4hep::mm << "\t" << l.radius_sw_z0 / dd4hep::mm << "\t" << l.stereo_sw_z0 / dd4hep::deg
  ...
  struct STT_info_struct : WireTracker_info_struct {
    ...
    void BuildLayerDatabase() override final {
      ...
      /// default: 80 = 8 * 10
      this->nlayers = 0;
      for (const int nl : this->nlayersPerSuperlayer)
        this->nlayers += nl;
    ...
    void ShowDatabase(std::ostream& oss) const override final {
      ...
      std::copy(nlayersPerSuperlayer.begin(), nlayersPerSuperlayer.end(), std::ostream_iterator<int>(oss, ", "));
      oss << "\n";
      oss << "\tN layers = " << nlayers << '\n';
      oss << "\n";
      ...
      for (const auto& [nlayer, l] : database) {
        oss << "\t" << l.layer << "\t" << l.ncells << "\t" << l.radius_sw_z0 / dd4hep::mm << "\t"
