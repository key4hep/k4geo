...
namespace dd4hep  {
  ...
  /// Namespace for the Geant4 based simulation part of the AIDA detector description toolkit
    namespace sim  {
        ...
        template <> void Geant4FSShowerModel<DRCFiberModel>::initialize() {
            this->m_applicablePartNames.emplace_back("opticalphoton");
        }

        template <> void Geant4FSShowerModel<DRCFiberModel>::constructSensitives(Geant4DetectorConstructionContext* ctxt) {
            this->Geant4FastSimShowerModel::constructSensitives(ctxt);
        }

        template <> void Geant4FSShowerModel<DRCFiberModel>::modelShower(const G4FastTrack& fasttrack, G4FastStep& faststep)   {
            auto* track = fasttrack.GetPrimaryTrack();

            if (locals.fKill) { // absorption
        ...
        template <> bool Geant4FSShowerModel<DRCFiberModel>::check_applicability(const G4ParticleDefinition& particle) {
            return &particle == G4OpticalPhoton::OpticalPhotonDefinition();
        }

        template <> bool Geant4FSShowerModel<DRCFiberModel>::check_trigger(const G4FastTrack& fasttrack)   {
            if (!locals.fSwitch)
            ...
            if (locals.fTransported) { // track is already transported and did pass NILL check, nothing to do
