...
struct TPCSDData {
  ...
  TPCSDData()
      : fThresholdEnergyDeposit(0), fHCID(-1), fSpaceHitCollectionID(-1),
  ...
  /// Returns the volumeID of sensitive volume corresponding to the step (either
  ...
  long long int volID(G4Step GEANT4_CONST_STEP *step, bool usePostPos = false) {
    ...
    VolumeID volID = (usePostPos ? volMgr.volumeID(h.postTouchable())
                                 : volMgr.volumeID(h.preTouchable()));
  ...
  void dumpStep(Geant4StepHandler h, G4Step GEANT4_CONST_STEP *s) {
    ...
    std::cout << " ----- step in detector " << h.sdName(s->GetPreStepPoint())
              << " prePos  " << h.prePos() << " postPos " << h.postPos()
              << " preStatus  " << h.preStepStatus() << " postStatus  "
              << h.postStepStatus() << " preVolume "
              << h.volName(s->GetPreStepPoint()) << " postVolume "
              << h.volName(s->GetPostStepPoint()) << " CurrentCopyNumbers "
              << getCopyNumber(s, false) << " -  " << getCopyNumber(s, true)
              << std::endl
  ...
  G4bool process(G4Step GEANT4_CONST_STEP *step, G4TouchableHistory *) {
    ...
    // FIXME:
    // a particle that crosses the boundry between two pad-ring halves will have
    // the hit placed on this surface at the last crossing point, and will be
    // assinged the total energy deposited in the whole pad-ring. This is a
    // possible source of bias for the hit
    ...
    float ptSQRD =
        thisMomentum[0] * thisMomentum[0] + thisMomentum[1] * thisMomentum[1];
    ...
    if (ptSQRD >= (Control.TPCLowPtCut * Control.TPCLowPtCut)) {
      ...
      // This step does not continue the previous path. Deposit the energy up to
      // the previous step
      ...
      if (step->GetPostStepPoint()->GetStepStatus() == fGeomBoundary) {
        ...
        if (innercopy ==
            getCopyNumber(step,
                          true)) { // pre == post; step within the same pair of
                                   // upper and lower pad ring halves
          // this step must have ended on the boundry between these two pad ring
          // halfves record the tracks coordinates at this position
          ...
          // here we memorise the padrow positions, we need them in some special
          // cases
          if (padRowCentralRadii.find(innercopy) == padRowCentralRadii.end()) {
            padRowCentralRadii[innercopy] =
                sqrt(pow(CrossingOfPadRingCentre.x(), 2) +
                     pow(CrossingOfPadRingCentre.y(), 2));
          }
        ...
        if (step->GetPostStepPoint()->GetKineticEnergy() ==
            0) { // particle stopped in padring
          DepositHiPtHit(step);
        } else if (step->GetPostStepPoint()
          ...
          Geant4Tracker::Hit *hit = new Geant4Tracker::Hit(
              ...
              0.0,                        // dE set to ZERO
              globalTimeAtPadRingCentre); // this time may or may not be defined
                                          // already
    ...
    //   ptSQRD < (Control.TPCLowPtCut*Control.TPCLowPtCut)
    ...
    else if (Control.TPCLowPtStepLimit) { // low pt tracks will be treated
                                          // differently as their step length is
                                          // limited by the special low pt
                                          // steplimiter

      //--------------------------------
      // first check if there is padrow stuff left over to deposit
      if (dEInPadRow > 0) {
        G4cout << " WARNING left over padrow stuff (from high pt tracks) "
                  "deposit hipt"
               << std::endl;
      ...
      if (previousStep && (previousStep->GetPostStepPoint()->GetPosition() -
                           step->GetPreStepPoint()->GetPosition())
                                  .mag() > 1.0e-6 * CLHEP::mm) {

        // This step does not continue the previous path. Deposit the energy and
        // begin a new Pt hit.
        ...
        if (CumulativeEnergyDeposit > fThresholdEnergyDeposit) {
          ...
          // The previous track has ended and the cumulated energy left at the
          // end was not enough to ionize
      ...
      if (step->GetPostStepPoint()->GetKineticEnergy() ==
          0) { // particle stopped
        if (CumulativeEnergyDeposit >
            fThresholdEnergyDeposit) { // enough energy
      ...
      if ((CumulativePathLength > Control.TPCLowPtMaxHitSeparation)) {
        ...
        // hit is deposited because the step limit is reached and there is
  ...
  void
  DepositHiPtHit(G4Step GEANT4_CONST_STEP *step) // DJ extracted to separate fn
  {
    if (dEInPadRow > fThresholdEnergyDeposit) {

      bool rareError = false;

      if (CrossingOfPadRingCentre[0] < 0.1 &&
          CrossingOfPadRingCentre[1] < 0.1 &&
          CrossingOfPadRingCentre[2] < 0.1) {
        // series of steps did not cross the centre of pad row; make reasonable
        // estimate
        int innercopy = getCopyNumber(step, false);
        if (padRowCentralRadii.find(innercopy) !=
            padRowCentralRadii.end()) { // we know radius of this pad row
          // average of first and last points of this series of steps
          const G4ThreeVector AvePos =
              0.5 * (StepAtEntranceToPadRing->GetPreStepPoint()->GetPosition() +
                     step->GetPostStepPoint()->GetPosition());
          G4double radius = sqrt(pow(AvePos.x(), 2) + pow(AvePos.y(), 2));
          CrossingOfPadRingCentre =
              AvePos * (padRowCentralRadii[innercopy] /
                        radius); // move radially to centre of pad row
          // time and momentum: average of intial and final
          globalTimeAtPadRingCentre =
              0.5 * (StepAtEntranceToPadRing->GetTrack()->GetGlobalTime() +
                     step->GetTrack()->GetGlobalTime());
          MomentumAtPadRingCentre =
              0.5 * (StepAtEntranceToPadRing->GetPreStepPoint()->GetMomentum() +
                     step->GetPostStepPoint()->GetMomentum());
        } else { // have not yet seen this pad row in this job
          // in principle we can get this from geometry information, but don't
          // want to add extra dependencies...
          G4cout << " WARNING: dont yet know radius of this pad row...ignoring "
                    "this energy deposit (should be v rare: eg possibly in "
                    "first track(s) of first event)"
                 << std::endl;
          rareError = true;
        }
      }
      ...
      if (!rareError) {
        Geant4Tracker::Hit *hit = new Geant4Tracker::Hit(
            step->GetTrack()->GetTrackID(),
            step->GetTrack()->GetDefinition()->GetPDGEncoding(), dEInPadRow,
            globalTimeAtPadRingCentre);
        ...
        sensitive->printM2(
            "+++ TrackID:%6d [%s] CREATE TPC hit at pad row crossing :"
  ...
  void DepositLowPtHit(G4Step GEANT4_CONST_STEP *step) {
    ...
    Geant4Tracker::Hit *hit = new Geant4Tracker::Hit(
        ...
        CumulativeMeanTime /
            CumulativeNumSteps); // DJ : simple average of all steps' time
  ...
  void CumulateLowPtStep(G4Step GEANT4_CONST_STEP *step) {
    ...
    ++CumulativeNumSteps;
    CumulativeMeanPosition += (step->GetPreStepPoint()->GetPosition() +
                               step->GetPostStepPoint()->GetPosition()) /
                              2;
    CumulativeMeanMomentum += (step->GetPreStepPoint()->GetMomentum() +
                               step->GetPostStepPoint()->GetMomentum()) /
                              2;
...
template <> void Geant4SensitiveAction<TPCSDData>::initialize() {
  ...
  declareProperty("TPCLowPtMaxHitSeparation",
                  m_userData.Control.TPCLowPtMaxHitSeparation);
...
template <> void Geant4SensitiveAction<TPCSDData>::defineCollections() {
  m_collectionID =
      defineCollection<Geant4Tracker::Hit>(m_sensitive.readout().name());
  m_userData.fSpaceHitCollectionID =
      defineCollection<Geant4Tracker::Hit>("TPCSpacePointCollection");
  m_userData.fLowPtHitCollectionID =
      defineCollection<Geant4Tracker::Hit>("TPCLowPtCollection");
}
...
/// Method for generating hit(s) using the information of G4Step object.
template <>
G4bool Geant4SensitiveAction<TPCSDData>::process(G4Step GEANT4_CONST_STEP *step,
