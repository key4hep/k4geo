...
namespace sim {
  ...
  struct CalorimeterWithPreShowerLayer : public Geant4Calorimeter {
    ...
    CalorimeterWithPreShowerLayer()
        : Geant4Calorimeter(), _preShowerCollectionID(0), _firstLayerNumber(1), // fixme: can we make this a parameter ?
          _preShowerCollection(0) {}
  };

  /// Define collections created by this sensitivie action object
  template <>
  void Geant4SensitiveAction<CalorimeterWithPreShowerLayer>::defineCollections() {
  ...
  bool Geant4SensitiveAction<CalorimeterWithPreShowerLayer>::process(G4Step const* step, G4TouchableHistory*) {
    ...
    try {
      ...
      out << "Position: "
          ...
          << "Post (" << std::setw(24) << step->GetPostStepPoint()->GetPosition() << ") " << std::endl;
      out << "Momentum: "
          << " Pre (" << std::setw(24) << step->GetPreStepPoint()->GetMomentum() << ") "
          << " Post (" << std::setw(24) << step->GetPostStepPoint()->GetMomentum() << ") " << std::endl;
