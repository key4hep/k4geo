...
namespace sim {
  ...
  public:
    ...
    Geant4Output2EDM4hep_DRC(Geant4Context* ctxt, const std::string& nam);
    /// Default destructor
    virtual ~Geant4Output2EDM4hep_DRC();
  ...
  template <>
  void EventParameters::extractParameters(podio::Frame& frame) {
  ...
  template <>
  void RunParameters::extractParameters(podio::Frame& frame) {
    for (auto const& p : this->intParameters()) {
...
/// #include <DDG4/Geant4Output2EDM4hep_DRC.h>
///  Geant4 headers
...
DECLARE_GEANT4ACTION(Geant4Output2EDM4hep_DRC)

/// Standard constructor
Geant4Output2EDM4hep_DRC::Geant4Output2EDM4hep_DRC(Geant4Context* ctxt, const std::string& nam)
...
// Callback to store the Geant4 run information
void Geant4Output2EDM4hep_DRC::beginRun(const G4Run* run) {
  G4AutoLock protection_lock(&action_mutex);
...
/// Commit data at end of filling procedure
void Geant4Output2EDM4hep_DRC::commit(OutputContext<G4Event>& /* ctxt */) {
  if (m_file) {
    G4AutoLock protection_lock(&action_mutex);
...
/// Callback to store the Geant4 run information
void Geant4Output2EDM4hep_DRC::saveRun(const G4Run* run) {
  G4AutoLock protection_lock(&action_mutex);
...
/// Data conversion interface for MC particles to EDM4hep format
void Geant4Output2EDM4hep_DRC::saveParticles(Geant4ParticleMap* particles) {
  ...
  if (pm.size() > 0) {
    ...
    for (const auto& iParticle : pm) {
      ...
      //      std::cout << " ********** mcp status : 0x" << std::hex << p->status << ",
      //      mask.isSet(G4PARTICLE_GEN_STABLE) x" << std::dec << mask.isSet(G4PARTICLE_GEN_STABLE)  <<std::endl ;
      ...
      if (p->genStatus) {
        ...
        else if (mask.isSet(G4PARTICLE_GEN_DECAYED))
          mcp.setGeneratorStatus(2);
        else if (mask.isSet(G4PARTICLE_GEN_DOCUMENTATION))
          mcp.setGeneratorStatus(3);
        else if (mask.isSet(G4PARTICLE_GEN_BEAM))
          mcp.setGeneratorStatus(4);
        else if (mask.isSet(G4PARTICLE_GEN_OTHER))
          mcp.setGeneratorStatus(9);
      }
...
/// Callback to store the Geant4 event
void Geant4Output2EDM4hep_DRC::saveEvent(OutputContext<G4Event>& ctxt) {
  ...
  printout(INFO, "Geant4Output2EDM4hep_DRC", "+++ Saving EDM4hep event %d run %d.", eventNumber, runNumber);
...
/// Callback to store each Geant4 hit collection
void Geant4Output2EDM4hep_DRC::saveCollection(OutputContext<G4Event>& /*ctxt*/, G4VHitsCollection* collection) {
