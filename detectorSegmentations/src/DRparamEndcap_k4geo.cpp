...
DRparamEndcap_k4geo::DRparamEndcap_k4geo() {
  fIsRHS = 0;
  fPhiZRot = 0.;
  fInnerX = 0.;
  fTowerH = 0.;
...
void DRparamEndcap_k4geo::SetDeltaThetaByTowerNo(int signedTowerNo, int BEtrans) {
...
void DRparamEndcap_k4geo::SetThetaOfCenterByTowerNo(int signedTowerNo, int BEtrans) {
  if (!fFilled) throw std::runtime_error("Attempt to set by tower num while endcap parameter is not filled!");
