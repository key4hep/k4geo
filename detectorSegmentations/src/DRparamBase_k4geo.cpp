...
namespace DDSegmentation {
  ...
  dd4hep::Position DRparamBase_k4geo::GetAssemblePos(int numPhi) {
    double numPhi_ = (double)numPhi;
    double x = std::cos(numPhi_ * fPhiZRot) * fCurrentCenter.X() * (fCurrentCenter.Mag() + fSipmHeight / 2.) /
               fCurrentCenter.Mag();
    double y = std::sin(numPhi_ * fPhiZRot) * fCurrentCenter.X() * (fCurrentCenter.Mag() + fSipmHeight / 2.) /
               fCurrentCenter.Mag();
  ...
  dd4hep::Position DRparamBase_k4geo::GetSipmLayerPos(int numPhi) {
    double numPhi_ = (double)numPhi;
    double x = std::cos(numPhi_ * fPhiZRot) * fCurrentCenter.X() *
               (fCurrentCenter.Mag() + fTowerH / 2. + fSipmHeight / 2.) / fCurrentCenter.Mag();
    double y = std::sin(numPhi_ * fPhiZRot) * fCurrentCenter.X() *
               (fCurrentCenter.Mag() + fTowerH / 2. + fSipmHeight / 2.) / fCurrentCenter.Mag();
