...
Vector3D FCCSWEndcapTurbine_k4geo::position(const CellID& cID) const {
  ...
  std::cout << " Cell ID " << std::hex << cID << std::dec << " results in position rho, phi, z = " << rhoVal << " " << phiVal << " " << zVal << std::endl;
...
CellID FCCSWEndcapTurbine_k4geo::cellID(const Vector3D& /* localPosition */, const Vector3D& globalPosition,
  ...
  if (iRho >= m_numReadoutRhoLayers[iWheel]) {
    ...
    std::cout << "Found rho out in the hinterlands " << lRho << std::endl;
  }
  ...
  std::cout << "grid_size_z = " << m_gridSizeZ[iWheel] << " offset_z = " << m_offsetZ[iWheel] << std::endl;
  std::cout << "grid_size_rho = " << m_gridSizeRho[iWheel] << " offset_rho = " << m_offsetRho[iWheel] << std::endl; 
  std::cout << "Global position rho, phi, z = " << lRho << " " <<  phiFromXYZ(globalPosition) << " " <<  globalPosition.Z << " results in cellID " << std::hex << cID << std::dec << std::endl;
